...
Seeds a running zynk server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  ZYNK_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/zynk-dev/zynk.sock.
  ZYNK_NAV_CWD          Workspace cwd for created panes. Defaults to the repo root.
  ZYNK_NAV_BIN          Zynk binary to call. Defaults to cargo run from the repo.
USAGE
}
...
workspace_cwd="${ZYNK_NAV_CWD:-$repo_dir}"
config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
dev_socket="$config_home/zynk-dev/zynk.sock"
main_socket="$config_home/zynk/zynk.sock"
export ZYNK_SOCKET_PATH="${ZYNK_NAV_SOCKET_PATH:-$dev_socket}"

if [[ "$allow_main" != 1 && "$ZYNK_SOCKET_PATH" == "$main_socket" ]]; then
  echo "refusing to seed main zynk session: $ZYNK_SOCKET_PATH" >&2
  echo "use ZYNK_NAV_SOCKET_PATH for a dev socket, or pass --allow-main intentionally" >&2
  exit 1
fi

if [[ ! -S "$ZYNK_SOCKET_PATH" ]]; then
  echo "zynk socket not found: $ZYNK_SOCKET_PATH" >&2
  echo "start a dev server first, or set ZYNK_NAV_SOCKET_PATH" >&2
  exit 1
fi

cd "$repo_dir"

if [[ -n "${ZYNK_NAV_BIN:-}" ]]; then
  run() { "$ZYNK_NAV_BIN" "$@"; }
else
  run() { cargo run --quiet -- "$@"; }
fi
...
cat <<EOF
Seeded navigator demo data via $ZYNK_SOCKET_PATH
