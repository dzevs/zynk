...
def git_subjects(rev_range: str) -> list[str]:
    # A push that creates a new ref/branch (e.g. the first push to a fresh public repo) sends an
    # all-zero "before" SHA. `git log <zeros>..<after>` would fail, so validate the commits
    # reachable from <after> instead. A ref deletion (<x>..<zeros>) has nothing to validate.
    if ".." in rev_range:
        before, _, after = rev_range.partition("..")
        before, after = before.strip(), after.strip()
        if _ALL_ZEROS.fullmatch(after):
            return []
        if _ALL_ZEROS.fullmatch(before):
            rev_range = after
