...
ZYNK_BIN="${ZYNK_BIN:-$ROOT/target/debug/zynk}"
BASE="${BASE:-$(mktemp -d /tmp/zynk-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_zynk "$session" server stop >/dev/null 2>&1 || true
  done
...
run_zynk() {
  ...
  env -u ZYNK_SOCKET_PATH \
    -u ZYNK_CLIENT_SOCKET_PATH \
    -u ZYNK_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$ZYNK_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/zynk-dev' "$CONFIG_HOME"
  else
    printf '%s/zynk-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/zynk.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/zynk-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/zynk-dev/zynk.sock | "$CONFIG_HOME"/zynk-dev/sessions/*/zynk.sock)
      ;;
    *)
...
echo "using zynk: $ZYNK_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/zynk-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/zynk-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_zynk "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
