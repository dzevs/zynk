...
class GitleaksTrackedTests(unittest.TestCase):
    def _repo(self, files):
        ...
        for rel, text in files.items():
            ...
            p.write_text(text)
        run_git(d, "add", "-A")
        run_git(d, "commit", "-qm", "init")
        return d

    def _run(self, d):
        return subprocess.run(["bash", "scripts/gitleaks_tracked.sh"], cwd=d, env=git_env()).returncode
    ...
    def test_staged_nested_gitleaks_toml_not_exempted(self):
        ...
        run_git(d, "add", "-A")
        rc = subprocess.run(["gitleaks", "protect", "--staged", "--config", ".gitleaks.toml"], cwd=d, env=git_env()).returncode
