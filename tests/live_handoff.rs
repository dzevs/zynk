...
fn spawn_server_with_args_and_socket_env(
    ...
    if let Some(api_socket_env) = api_socket_env {
        cmd.env("ZYNK_SOCKET_PATH", api_socket_env);
    } else {
        cmd.env_remove("ZYNK_SOCKET_PATH");
    }
    if let Some(client_socket_env) = client_socket_env {
        cmd.env("ZYNK_CLIENT_SOCKET_PATH", client_socket_env);
    } else {
        cmd.env_remove("ZYNK_CLIENT_SOCKET_PATH");
    }
    cmd.env("SHELL", "/bin/sh");
...
fn live_handoff_ignores_leaked_default_socket_env_for_named_session() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let default_session_dir = config_home.join("zynk-dev");
    let default_api_socket = default_session_dir.join("zynk.sock");
    let default_client_socket = default_session_dir.join("zynk-client.sock");
    let work_session_dir = config_home.join("zynk-dev/sessions/work");
    let work_api_socket = work_session_dir.join("zynk.sock");
    let work_client_socket = work_session_dir.join("zynk-client.sock");
...
fn live_handoff_preserves_client_socket_env_without_api_socket_env() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("zynk-dev/zynk.sock");
...
fn live_handoff_preserves_installed_plugins() {
    ...
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("zynk-dev/zynk.sock");
    let registry_path = config_home.join("zynk-dev/plugins.json");
