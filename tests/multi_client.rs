...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/zynk-multi-client-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(config_home: &Path, runtime_dir: &Path, api_socket_path: &Path) -> SpawnedZynk {
    // Write config into the app dir the running binary actually reads. In debug
    // builds that is `zynk-dev` (see `app_dir_name()`); release builds read
    // `zynk`. Skipping onboarding gives a deterministic startup (no onboarding
    // overlay) so the tests drive panes directly. The pane each test sends input
    // to is the one it creates with `focus: true` (see
    // `create_workspace_and_root_pane`), so it is always the active, input-routed
    // pane regardless of any auto-created default workspace.
...
fn server_log_path(config_home: &Path) -> PathBuf {
    ...
    config_home.join(app_dir).join("zynk-server.log")
}
