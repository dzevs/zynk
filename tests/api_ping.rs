...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    support::test_root().join(format!("hapi-{}-{nanos}", std::process::id()))
}
...
fn spawn_zynk(config_home: &Path, runtime_dir: &Path, socket_path: &Path) -> SpawnedZynk {
    spawn_zynk_with_options(config_home, runtime_dir, socket_path, None, "/bin/sh")
}
...
fn spawn_zynk_with_shell(
    ...
    spawn_zynk_with_options(config_home, runtime_dir, socket_path, None, shell)
}

fn spawn_zynk_with_options(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
    path_override: Option<&Path>,
    ...
    // #117 DB isolation: the server migrates the global DB at startup. Pin it to an
    // isolated per-test sqlite home AND scrub any inherited live ZYNK_HOME so a live
    // value can never leak in — the server can never touch ~/.zynk.
    cmd.env("ZYNK_SQLITE_HOME", config_home.join("sqlite"));
    cmd.env_remove("ZYNK_HOME");
    cmd.env("SHELL", shell);
    cmd.env_remove("ZYNK_ENV");
    cmd.env_remove("ZYNK_STARTUP_CWD");
...
fn server_reload_agent_manifests_reports_runtime_override() {
    ...
    // zynk fork: app_dir_name() is "zynk-dev" in debug (upstream used "zynk-dev"); the override
    // dir must match what the server reads. See docs/zynk/fork-patch-ledger.md.
    let override_dir = config_home.join("zynk-dev").join("agent-detection");
...
fn events_subscribe_streams_workspace_tab_and_agent_events() {
    ...
    let child = spawn_zynk_with_path(
        &config_home,
        &runtime_dir,
        &socket_path,
...
fn pane_report_agent_updates_effective_state() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_hook_5","method":"pane.report_agent","params":{{"pane_id":"{}","source":"zynk:pi","agent":"pi","state":"working","message":"thinking","agent_session_path":"{}"}}}}"#,
            pane_id,
    ...
    assert_eq!(pane["result"]["pane"]["agent_session"]["source"], "zynk:pi");
...
fn pane_release_agent_suppresses_reacquire_during_graceful_exit() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"zynk:pi","agent":"pi","state":"working"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(hook["result"]["type"], "ok");
    ...
    let released = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_release_5","method":"pane.release_agent","params":{{"pane_id":"{}","source":"zynk:pi","agent":"pi"}}}}"#,
            pane_id
        ),
    );
...
fn pane_clear_agent_authority_restores_fallback_state() {
    ...
    let hook = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_clear_4","method":"pane.report_agent","params":{{"pane_id":"{}","source":"zynk:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(hook["result"]["type"], "ok");
    ...
    let cleared = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_clear_5","method":"pane.clear_agent_authority","params":{{"pane_id":"{}","source":"zynk:pi"}}}}"#,
            pane_id
        ),
    );
...
fn metadata_status_subscription_filter_and_ttl_expiry_are_observable() {
    ...
    let report_agent = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_meta_sub_2","method":"pane.report_agent","params":{{"pane_id":"{}","source":"zynk:pi","agent":"pi","state":"working"}}}}"#,
            pane_id
        ),
    );
