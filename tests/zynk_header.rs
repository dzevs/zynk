...
fn unique_base() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    support::test_root().join(format!("zynk-header-test-{}-{nanos}", std::process::id()))
}
...
fn spawn_server_process(
    ...
    let pair = native_pty_system()
        .openpty(PtySize {
            rows: 24,
            // Wide enough that a long CI-runner hostname prompt + the header box do not soft-wrap
            // in this fixture. The header box is capped by `HeaderOptions.max_width` (default 100,
            // NOT pane-relative), so 240 cols leaves ample room. Header content assertions also read
            // `recent_unwrapped` so they ignore any residual soft-wrap.
            cols: 240,
            pixel_width: 0,
            pixel_height: 0,
        })
...
fn pane_recent_text(socket_path: &Path, pane_id: &str) -> String {
    let response = send_json(
        socket_path,
        &format!(
            "{{\"id\":\"read\",\"method\":\"pane.read\",\"params\":{{\"pane_id\":\"{pane_id}\",\"source\":\"recent_unwrapped\",\"lines\":200}}}}"
        ),
    );
...
/// target. A plain shell EXECUTES the injected multi-line header and interleaves its own
/// `command not found` errors into the header lines (corrupting longer lines like the note), so an
/// asserted substring may never be contiguous; `cat` echoes the delivered bytes verbatim, keeping the
/// header contiguous while still exercising real CLI header injection into an agent-target PTY.
...
fn assert_header_prepended_for_agent(agent: &str) {
    ...
    // The delivered pane text carries the visible header box + body. Wait for each expected
    // fragment to RENDER (async, fragment-by-fragment) before snapshotting: a single early
    // snapshot after only the top border can miss later lines, and a wrapped CI shell prompt can
    // interleave noise. The header renders top-to-bottom then the body, so these appear in order.
    let id_line = format!("id:   {message_id}");
    for needle in [HEADER_TOP, id_line.as_str(), "conv:", "zbodysentinel"] {
        assert!(
            wait_for_pane_text(&fixture.socket_path, &pane, needle, Duration::from_secs(10)),
            "agent {agent}: expected {needle:?} in delivered pane text; pane text: {:?}",
            pane_recent_text(&fixture.socket_path, &pane)
        );
    }
    ...
    // The header carries id + conv lines (awareness) and the message body (re-checked on the
    // final snapshot; the waits above guarantee they have rendered).
    assert!(
        pane_text.contains(&id_line),
...
fn header_verbose_env_shows_reply_and_note() {
    ...
    for needle in [HEADER_NOTE, "reply: zynk reply"] {
        assert!(
            wait_for_pane_text(&fixture.socket_path, &pane, needle, Duration::from_secs(10)),
            "ZYNK_HEADER_VERBOSE=1 must SHOW {needle:?}; pane text: {:?}",
            pane_recent_text(&fixture.socket_path, &pane)
        );
    }
