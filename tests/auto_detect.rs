...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/zynk-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
fn spawn_server(
    ...
    cmd.env("ZYNK_SOCKET_PATH", api_socket_path);
...
/// Spawn `zynk` (no subcommand) — the auto-detect launch path.
fn spawn_zynk_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
...
/// Spawn `zynk --no-session` — the monolithic escape hatch.
fn spawn_zynk_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedZynk {
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `zynk` (no subcommand) — should auto-detect, spawn server, attach as client.
    let zynk = spawn_zynk_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = zynk.child.process_id().expect("client should have PID");
    assert!(
...
/// Running `zynk` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `zynk` (no subcommand) — should detect the running server and attach.
    ...
    cleanup_spawned_zynk(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_zynk(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `ZYNK_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `zynk` with custom socket paths.
    let zynk = spawn_zynk_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `zynk --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `zynk --no-session` — monolithic mode, no server/client.
    let zynk = spawn_zynk_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one zynk process.
    let client_pid = zynk.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
fn cli_subcommands_work_through_server() {
    ...
    // Test `zynk workspace list` through the server's API socket.
    ...
    // Test `zynk pane list` through the server's API socket.
...
/// persists after the client exits, and a new `zynk` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `zynk` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_zynk_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `zynk` again — should detect the running server and reattach.
    let client2 = spawn_zynk_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_zynk(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set ZYNK_SOCKET_PATH or ZYNK_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("zynk.sock");
    let client_socket = config_home.join(app_dir_name).join("zynk-client.sock");
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_zynk_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("zynk-client.log");
    let server_log = log_dir.join("zynk-server.log");
    let monolith_log = log_dir.join("zynk.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in zynk.log: {monolith_content}"
    );
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_zynk_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
