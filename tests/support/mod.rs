...
pub fn unregister_spawned_zynk_pid(pid: Option<u32>) {
    let Some(pid) = pid else {
        return;
    };
...
pub fn wait_for_socket(path: &Path, timeout: Duration) {
    ...
    // spawned server can take longer than a few seconds to bind its socket on a saturated host.
    // Floor the wait generously so default `just check` is deterministic — the happy path still
    // returns in <1s when the socket appears; the larger ceiling only delays a genuine failure.
...
pub fn cleanup_registered_zynk_pids() {
    let pids: Vec<u32> = {
        let mut registry = pid_registry_lock();
...
fn start_global_watchdog() {
    thread::spawn(|| loop {
        ...
        if let Err(err) = cleanup_servers_with_missing_runtime_dir() {
            eprintln!("zynk test cleanup watchdog error: {err}");
        }
    });
}
...
fn iter_worktree_server_pids() -> std::io::Result<Vec<u32>> {
    ...
    for entry in proc_entries {
        ...
        if is_test_zynk_server_process(pid) {
            pids.push(pid);
        }
    }
...
fn is_test_zynk_server_process(pid: u32) -> bool {
    let Some(exe_path) = proc_link_target(pid, "exe") else {
        return false;
    };

    if !is_test_zynk_binary(&exe_path) {
        return false;
    }
...
fn process_runtime_dir(pid: u32) -> std::io::Result<Option<PathBuf>> {
    ...
    for entry in environ.split(|byte| *byte == 0) {
        ...
        if let Some(value) = kv.strip_prefix("ZYNK_SOCKET_PATH=") {
...
mod tests {
    ...
    fn unique_missing_runtime_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "zynk-watchdog-scoping-{label}-{}-{unique}",
            std::process::id()
        ))
    }
    ...
    fn test_binary_matcher_accepts_current_checkout_debug_binary() {
        let binary = current_checkout_root().join("target/debug/zynk");
        assert!(
