...
use support::{
    ...
    register_runtime_dir, register_spawned_zynk_pid, unregister_spawned_zynk_pid,
    ...
    while Instant::now() < deadline {
        match read_server_message(&mut stream) {
            Ok((1, payload)) => {
                ...
                if last_frame_text.contains("config.toml:1:6")
                    && last_frame_text.contains("zynk config check")
                {
...
fn server_unreachable_shows_clear_error() {
    ...
    assert!(
        stderr.contains("Is zynk server running?"),
...
/// Two of the mouse-disable modes crossterm's `DisableMouseCapture` emits on
...
fn attach_thin_client(
...
) -> (SpawnedZynk, SpawnedZynk, SharedOutput) {
    ...
    while Instant::now() < deadline {
        ...
        if out.to_lowercase().contains("zynk:") {
            break;
        }
        thread::sleep(Duration::from_millis(30));
    }
...
fn assert_client_restores_terminal(trigger: impl FnOnce(&mut SpawnedZynk, &mut SpawnedZynk)) {
    let _lock = test_lock();
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    ...
    // SpawnedZynk::Drop kills and reaps both processes with a bounded wait.
    drop(spawned_server);
...
/// The path this commit actually fixes: a direct SIGHUP/SIGTERM to the client
/// (e.g. a terminal emulator SIGHUPing its foreground child on window close).
/// With `ctrlc`'s `termination` feature the handler sets `should_quit`, the
/// loop exits, and `TerminalGuard::Drop` restores the terminal. Without it the
