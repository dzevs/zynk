...
| DB init race (product bug) | `src/zynk/db.rs` (fork-owned) | Two zynk processes opening a fresh shared DB at once (two named-session servers; a CLI racing a starting server): sqlx creates `_sqlx_migrations` before the first migration commits, the second opener classified the ledger-only DB as FOREIGN and exited (`db_foreign_conflict`). Fix: `InitLock` (advisory lock on `<db>.init-lock` around classify + migrate, acquired COOPERATIVELY — `try_lock` + async sleep, 30 s bound → `db_init_lock_timeout` — so two openers on one executor cannot deadlock) + classification now reasons on the COMPLETE table set (`all_user_table_names`; the suffix-filtered `user_table_names` is display-only): a ledger-only/EMPTY DB classifies `Empty`, any other foreign table — including suffix-filtered names like `customer_data` — stays Foreign and byte-identical (ADR 0008 boundary unchanged; this also closes a pre-existing hole where a DB whose only table was suffix-filtered classified Empty). Found via the `named_sessions_use_separate_servers_and_workspace_state` flake (2/40 under load, captured). |
| DB guard hardening (Gate-3) | `src/zynk/db.rs` (fork-owned), `src/server/headless.rs`, new `docs/zynk/decisions/0011-db-inspection-sidecar-boundary.md` | Swarm Gate-3 reproduced three ways the guard granted writable authority from non-provenance evidence (foreign tables named like our lineage tables; foreign view/index/trigger only; a non-UTF-8 table name silently dropped). Classification now reads the complete `sqlite_master` object set losslessly and recognizes a native DB by migration PROVENANCE (ledger version+checksum vs the built-in migrator); the init lock is taken only when initialization OR a pending migration is needed (a fully current native DB never waits; bound 5 s); the schema, ledger and currentness are read inside ONE read transaction (single snapshot — a concurrent initializer's commit between two reads used to turn a genuine init into a false Foreign) with a production-wrapper interleaving regression + a negative control; the lineage policy requires a valid prefix of our migrations with our checksums, `success = 1`, and tolerates unknown versions only when newer; every DB open error is fatal at server start AND written to the durable server log (the daemon launcher nulls stderr). ADR 0011 records the inspection boundary: existing data bytes (main file + `-wal` content) never modified; `-shm` and an empty `-wal` may be created (EXCLUSIVE/immutable alternatives rejected with evidence). |
| tracked gaps (not fixed here) | — | (1) handoff-import server path (`run_handoff_import_server`) binds before the DB guard and `new_from_handoff` leaves the DB workers `None` — pre-existing; needs its own regression + review. (2) bundled SQLite 3.46.0 (`libsqlite3-sys` 0.30.1) predates the 3.51.3 WAL-reset fix (very rare per SQLite) — bump on the dependency branch with vec0/ADR 0006 re-verification. Both: review by **2026-09-30**. |
