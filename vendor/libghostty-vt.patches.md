...
upstream issue: ghostty resizecols cursor subtraction
...
upstream issue: kitty graphics image transmit time accessor
...
reason: zynk fingerprints kitty image data to decide when to re-encode an
...
`GHOSTTY_KITTY_IMAGE_DATA_TRANSMIT_TIME_NS` gives zynk an exact, O(1) change
...
```sh
zig build test-lib-vt -Demit-lib-vt -Dtest-filter="image_get transmit_time_ns changes on retransmission"
cargo nextest run --bin zynk kitty_image_fingerprint
```
