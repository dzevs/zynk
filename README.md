...
Download a prebuilt binary from [GitHub Releases](https://github.com/dzevs/zynk/releases), or use Nix or
build from source.

**Prebuilt binary** — Linux x86_64 (glibc ≥ 2.30):
...
`cargo install zynk` does **not** install the native app yet — the `zynk` crate on crates.io is still the
retired 2.x ACP protocol/helper CLI; the native 3.x crate publish and a Homebrew tap are planned.
...
To update, download a newer [GitHub Release](https://github.com/dzevs/zynk/releases) binary, use `nix`, or
rebuild from source:
