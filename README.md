...
No published binary release yet — run with Nix or build from source.
...
**Build from source** — requires Rust (stable) and **Zig 0.15.2** (the bundled `libghostty-vt` is built with
Zig):
...
Prebuilt binaries and additional install methods are planned; there is no release channel yet. (This is the
`zynk` source repository — not the unrelated `zynk` crate on crates.io.)
...
Auto-update and update channels are not available yet — there is no published Zynk release, so `zynk update`
and `zynk channel set <stable|preview>` fail closed with a "not available until the first Zynk release" notice
rather than fetching anything. To update, rebuild from source:
...
binary. Self-update, "new version available" notifications, and the stable/preview update channels arrive with
the first Zynk release.
