...
/// Zynk-branded runtime log basenames (ADR 0007 §5). Created fresh at runtime, so
/// the rename needs no migration. Centralized so every call site stays consistent.
pub(crate) const LOG_FILE_MONOLITH: &str = "zynk.log";
pub(crate) const LOG_FILE_CLIENT: &str = "zynk-client.log";
pub(crate) const LOG_FILE_SERVER: &str = "zynk-server.log";
...
pub(crate) fn init_file_logging(file_name: &str) {
    ...
    // ADR 0007 §5: ZYNK_LOG is the primary log-filter env var; ZYNK_LOG is the
    // retained transitional compat alias. ZYNK wins when both are set. The default
    // directive must target the binary crate's `module_path!()` root, which is the
    // `[[bin]]` name `zynk` (ADR 0007 §1) — NOT the package name `zynk`. Cargo roots
    // `module_path!()` at the bin crate name, so a stale `zynk=info` directive matches
    // nothing and silently drops every log line. `zynk=info` is the correct, minimal
    // target (category-2 internal filter directive; revealed by the bin rename).
    let filter = EnvFilter::try_from_env("ZYNK_LOG")
        .or_else(|_| EnvFilter::try_from_env("ZYNK_LOG"))
        .unwrap_or_else(|_| EnvFilter::new("zynk=info"));
...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus zynk-client.log, zynk-server.log)",
        dir.join("zynk.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("zynk.log")
    }

    #[test]
    fn rotated_log_path_appends_numeric_suffix() {
        let path = PathBuf::from("/tmp/zynk.log");
        assert_eq!(rotated_log_path(&path, 2), PathBuf::from("/tmp/zynk.log.2"));
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "zynk.log", 8, 0).unwrap();
        {
