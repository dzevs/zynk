...
fn kitty_placeholder_diacritic_index(codepoint: u32) -> Option<u32> {
    let map = KITTY_PLACEHOLDER_DIACRITICS.get_or_init(|| {
        // Reuse Ghostty's vendored table so Zynk decodes the same placeholder
...
mod tests {
    ...
    fn terminal_callbacks_report_pty_responses_and_pwd_changes() {
        ...
        terminal.write(b"\x1b[6n\x1b]7;file:///tmp/zynk\x07");

        let output = responses.lock().unwrap().clone();
        assert!(!output.is_empty());
        assert!(String::from_utf8_lossy(&output).contains("R"));
        assert_eq!(terminal.take_pwd_changes(), [b"file:///tmp/zynk".to_vec()]);
    }
    ...
    #[test]
    fn clipboard_callback_accepts_text_mime_parameters_at_the_size_limit() {
        let mut terminal = Terminal::new(10, 5, 0).unwrap();
        let data = vec![b'x'; MAX_CLIPBOARD_BYTES];
        let content = test_clipboard_content(b"Text/Plain; charset=UTF-8", &data);
        assert_eq!(
            invoke_clipboard_callback(
                &mut terminal,
                &[content],
                std::mem::size_of::<ffi::GhosttyClipboardWrite>()
            ),
            ffi::GhosttyClipboardWriteResult_GHOSTTY_CLIPBOARD_WRITE_RESULT_SUCCESS,
        );
        assert_eq!(terminal.take_clipboard_writes(), vec![data]);
    }

    #[test]
    fn clipboard_callback_refuses_null_requests_and_nonstandard_destinations() {
        let mut terminal = Terminal::new(10, 5, 0).unwrap();
        let userdata = (&mut *terminal.callback_state as *mut TerminalCallbackState).cast();
        // SAFETY: null inputs exercise the callback's guards, not dereferences.
        assert_eq!(
            unsafe { clipboard_write_trampoline(terminal.raw, userdata, ptr::null()) },
        ...
        let request = ffi::GhosttyClipboardWrite {
            ...
            location: ffi::GhosttyClipboardLocation_GHOSTTY_CLIPBOARD_LOCATION_PRIMARY,
            contents: ptr::null(),
            contents_len: 0,
        };
        // SAFETY: the full request lives through these synchronous calls.
        assert_eq!(
            unsafe { clipboard_write_trampoline(terminal.raw, userdata, &request) },
            ffi::GhosttyClipboardWriteResult_GHOSTTY_CLIPBOARD_WRITE_RESULT_UNSUPPORTED
        );
        // SAFETY: the userdata guard returns before accessing the request's content.
        assert_eq!(
            unsafe { clipboard_write_trampoline(terminal.raw, ptr::null_mut(), &request) },
            ffi::GhosttyClipboardWriteResult_GHOSTTY_CLIPBOARD_WRITE_RESULT_INVALID_DATA
        );
        assert!(terminal.take_clipboard_writes().is_empty());
    }
