...
pub(crate) fn render_virtual_with_runtime_registry(
    ...
    let pre_compute_suppresses_popup_cursor = app_state
        .popup_pane
        .as_ref()
    ...
    let cursor = if popup_visible {
        if pre_compute_suppresses_popup_cursor {
            None
        } else {
...
pub(crate) fn visible_hyperlinks(
    ...
    if let Some(popup) = &app_state.popup_pane {
        if let Some((outer, inner)) =
            crate::ui::popup_pane_rects(app_state, app_state.view.terminal_area)
        {
            links.retain(|((x, y), _, _)| !outer.contains((*x, *y).into()));
            if let Some(runtime) = terminal_runtimes.get(&popup.terminal_id) {
                links.extend(runtime.visible_hyperlinks(inner));
            }
        }
    }
    links
