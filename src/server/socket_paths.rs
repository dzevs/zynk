...
/// Zynk-branded client-socket-path override (ADR 0007 §5): the primary, documented name.
pub const ZYNK_CLIENT_SOCKET_PATH_ENV_VAR: &str = "ZYNK_CLIENT_SOCKET_PATH";
/// Transitional `ZYNK_*` compat alias for the client-socket-path override.
///
/// Contractual override behavior for auto-detect uses `ZYNK_SOCKET_PATH`
/// (`ZYNK_SOCKET_PATH` compat). This variable is kept as a fallback for callers
/// that explicitly need a client-only override when the API-socket override is
/// not set. `ZYNK_CLIENT_SOCKET_PATH` wins over this compat alias when both are set.
pub const CLIENT_SOCKET_PATH_ENV_VAR: &str = "ZYNK_CLIENT_SOCKET_PATH";

/// Socket permission mode (owner read/write only).
const SOCKET_PERMISSION_MODE: u32 = 0o600;

/// Resolve the client-socket-path override, preferring the Zynk-branded
/// `ZYNK_CLIENT_SOCKET_PATH` over the retained `ZYNK_CLIENT_SOCKET_PATH` compat
/// alias (ADR 0007 §5).
fn client_socket_path_override() -> Option<String> {
    crate::config::env_first(&[ZYNK_CLIENT_SOCKET_PATH_ENV_VAR])
}
...
/// 2. If the API-socket override (`ZYNK_SOCKET_PATH`, or `ZYNK_SOCKET_PATH` compat)
///    is set, derive the client socket path from it by inserting `-client` before
///    `.sock` (e.g. `zynk.sock` -> `zynk-client.sock`). This keeps JSON API and
///    client socket overrides consistent.
/// 3. Otherwise, honor the client-socket override (`ZYNK_CLIENT_SOCKET_PATH`, or
///    `ZYNK_CLIENT_SOCKET_PATH` compat; legacy/testing fallback).
...
pub fn client_socket_path() -> PathBuf {
    ...
    client_socket_path_from_overrides(
        crate::api::socket_path_override().as_deref(),
        client_socket_path_override().as_deref(),
    )
}
...
pub(crate) fn prepare_socket_path(path: &Path) -> io::Result<()> {
    crate::ipc::prepare_socket_path(path, |path| {
        format!(
            "zynk server is already running (socket busy at {})",
            path.display()
        )
    })
}
...
mod tests {
    ...
    fn client_socket_path_derived_from_api_socket_override() {
        let path = client_socket_path_from_overrides(Some("/tmp/test-zynk.sock"), None);
        assert_eq!(path, PathBuf::from("/tmp/test-zynk-client.sock"));
    }
    ...
    fn client_socket_path_respects_legacy_client_override_without_api_override() {
        let path = client_socket_path_from_overrides(None, Some("/tmp/test-zynk-client.sock"));
        assert_eq!(path, PathBuf::from("/tmp/test-zynk-client.sock"));
    }

    #[test]
    fn client_socket_path_defaults_to_config_dir() {
        std::env::remove_var(crate::session::ZYNK_SESSION_ENV_VAR);
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
        let path = client_socket_path_from_overrides(None, None);
        // ADR 0007 §5: default runtime client-socket basename is Zynk-branded.
        assert_eq!(path, crate::config::config_dir().join("zynk-client.sock"));
    }
    ...
    #[test]
    fn zynk_client_socket_path_env_is_honored() {
        let _g = crate::config::test_config_env_lock().lock().unwrap();
        std::env::set_var(
            ZYNK_CLIENT_SOCKET_PATH_ENV_VAR,
            "/tmp/zynk-client-primary.sock",
        );
        assert_eq!(
            client_socket_path_override().as_deref(),
            Some("/tmp/zynk-client-primary.sock")
        );
        std::env::remove_var(ZYNK_CLIENT_SOCKET_PATH_ENV_VAR);
    }
