...
impl HeadlessServer {
    ...
    pub(super) fn handle_pane_graphics_stream_frame(
        ...
        if self.shutting_down || self.should_quit.load(std::sync::atomic::Ordering::Acquire) {
            let _ = self.handle_api_request_with_shutdown_check_inner(msg, false);
            return RenderImpact::None;
        }

        let mut internal_changed = self.app.expire_due_metadata(std::time::Instant::now());
        internal_changed |= self.drain_all_internal_events_with_forwarding();
        internal_changed |= self.app.sync_pane_graphics_streams();
        ...
        let response = self
            .app
            .handle_api_request_after_internal_events_drained_from_socket(msg.request, msg.caller);
    ...
    pub(super) fn render_retained_graphics_update_and_stream(&mut self) -> RetainedGraphicsOutcome {
        ...
        for (client_id, encoded, next_graphics_cache) in prepared {
            ...
            let commit_cache = !encoded.skipped_oversized || !encoded.bytes.is_empty();
            let serialized = if encoded.bytes.is_empty() {
                None
            } else {
                match Self::frame_server_message_with_max(
                    &ServerMessage::Graphics {
                        bytes: frame_pane_graphics_for_client(encoded.bytes),
                    },
