...
//! Headless server mode — runs the zynk event loop without a real terminal.
//!
//! The server:
//! - Does not enter raw mode or read stdin
//! - Creates and listens on both `zynk.sock` (existing JSON API) and
//!   `zynk-client.sock` (new binary protocol)
...
fn retained_render_plan_with_graphics(
...
enum AltScreenReadConflict {
    ...
    DeferTerminal(crate::terminal::TerminalId),
...
enum DeferredAltScreenItem {
    Request(Box<api::ApiRequestMessage>),
    HandoffBarrier(DeferredHandoffId),
}

struct DeferredLiveHandoff {
    message: api::ApiRequestMessage,
    waiting: HashSet<crate::terminal::TerminalId>,
    terminals: Vec<crate::terminal::TerminalId>,
}

#[derive(Default)]
struct DeferredAltScreenRequests {
    terminal_queues: HashMap<crate::terminal::TerminalId, VecDeque<DeferredAltScreenItem>>,
    handoffs: BTreeMap<DeferredHandoffId, DeferredLiveHandoff>,
    terminal_barrier_counts: HashMap<crate::terminal::TerminalId, usize>,
    next_handoff_id: DeferredHandoffId,
    #[cfg(test)]
    terminal_queue_examination_count: usize,
}

impl DeferredAltScreenRequests {
    fn defer_request(
        &mut self,
        terminal_id: crate::terminal::TerminalId,
        message: api::ApiRequestMessage,
        ...
        self.terminal_queues
            .entry(terminal_id)
            .or_default()
            .push_back(DeferredAltScreenItem::Request(Box::new(message)));
    }
    ...
    fn defer_handoff(
        ...
        let terminals: Vec<_> = terminals
            .into_iter()
            .filter(|terminal_id| seen.insert(terminal_id.clone()))
            .collect();
        debug_assert!(!terminals.is_empty());
        let handoff_id = self.next_handoff_id;
        self.next_handoff_id = self.next_handoff_id.wrapping_add(1);
        for terminal_id in &terminals {
            self.terminal_queues
                .entry(terminal_id.clone())
                .or_default()
                .push_back(DeferredAltScreenItem::HandoffBarrier(handoff_id));
            *self
                .terminal_barrier_counts
                .entry(terminal_id.clone())
                .or_default() += 1;
        }
        self.handoffs.insert(
            ...
            DeferredLiveHandoff {
                message,
                waiting: terminals.iter().cloned().collect(),
                terminals,
            },
        );
    }

    fn terminal_is_blocked_by_handoff(&self, terminal_id: &crate::terminal::TerminalId) -> bool {
    ...
    fn take_terminal_queue(
        &mut self,
        terminal_id: &crate::terminal::TerminalId,
    ) -> Option<VecDeque<DeferredAltScreenItem>> {
        let queue = self.terminal_queues.remove(terminal_id);
        #[cfg(test)]
        if queue.is_some() {
            self.terminal_queue_examination_count =
                self.terminal_queue_examination_count.saturating_add(1);
        }
        queue
    }

    fn restore_terminal_queue(
        &mut self,
        terminal_id: crate::terminal::TerminalId,
        mut queue: VecDeque<DeferredAltScreenItem>,
    ) {
        if queue.is_empty() {
            return;
        }
        if let Some(existing) = self.terminal_queues.get_mut(&terminal_id) {
            existing.append(&mut queue);
        } else {
            self.terminal_queues.insert(terminal_id, queue);
        }
    }

    fn mark_handoff_barrier_reached(
        &mut self,
        handoff_id: DeferredHandoffId,
        terminal_id: &crate::terminal::TerminalId,
    ) {
        if let Some(handoff) = self.handoffs.get_mut(&handoff_id) {
            handoff.waiting.remove(terminal_id);
        }
    }

    fn take_ready_handoff(&mut self) -> Option<DeferredLiveHandoff> {
        let (&handoff_id, handoff) = self.handoffs.first_key_value()?;
        if !handoff.waiting.is_empty() {
            return None;
        }
        let handoff = self.handoffs.remove(&handoff_id)?;
        for terminal_id in &handoff.terminals {
            let remove = if let Some(count) = self.terminal_barrier_counts.get_mut(terminal_id) {
            ...
            if remove {
                self.terminal_barrier_counts.remove(terminal_id);
            }
        }
        Some(handoff)
    }

    fn drain_messages(&mut self) -> Vec<api::ApiRequestMessage> {
        let terminal_queues = std::mem::take(&mut self.terminal_queues);
        let handoffs = std::mem::take(&mut self.handoffs);
        self.terminal_barrier_counts.clear();
        let mut messages = Vec::new();
        for queue in terminal_queues.into_values() {
            messages.extend(queue.into_iter().filter_map(|item| match item {
                DeferredAltScreenItem::Request(message) => Some(*message),
                DeferredAltScreenItem::HandoffBarrier(_) => None,
            }));
        }
        messages.extend(handoffs.into_values().map(|handoff| handoff.message));
        messages
    }

    #[cfg(test)]
    fn is_empty(&self) -> bool {
        self.terminal_queues.is_empty()
            && self.handoffs.is_empty()
            && self.terminal_barrier_counts.is_empty()
    }
    ...
    fn len(&self) -> usize {
        self.terminal_queues
            ...
            .map(|queue| {
                queue
                    .iter()
                    .filter(|item| matches!(item, DeferredAltScreenItem::Request(_)))
                    .count()
            })
            .sum::<usize>()
...
pub struct HeadlessServer {
    ...
    /// Last outer title delivered, keyed by the client that received it.
    sent_window_title: Option<(u64, Option<String>)>,
    /// Explicit API override; while present it wins over `ui.window_title`.
...
impl HeadlessServer {
    ...
    pub async fn run(&mut self) -> io::Result<()> {
        ...
        loop {
            ...
            if self.pane_graphics_runtime_active() {
                let impact = self.drain_api_requests_with_render_impact();
            ...
            if self.pane_graphics_runtime_active() {
                let impact = self.drain_server_events_with_render_impact();
            ...
            if needs_render
                && (render_cadence_due
                    || (self.app.can_present_now(now)
                        && self.has_pending_presentation_work_with_graphics(
                            needs_full_render,
    ...
    /// so a deferred UI request and a `workspace.create` request take one code path.
    ///
    /// `skip_default_workspace_for_request` is set: this call IS the workspace/tab creation, so the
    /// post-request `ensure_default_workspace()` fallback must not race it (and must not paper over
    /// a failed create by seeding a default workspace instead).
    ...
    fn perform_live_handoff(
        ...
        #[cfg(test)]
        if let Some(result) = self.test_live_handoff_results.pop_front() {
        ...
        if pane_by_terminal.len() > crate::server::handoff::MAX_FDS_PER_HANDOFF {
            ...
            return Err(io::Error::new(
                ...
                format!(
                    "live handoff supports at most {} panes in one update; close panes or restart zynk normally",
    ...
    fn sync_terminal_title_sources(
        ...
        let changes = self.app.sync_terminal_title_sources(sources);
            ...
            ServerEvent::ClientPasteRejected {
                ...
                self.send_to_client(
                    ...
                    ServerMessage::Notify {
                        ...
                        body: Some(format!(
                            "Input message is {size} bytes; Zynk's limit is {max} bytes"
                        )),
                    },
                );
                false
    ...
    fn handle_server_event_with_render_impact(&mut self, ev: ServerEvent) -> RenderImpact {
        ...
        let changed = self.handle_server_event(ev);
        if !changed {
            RenderImpact::None
        } else if deferred == DeferredRender::Graphics {
            RenderImpact::Graphics
        } else {
            RenderImpact::Full
        }
    ...
    fn agent_read_not_idle_error(
        ...
        let target = self.app.resolve_terminal_target(&params.target).ok()?;
        let terminal = self
            .app
            .state
            .terminals
    ...
    fn resolve_alt_screen_terminal_id(&self, target: &str) -> Option<crate::terminal::TerminalId> {
        #[cfg(test)]
        self.alt_screen_target_resolution_count.set(
            self.alt_screen_target_resolution_count
                .get()
                .saturating_add(1),
        );
        let target = self.app.resolve_terminal_target(target).ok()?;
        self.app
            .state
            .workspaces
            .get(target.ws_idx)?
    ...
    fn alt_screen_read_conflict_with_release_context(
        &self,
        request: &api::schema::Request,
            ...
            return if traversal_pending
                || (!released_before_barrier
                    && self
                        ...
                        .terminal_is_blocked_by_handoff(&target_terminal_id))
            {
                AltScreenReadConflict::DeferTerminal(target_terminal_id)
            } else {
        ...
        {
            return AltScreenReadConflict::DeferHandoff(
        ...
        let (target, source, lines, format) = match &request.method {
            api::schema::Method::AgentRead(params) => (
                released_terminal_id
                    .cloned()
                    .or_else(|| self.resolve_alt_screen_terminal_id(&params.target)),
                params.source,
                params.lines,
                params.format,
            ),
            api::schema::Method::PaneRead(params) => (
                released_terminal_id
                    .cloned()
                    .or_else(|| self.resolve_alt_screen_terminal_id(&params.pane_id)),
                params.source,
                params.lines,
                params.format,
            ),
        ...
        {
            return AltScreenReadConflict::DeferTerminal(target);
        }
        let Some(pending) = self
            .pending_alt_screen_reads
            .iter()
            .find(|pending| pending.terminal_id == target)
        else {
            return AltScreenReadConflict::None;
        };
    ...
    fn release_deferred_alt_screen_terminals(
        ...
        self.release_deferred_alt_screen_terminals_with(
            terminal_ids,
            |server, released_terminal_id, message| {
                if let Some(released_terminal_id) = released_terminal_id {
                    server.handle_api_request_with_shutdown_check_inner_with_release_context(
                        message,
                        false,
                        Some(released_terminal_id),
                    )
                } else {
    ...
    fn release_deferred_alt_screen_terminals_with(
        &mut self,
        terminal_ids: impl IntoIterator<Item = crate::terminal::TerminalId>,
        ...
        let mut scheduled_terminals = HashSet::new();
        for terminal_id in terminal_ids {
            if scheduled_terminals.insert(terminal_id.clone()) {
                pending_terminals.push_back(terminal_id);
            }
        }
        let mut changed = false;
        ...
        loop {
            while let Some(terminal_id) = pending_terminals.pop_front() {
                ...
                while let Some(item) = queue.pop_front() {
                    match item {
                        DeferredAltScreenItem::Request(message) => {
                            if self
                                .pending_alt_screen_reads
                                .iter()
                                .any(|pending| pending.terminal_id == terminal_id)
                            {
                                retained.push_back(DeferredAltScreenItem::Request(message));
                                retained.extend(queue);
                                break;
                            }
                            changed |= dispatch(self, Some(&terminal_id), *message);
                            if self.shutting_down {
                                return changed;
                            }
                            if self
                ...
                self.deferred_alt_screen_reads
                    .restore_terminal_queue(terminal_id, retained);
            }

            let Some(handoff) = self.deferred_alt_screen_reads.take_ready_handoff() else {
                break;
            };
            let affected_terminals = handoff.terminals;
            changed |= dispatch(self, None, handoff.message);
            if self.shutting_down {
                return changed;
            }
            for terminal_id in affected_terminals {
                let still_pending = self
                    ...
                    .iter()
                    .any(|pending| pending.terminal_id == terminal_id);
                if !still_pending && scheduled_terminals.insert(terminal_id.clone()) {
                    pending_terminals.push_back(terminal_id);
                }
            }
        }
    ...
    fn drain_api_requests_with_render_impact(&mut self) -> RenderImpact {
        let mut impact = if self.app.sync_pane_graphics_streams() {
            RenderImpact::Full
        } else {
            RenderImpact::None
        };
    ...
    fn reject_deferred_alt_screen_requests_for_shutdown(&mut self) {
        for msg in self.deferred_alt_screen_reads.drain_messages() {
    ...
    fn handle_api_request_with_shutdown_check(&mut self, msg: api::ApiRequestMessage) -> bool {
        let before = self.app.sync_pane_graphics_streams();
        let changed = self.handle_api_request_with_shutdown_check_inner(msg, false);
        before | changed | self.app.sync_pane_graphics_streams()
    }
    ...
    fn handle_api_request_with_render_impact(
        ...
        if matches!(
            ...
            self.handle_pane_graphics_stream_frame(msg)
        } else if self.handle_api_request_with_shutdown_check(msg) {
            RenderImpact::Full
        } else {
            RenderImpact::None
    ...
    fn handle_api_request_with_shutdown_check_inner(
        ...
        self.handle_api_request_with_shutdown_check_inner_with_release_context(
            msg,
            skip_default_workspace_for_request,
            None,
        )
    }

    fn handle_api_request_with_shutdown_check_inner_with_release_context(
        &mut self,
        msg: api::ApiRequestMessage,
        skip_default_workspace_for_request: bool,
        released_terminal_id: Option<&crate::terminal::TerminalId>,
        ...
        let frozen_alt_screen_read = match self
            .alt_screen_read_conflict_with_release_context(&msg.request, released_terminal_id)
        {
            AltScreenReadConflict::None => None,
            AltScreenReadConflict::Frozen(snapshot) => Some(snapshot),
            AltScreenReadConflict::DeferTerminal(terminal_id) => {
                self.deferred_alt_screen_reads
                    .defer_request(terminal_id, msg);
                return false;
            }
            AltScreenReadConflict::DeferHandoff(terminal_ids) => {
                self.deferred_alt_screen_reads
                    .defer_handoff(terminal_ids, msg);
                return false;
            }
        };
        ...
        // Zynk delivery renders the toast in-frame and must not ask clients to
    ...
    fn stream_host_keyboard_enhancement_flags(&mut self) {
        let enabled = self.app.host_keyboard_report_all_requested();
        let serialized =
            match Self::frame_server_message(&ServerMessage::KittyKeyboardReportAll { enabled }) {
                Ok(framed) => framed,
                Err(err) => {
        ...
        for (&client_id, client) in &mut self.clients {
            if !client.is_full_app_client()
                || client.host_keyboard_report_all_active == Some(enabled)
            {
                continue;
            }
            let Some(writer) = &client.writer else {
    ...
    fn has_pending_presentation_work_with_graphics(
        &self,
        needs_full_render: bool,
        needs_graphics_render: bool,
    ) -> bool {
        needs_graphics_render || self.has_pending_presentation_work(needs_full_render)
    }
    ...
    fn retained_pty_update_allowed_by_app_state(&self) -> bool {
        self.app.state.mode == app::Mode::Terminal
            ...
            && self.app.state.config_diagnostic.is_none()
    ...
    async fn complete_shutdown(&mut self) -> io::Result<()> {
        ...
        // zynk fork (M3a): stop + join the receipt DB worker (ordered, before the
        // sockets close). Dropping the handle signals shutdown and joins the thread;
        // it never touches live Zynk state.
        let _ = self.app.zynk_receipt_worker.take();
        let _ = self.app.zynk_embedding_worker.take();
...
fn print_ready_message(api_socket: &Path, client_socket: &Path) {
    eprintln!("zynk server running; you can use any zynk CLI command in another terminal.");
    ...
    eprintln!("did you mean to open the Zynk TUI? run `zynk`; you do not need `zynk server`.");
}

/// Initialize logging for the server process.
fn init_logging() {
    crate::logging::init_file_logging(crate::logging::LOG_FILE_SERVER);
}
...
mod tests {
    ...
    fn m832a_headless_cell_size_tracks_only_known_enabled_foreground_state() {
        ...
        server.clients.get_mut(&1).unwrap().cell_size =
            crate::kitty_graphics::HostCellSize::default();
        server.sync_foreground_client_state();
        assert!(!server.app.state.host_cell_size.is_known());
        server.clients.get_mut(&1).unwrap().cell_size = crate::kitty_graphics::HostCellSize {
            width_px: 13,
            height_px: 26,
        };
        server.sync_foreground_client_state();
        assert_eq!(server.app.state.host_cell_size.width_px, 13);
    ...
    #[tokio::test]
    async fn m832a_static_layer_uses_existing_full_frame_and_clear_path() {
        let (mut server, client_rx, pane) = retained_test_server(b"aaaa");
        server.app.state.kitty_graphics_enabled = true;
        server.clients.get_mut(&1).unwrap().cell_size = crate::kitty_graphics::HostCellSize {
            width_px: 10,
            height_px: 20,
        ...
        let initial = read_server_frame(
            client_rx
                ...
                .expect("full frame with layer"),
        );
        let graphics = std::str::from_utf8(&initial.graphics).unwrap();
        ...
        let request = serde_json::from_value(serde_json::json!({
            "id": "clear-static", "method": "pane.graphics.clear", "params": {"pane_id": target}}))
        .unwrap();
        ...
        server.render_and_stream();
        let cleared = read_server_frame(
            client_rx
                .recv_timeout(Duration::from_secs(3))
                .expect("full frame with graphics deletion"),
        );
        assert!(std::str::from_utf8(&cleared.graphics)
            .unwrap()
            .contains("a=d,d=I,"));
        assert!(cleared.cells.iter().any(|cell| cell.symbol == "a"));
        assert!(server.clients[&1].graphics_cache.is_empty());
        shutdown_test_runtimes(&mut server);
    }
    ...
    /// A deferred create dispatched by the server IS the workspace/tab creation, so it must run
    /// with `skip_default_workspace_for_request`. Otherwise a create that the API rejects falls
    /// through to the post-request `ensure_default_workspace()` fallback, which seeds a workspace
    /// nobody asked for and hides the failure.
    #[tokio::test]
    async fn headless_deferred_create_failure_does_not_seed_a_default_workspace() {
        let mut server = test_headless_server();
        ...
        assert!(latest_app_client(&server.clients).is_some());
        assert!(server.app.state.workspaces.is_empty());
        // Out of onboarding, so `ensure_default_workspace()` is live and would fire if the
        // dispatch did not skip it.
        server.app.state.mode = app::Mode::Navigate;

        // `tab.create` with no active workspace is rejected by the API handler.
        server.app.state.request_new_tab = true;
        ...
        assert!(
            ...
            "a rejected deferred create must not fall through to ensure_default_workspace"
        );
        assert_eq!(server.app.terminal_runtimes.len(), 0);
        shutdown_test_runtimes(&mut server);
    }
...
"#,
        ...
        assert!(content.contains("delivery = \"zynk\""));
...
"#,
    ...
    #[test]
    fn mfinal_deferred_alt_screen_work_is_bounded_by_intake_and_terminal_transitions() {
        const BACKLOG: usize = 10_000;
        let mut observations = Vec::new();

        for pane_count in [1usize, 15] {
            with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
                ...
                for request_idx in 0..BACKLOG {
                    let (message, _response_rx) = test_api_message(
                        &format!("deferred-{request_idx}"),
                ...
                observations.push((
                    ...
                    server
                        .deferred_alt_screen_reads
                        .terminal_queue_examination_count,
                    server.deferred_alt_screen_reads.is_empty(),
                ));
            });
        }
    ...
    #[test]
    fn mfinal_deferred_alt_screen_queue_restore_merges_without_overwrite() {
        let terminal_id = crate::terminal::TerminalId::alloc();
        let mut deferred = DeferredAltScreenRequests::default();
        let (first, _first_response_rx) = test_api_message(
            "first",
            api::schema::Method::PaneSendInput(api::schema::PaneSendInputParams {
                pane_id: "w1:p1".into(),
                text: "first".into(),
        ...
        deferred.defer_request(terminal_id.clone(), first);
        let (second, _second_response_rx) = test_api_message(
            "second",
            api::schema::Method::PaneSendInput(api::schema::PaneSendInputParams {
                pane_id: "w1:p1".into(),
                text: "second".into(),
        ...
        let mut retained = VecDeque::new();
        retained.push_back(DeferredAltScreenItem::Request(Box::new(second)));

        deferred.restore_terminal_queue(terminal_id.clone(), retained);

        let ids = deferred.terminal_queues[&terminal_id]
            .iter()
            .filter_map(|item| match item {
                DeferredAltScreenItem::Request(message) => Some(message.request.id.as_str()),
                DeferredAltScreenItem::HandoffBarrier(_) => None,
            })
            .collect::<Vec<_>>();
        ...
        with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
            ...
            for method in matching_methods {
                assert!(matches!(
                    ...
                    AltScreenReadConflict::DeferTerminal(ref deferred_terminal_id)
                        if deferred_terminal_id == &terminal_id
                ));
            }
            ...
            assert!(matches!(
                ...
                AltScreenReadConflict::DeferHandoff(ref terminal_ids)
                    if terminal_ids == std::slice::from_ref(&terminal_id)
            ));
        });
    }
    ...
    fn mfinal_alt_screen_read_releases_ready_terminal_without_cross_terminal_blocking() {
        with_terminal_session_test_server(|server, first_terminal_id, _, first_pane_id| {
            ...
            server.release_deferred_alt_screen_terminals([second_terminal_id]);
            assert_eq!(
                second_input_rx
                    .try_recv()
            ...
            server.pending_alt_screen_reads.clear();
            server.release_deferred_alt_screen_terminals([first_terminal_id]);
            assert_eq!(
                first_input_rx
                    .try_recv()
    ...
    #[test]
    fn mfinal_alt_screen_read_real_dispatch_runs_pre_handoff_input_before_shutdown() {
        with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
            let (runtime, mut input_rx) =
                crate::terminal::TerminalRuntime::test_with_channel_capacity(80, 24, 2);
            server
            ...
            server.pending_alt_screen_reads.clear();
            server.test_live_handoff_results.push_back(Ok(()));
            assert!(server.release_deferred_alt_screen_terminals([terminal_id]));

            assert_eq!(
                input_rx.try_recv().expect("pre-handoff input").as_ref(),
                b"before"
            );
            assert!(matches!(
                input_rx.try_recv(),
            ...
            let before_response = before_response_rx
                .recv_timeout(Duration::from_millis(100))
                .expect("pre-handoff response");
            assert!(serde_json::from_str::<api::schema::SuccessResponse>(&before_response).is_ok());
            ...
            let after_response = after_response_rx
                .recv_timeout(Duration::from_millis(100))
                .expect("post-handoff response");
            let error: api::schema::ErrorResponse =
                serde_json::from_str(&after_response).expect("server stopping response");
    ...
    #[test]
    fn mfinal_alt_screen_read_real_dispatch_preserves_pre_handoff_input_without_suffix() {
        with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
            let (runtime, mut input_rx) =
                crate::terminal::TerminalRuntime::test_with_channel_capacity(80, 24, 1);
            server
    ...
    #[test]
    fn mfinal_alt_screen_read_real_dispatch_keeps_barrier_behind_new_traversal() {
        with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
            let (runtime, mut input_rx) =
                crate::terminal::TerminalRuntime::test_with_channel_capacity(80, 24, 16);
            runtime.test_process_pty_bytes(b"\x1b[?1049h\x1b[?1000h\x1b[?1006halternate-screen");
            server
                .app
                .terminal_runtimes
                .insert(terminal_id.clone(), runtime);
            ...
            let (read, read_response_rx) = test_api_message(
                "read-before-handoff",
            ...
            let (later, later_response_rx) = test_api_message(
                "later-before-handoff",
                api::schema::Method::PaneSendInput(api::schema::PaneSendInputParams {
                    pane_id,
                    text: "later".into(),
                    keys: Vec::new(),
            ...
            server
                .deferred_alt_screen_reads
                .defer_request(terminal_id.clone(), read);
            server
                .deferred_alt_screen_reads
                .defer_request(terminal_id.clone(), later);
            let (handoff, handoff_response_rx) = test_api_message(
                "handoff",
                api::schema::Method::ServerLiveHandoff(Default::default()),
            );
            ...
            server.pending_alt_screen_reads.clear();
            server.test_live_handoff_results.push_back(Ok(()));
            let _ = server.release_deferred_alt_screen_terminals([terminal_id.clone()]);

            assert_eq!(server.pending_alt_screen_reads.len(), 1);
            assert!(matches!(
                read_response_rx.try_recv(),
                Err(std::sync::mpsc::TryRecvError::Empty)
            ));
            assert!(matches!(
                later_response_rx.try_recv(),
                Err(std::sync::mpsc::TryRecvError::Empty)
            ));
            assert!(matches!(
            ...
            while let Ok(bytes) = input_rx.try_recv() {
                assert_ne!(bytes.as_ref(), b"later");
            }
            let queue = server
                .deferred_alt_screen_reads
                .terminal_queues
                .get(&terminal_id)
                .expect("queue retained behind new traversal");
            assert!(matches!(
                queue.front(),
                Some(DeferredAltScreenItem::Request(message))
                    if message.request.id == "later-before-handoff"
            ));
            assert!(matches!(
                queue.back(),
                Some(DeferredAltScreenItem::HandoffBarrier(_))
            ));
            assert_eq!(server.deferred_alt_screen_reads.handoffs.len(), 1);
        });
    }
    ...
    fn mfinal_alt_screen_read_keeps_handoff_order_without_blocking_other_terminals() {
        with_terminal_session_test_server(|server, first_terminal_id, _, first_pane_id| {
            ...
            server.release_deferred_alt_screen_terminals_with(
                [first_terminal_id],
                |server, _, msg| {
            ...
            assert_eq!(dispatch_order, ["handoff"]);
            let handoff_response = handoff_response_rx
                .recv_timeout(Duration::from_millis(100))
                .expect("handoff response");
            assert!(
    ...
    #[test]
    fn mfinal_alt_screen_read_terminal_removal_releases_deferred_request_as_missing() {
        with_terminal_session_test_server(|server, _, _, _| {
            let target_pane =
                server.app.state.workspaces[0].test_split(ratatui::layout::Direction::Horizontal);
            server.app.state.ensure_test_terminals();
            ...
            assert_eq!(completed, [target_terminal]);
            assert!(!server.release_deferred_alt_screen_terminals(completed));
    ...
    #[test]
    fn mfinal_alt_screen_read_failed_handoffs_resume_fifo_in_barrier_order() {
        with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
            let (runtime, mut input_rx) =
                crate::terminal::TerminalRuntime::test_with_channel_capacity(80, 24, 2);
            server
            ...
            for (id, method) in methods {
                let (message, response_rx) = test_api_message(id, method);
                server.handle_api_request_with_shutdown_check(message);
                responses.push((id, response_rx));
            }

            server.pending_alt_screen_reads.clear();
            server.test_live_handoff_results.extend([
                Err("first handoff rejected".into()),
                Err("second handoff rejected".into()),
            ]);
            assert!(server.release_deferred_alt_screen_terminals([terminal_id]));

            assert_eq!(
                input_rx.try_recv().expect("between input").as_ref(),
                b"between"
            );
            assert_eq!(input_rx.try_recv().expect("after input").as_ref(), b"after");
            for (id, response_rx) in responses {
                let response = response_rx
                    .recv_timeout(Duration::from_millis(100))
                    .expect("deferred response");
                if id.starts_with("handoff") {
                    let error: api::schema::ErrorResponse =
                        serde_json::from_str(&response).expect("handoff failure response");
                    assert_eq!(error.error.code, "handoff_failed");
                } else {
                    assert!(
    ...
    fn mfinal_alt_screen_read_dispatches_deferred_prompt_only_after_restoration() {
        with_terminal_session_test_server(|server, terminal_id, _, _| {
            ...
            server.pending_alt_screen_reads.clear();
            server.release_deferred_alt_screen_terminals([terminal_id]);
            let response = response_rx
                .recv_timeout(Duration::from_millis(100))
    ...
    fn mfinal_alt_screen_read_timeout_releases_deferred_input() {
        with_terminal_session_test_server(|server, terminal_id, _, pane_id| {
            ...
            assert!(!server
                ...
                .expect("runtime")
                .screen_detection_paused());
            server.release_deferred_alt_screen_terminals(completed);
    ...
    async fn mfinal_alt_screen_read_releases_deferred_requests_on_stop_and_handoff() {
        for handoff in [false, true] {
            ...
            server
                .deferred_alt_screen_reads
                .defer_request(terminal_id.clone(), message);
    ...
    #[test]
    fn controller_disconnect_does_not_promote_an_observer() {
        with_terminal_session_test_server(|server, _terminal_id, terminal_id_string, _| {
            connect_pending_terminal_client(server, 7);
            assert!(
                server.handle_server_event(ServerEvent::ClientControlTerminal {
    ...
    async fn headless_scheduled_tasks_start_pending_agent_resume_without_foreground_client() {
        ...
        assert!(server.clients.is_empty());
        assert!(server.app.state.host_terminal_theme.is_empty());
        server.render_and_stream();
        assert_ne!(server.app.state.view.terminal_area, Rect::default());
        let pane_inner = server
            .app
            .state
            .view
            .pane_infos
            ...
            .expect("clientless render should compute restored pane geometry")
            .inner_rect;
        ...
        assert_eq!(
            server
                ...
                .terminal_runtimes
                .get(&terminal_id)
                .expect("clientless resume should launch after the theme wait")
                .current_size(),
            (pane_inner.height, pane_inner.width)
        );
        assert!(server
            .app
            .state
    ...
    #[test]
    fn outer_focus_gained_repaints_an_unchanged_semantic_frame() {
        let mut server = test_headless_server();
        let (client_tx, _client_control_rx, client_rx) = test_client_writer();
        ...
        let repaint = read_server_frame(
            client_rx
                .recv_timeout(Duration::from_millis(100))
                .expect("semantic focus repaint"),
        );
        assert_frame_data_eq(&repaint, &initial);
    }

    #[test]
    fn terminal_ansi_repaint_survives_a_full_render_queue_without_clearing() {
        let mut server = test_headless_server();
        let (client_tx, _client_control_rx, client_rx) = test_client_writer();
        server.clients.insert(
            1,
        ...
        match read_server_message(client_rx.recv_timeout(Duration::from_millis(100)).unwrap()) {
            ...
            other => panic!("expected terminal repaint frame, got {other:?}"),
        }
    }

    #[test]
    fn focus_and_shared_resize_repaints_preserve_uploaded_graphics_cache() {
        fn marked_client(writer: ClientWriter) -> ClientConnection {
        ...
        let mut focus_server = test_headless_server();
        let (writer, _control_rx, _render_rx) = test_client_writer();
        focus_server.clients.insert(1, marked_client(writer));
        focus_server.foreground_client_id = Some(1);
        assert!(focus_server.handle_server_event(ServerEvent::ClientInput {
            client_id: 1,
            data: b"\x1b[I".to_vec(),
        }));
        let focus_client = focus_server.clients.get(&1).unwrap();
        assert!(!focus_client.graphics_surface_reset_pending);
        assert!(!focus_client.graphics_cache.is_empty());

        let mut resize_server = test_headless_server();
        let (writer, _control_rx, _render_rx) = test_client_writer();
        resize_server.clients.insert(1, marked_client(writer));
        resize_server.foreground_client_id = Some(1);
        resize_server.resize_shared_runtime_to_effective_size();
        let resize_client = resize_server.clients.get(&1).unwrap();
        assert!(!resize_client.graphics_surface_reset_pending);
        assert!(!resize_client.graphics_cache.is_empty());
    }
    ...
    async fn retained_pty_update_declines_while_copy_feedback_is_visible() {
        ...
        server.render_and_stream();
        // Drain the initial full frame so the later timeout only observes retained-path output.
    ...
    async fn retained_pty_update_declines_while_config_diagnostic_is_visible() {
        ...
        assert!(
            client_rx.recv_timeout(Duration::from_millis(50)).is_err(),
            "retained path should not stream a frame that can overwrite config diagnostic cells"
        );
    }
    ...
    #[test]
    fn configured_window_title_reaches_foreground_once_per_change() {
    ...
    #[test]
    fn attaching_client_receives_unchanged_window_title() {
        ...
        // ClientConnected assigns this directly, so delivery must be keyed by
        // recipient rather than only by title bytes.
        server.foreground_client_id = Some(2);
        server.sync_window_title();
    ...
    #[test]
    fn api_window_title_wins_until_cleared_then_restores_configured_title() {
        let (mut server, control_rx) = window_title_test_server();
        server.app.configure_window_title("{workspace}");
    ...
    #[tokio::test]
    async fn focused_terminal_title_syncs_without_sidebar_render_demand() {
        let (mut server, control_rx) = window_title_test_server();
        ...
        runtime.test_process_pty_bytes("\x1b]0;\u{280b} building\x07".as_bytes());
        server.app.terminal_runtimes.insert(terminal_id, runtime);
    ...
    fn oversized_paste_rejection_notifies_only_the_sending_client() {
        ...
        match read_server_message(
            ...
            ServerMessage::Notify {
                ...
                assert_eq!(
                    body.as_deref(),
                    Some("Input message is 5000012 bytes; Zynk's limit is 1048576 bytes")
                );
            }
    ...
    #[test]
    fn zynk_toast_delivery_keeps_toast_in_frame_without_client_notify() {
        let mut server = test_headless_server();
        let (client_tx, client_control_rx, _client_rx) = test_client_writer();
        ...
        assert!(
            client_control_rx
                .recv_timeout(Duration::from_millis(50))
                .is_err(),
            "zynk delivery should render in-frame instead of forwarding a client-local notification"
        );
    }
    ...
    fn system_toast_delivery_forwards_system_notify_kind() {
        ...
        match read_server_message(
            ...
            ServerMessage::Notify {
                ...
                assert_eq!(
                    body.as_deref(),
                    Some("detach, run `zynk update`, then follow its restart guidance")
                );
            }
            ...
            request: api::schema::Request {
                ...
                method: api::schema::Method::NotificationShow(
                    api::schema::NotificationShowParams {
                        ...
                        position: Some(crate::config::ToastZynkPosition::TopLeft),
    ...
    #[test]
    fn notification_show_api_zynk_toast_expires_headless() {
        let mut server = test_headless_server();
        server.app.state.toast_config.delivery = crate::config::ToastDelivery::Zynk;
    ...
    #[test]
    fn notification_show_api_forwards_sound_for_zynk_delivery() {
        let mut server = test_headless_server();
        let (client_tx, client_control_rx, _client_rx) = test_client_writer();
