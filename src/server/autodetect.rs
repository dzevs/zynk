...
/// directory where the user ran `zynk`.
pub(crate) const STARTUP_CWD_ENV_VAR: &str = "ZYNK_STARTUP_CWD";

// ---------------------------------------------------------------------------
// Server detection
// ---------------------------------------------------------------------------

/// Checks whether a zynk server is currently listening on the client socket.
///
...
fn validate_running_server_compatibility() -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a zynk server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
    ...
    Err(io::Error::other(format!(
        "Zynk was updated, but this session is still running the old server.\n\nserver: v{} protocol {}\nclient: v{} protocol {}\n\n{}",
...
/// Spawns the zynk server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `ZYNK_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    let exe = std::env::current_exe().map_err(|err| {
        io::Error::new(
            err.kind(),
            format!("failed to determine zynk executable path: {err}"),
        )
    })?;
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(err.kind(), format!("failed to spawn zynk server: {err}"))
    })?;
...
///
/// This is the entry point called from `main.rs` when the user runs `zynk`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("ZYNK_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/zynk-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("ZYNK_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var("ZYNK_CLIENT_SOCKET_PATH");
        std::env::remove_var(crate::session::SESSION_ENV_VAR);
        crate::session::clear_explicit_session_for_test();
    }
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        // Clear the explicit session BEFORE setting `ZYNK_SESSION`: post-rebrand the clear
        // removes the same `ZYNK_SESSION` var, which would otherwise leave socket resolution
        // on the default (no-session) path — never the listener bound below.
        crate::session::clear_explicit_session_for_test();
        std::env::set_var(crate::session::ZYNK_SESSION_ENV_VAR, "work");
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        let path = crate::session::api_socket_path_for(Some("work"));
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        let listener = UnixListener::bind(&path).unwrap();
        let handle = std::thread::spawn(move || -> std::io::Result<()> {
            // Accept with a deadline so a path-resolution mismatch fails instead of hanging.
                    ...
                    Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => {
                        if std::time::Instant::now() >= deadline {
                            return Err(std::io::Error::new(
                                ...
                                "listener did not receive protocol mismatch probe",
                            ));
                        }
            ...
            if !request.contains("ping") {
                return Err(std::io::Error::new(
                    ...
                    format!("protocol mismatch probe did not contain ping: {request:?}"),
                ));
            }
            let body = format!(
        ...
        handle
            .join()
            .expect("protocol mismatch listener thread panicked")
            .expect("protocol mismatch listener failed");
        assert!(
            message.contains("Zynk was updated"),
            "expected protocol mismatch/update error: {message}"
        );
        assert!(
            message.contains("server: v0.5.5 protocol"),
            "expected server protocol details: {message}"
        );
        assert!(
            message.contains("client: v") && message.contains("protocol"),
            "expected client protocol details: {message}"
        );
        assert!(
        ...
        assert!(
            message.contains("Run `zynk session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `zynk session attach work` again"),
            "unexpected error: {message}"
        );
