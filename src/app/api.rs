...
impl App {
    ...
    pub(crate) fn refresh_new_zynk_toast_context_for_update(
        &mut self,
    ...
    fn respawn_shell_for_launch_pane(&mut self, pane_id: crate::layout::PaneId) -> bool {
        ...
        // Preserve the pane identity (ZYNK_WORKSPACE_ID/ZYNK_TAB_ID/ZYNK_PANE_ID)
        // across a launch-pane respawn so hooks keep recognizing the pane.
        let launch_env = self
        ...
        let response = match request.method {
            ...
            Method::AgentWait(_) => {
                return responses::encode_error(
                    request.id,
                    "invalid_request",
                    "agent.wait is handled by the socket server",
                )
            }
            ...
            Method::ZynkMessageReceived(params) => {
                return self.handle_zynk_message_received(request.id, params);
            }
            _ => {
                return responses::encode_error(
                    request.id,
...
mod tests {
    ...
    async fn zynk_toast_context_uses_live_root_runtime_cwd_label() {
        ...
        let stale_cwd = temp_root.join("__zynk_original__");
        let live_cwd = temp_root.join("__zynk_projects__");
    ...
    #[test]
    fn m840_idle_agent_exit_emits_release_without_an_effective_state_change() {
    ...
    #[test]
    fn m840_stale_exit_does_not_publish_release_for_provisional_hook_owner() {
        let event_hub = crate::api::EventHub::default();
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
    ...
    fn terminal_delivery_does_not_refresh_existing_targeted_toast() {
        ...
        let mut workspace = crate::workspace::Workspace::test_new("stale");
        workspace.custom_name = None;
        workspace.identity_cwd = "/__zynk_original__".into();
        let root = workspace.tabs[0].root_pane;
        let terminal_id = workspace.terminal_id(root).cloned().unwrap();
        let workspace_id = workspace.id.clone();
        app.state.workspaces = vec![workspace];
        app.state.ensure_test_terminals();
        app.state.terminals.get_mut(&terminal_id).unwrap().cwd = "/__zynk_projects__".into();
        app.state.active = None;
        app.state.selected = 0;
        app.state.mode = Mode::Terminal;
        ...
        app.state.toast = Some(crate::app::state::ToastNotification {
            ...
            context: "__zynk_original__ · 1".into(),
            position: None,
