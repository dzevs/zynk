...
impl App {
    ...
    /// Build the [`PaneLaunchEnv`](crate::pane::PaneLaunchEnv) for a pane that
    /// already lives in `self.state`: resolves the pane's public workspace/tab/
    /// pane ids and folds them (plus caller `extra_env`) into an identity-bearing
    /// launch env. Returns `None` only if the pane cannot be located.
