...
pub(super) fn update_settings_state(state: &mut AppState, key: KeyEvent) -> Option<SettingsAction> {
    match state.settings.section {
        ...
        SettingsSection::PaneLabels => match key.code {
            ...
            KeyCode::Tab | KeyCode::Right | KeyCode::Char('l') => {
                state.settings.section = SettingsSection::Header;
                state.settings.list.selected = 0;
            }
            _ => {
                if let Some(super::modal::ModalAction::Close) =
        ...
        SettingsSection::Header => match key.code {
            // Feature #116: Up/Down (k/j) move between the two rows (verbose, max_width).
            KeyCode::Up | KeyCode::Char('k') => state.settings.list.move_prev(),
            KeyCode::Down | KeyCode::Char('j') => {
                state.settings.list.move_next(HeaderSettingRow::COUNT)
            }
            KeyCode::Enter | KeyCode::Char(' ') => {
                if HeaderSettingRow::from_index(state.settings.list.selected)
                    == HeaderSettingRow::Verbose
                {
                    // Toggle verbose on the verbose row.
                    return Some(SettingsAction::SaveHeaderVerbose(!state.header_verbose()));
                }
            }
            KeyCode::Left | KeyCode::Char('h')
                if HeaderSettingRow::from_index(state.settings.list.selected)
                    == HeaderSettingRow::MaxWidth =>
            {
                // Q2: step max_width down by 8, clamped to [24, 200].
                let next = header_max_width_step(state.header_max_width(), -8);
                return Some(SettingsAction::SaveHeaderMaxWidth(next));
            }
            KeyCode::Right | KeyCode::Char('l')
                if HeaderSettingRow::from_index(state.settings.list.selected)
                    == HeaderSettingRow::MaxWidth =>
            {
                // Q2: step max_width up by 8, clamped to [24, 200].
                let next = header_max_width_step(state.header_max_width(), 8);
                return Some(SettingsAction::SaveHeaderMaxWidth(next));
            }
            KeyCode::BackTab | KeyCode::Left | KeyCode::Char('h') => {
...
/// Feature #116 (Q2/Q4): step `[header] max_width` by `delta` (±8), clamped to
/// `[MIN_HEADER_MAX_WIDTH, MAX_HEADER_MAX_WIDTH]` ([24, 200]). The arithmetic is
/// saturating so a step past the floor lands exactly on it (and likewise at the cap).
fn header_max_width_step(current: usize, delta: i32) -> usize {
...
impl AppState {
    ...
    fn settings_tab_at(&self, col: u16, row: u16) -> Option<SettingsSection> {
        ...
        // Never map a column at/beyond the visible tab row end to a tab — even when
        // a tab cell starts before the edge but straddles it on an ultra-narrow
        // terminal (the `centered_popup_rect` cap can leave a partly-visible cell).
        // The explicit guard runs BEFORE any per-cell check (#116 hardening).
        let tab_row_end = inner.x.saturating_add(inner.width);
        if col >= tab_row_end {
            return None;
        }
        let mut x = inner.x;
        for section in SettingsSection::ALL {
            ...
            // Shared source of truth with the `Tabs` render so hit math can't drift.
            let width = section.tab_cell_width(self.settings_section_has_badge(*section));
    ...
    fn settings_list_index_at(&self, col: u16, row: u16) -> Option<usize> {
        ...
        match self.settings.section {
            ...
            SettingsSection::Header => {
                let list_y = area.y + 3;
                if row >= list_y && row < list_y + HeaderSettingRow::COUNT as u16 {
                    Some((row - list_y) as usize)
                } else {
                    None
                }
            ...
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(idx) = self.settings_list_index_at(mouse.column, mouse.row) {
                    ...
                    return match self.settings.section {
                        ...
                        SettingsSection::Header => match HeaderSettingRow::from_index(idx) {
                            // Clicking the verbose row toggles it; the max_width row only
                            // selects (the value steps via Left/Right keys).
                            HeaderSettingRow::Verbose => {
                                Some(SettingsAction::SaveHeaderVerbose(!self.header_verbose()))
                            }
...
mod tests {
    ...
    #[test]
    fn settings_header_verbose_enter_toggles_current() {
        let mut state = state_with_workspaces(&["test"]);
        state.header_verbose = false;
        open_settings_at(&mut state, SettingsSection::Header);
        // Row 0 is the verbose row by default.
        ...
        assert_eq!(action, Some(SettingsAction::SaveHeaderVerbose(true)));

        // When verbose is already on, Enter emits the toggle to off.
        state.header_verbose = true;
        let action = update_settings_state(
            &mut state,
            KeyEvent::new(KeyCode::Char(' '), KeyModifiers::empty()),
        );
        assert_eq!(action, Some(SettingsAction::SaveHeaderVerbose(false)));
    }

    #[test]
    fn settings_header_max_width_steps_by_eight_and_clamps() {
        let mut state = state_with_workspaces(&["test"]);
        state.header_max_width = 100;
        open_settings_at(&mut state, SettingsSection::Header);
        // Move down to the max_width row.
        update_settings_state(
            &mut state,
            KeyEvent::new(KeyCode::Down, KeyModifiers::empty()),
        );
        ...
        assert_eq!(action, Some(SettingsAction::SaveHeaderMaxWidth(108)));
        ...
        assert_eq!(action, Some(SettingsAction::SaveHeaderMaxWidth(92)));

        // Clamp at the floor: stepping down from 28 lands on 24, not 20.
        state.header_max_width = 28;
        let action = update_settings_state(
            &mut state,
        ...
        assert_eq!(
            action,
            Some(SettingsAction::SaveHeaderMaxWidth(
                crate::config::MIN_HEADER_MAX_WIDTH
            ))
        );

        // Clamp at the cap: stepping up from 196 lands on 200, not 204.
        state.header_max_width = 196;
        let action = update_settings_state(
            &mut state,
    ...
    fn settings_tab_cycle_places_experiments_last() {
        ...
        assert_eq!(state.settings.section, SettingsSection::Header);
    ...
    #[test]
    fn settings_tab_row_fits_inner_width() {
        // #116 fix: the 7-tab row must fit the settings modal inner width (popup
        // width minus the two border columns). Pre-fix this summed to 76 > 74 and
        // the rightmost tab clipped.
        let inner = crate::ui::SETTINGS_POPUP_WIDTH - 2;
        let dividers = SettingsSection::ALL.len() as u16 - 1;
        let no_badge: u16 = SettingsSection::ALL
            .iter()
            .map(|s| s.tab_cell_width(false))
            .sum::<u16>()
            + dividers;
        assert!(no_badge <= inner, "tab row {no_badge} cols > inner {inner}");
        // worst case: the Integrations update badge is showing (+2 cols).
        let with_badge: u16 = SettingsSection::ALL
            .iter()
            .map(|s| s.tab_cell_width(*s == SettingsSection::Integrations))
            .sum::<u16>()
            + dividers;
        assert!(
            with_badge <= inner,
            "tab row with badge {with_badge} cols > inner {inner}"
        );
    }

    #[test]
    fn settings_tab_at_rightmost_reachable_and_within_inner() {
        let mut state = state_with_workspaces(&["test"]);
        ...
        // The rightmost tab (last in ALL) is reachable by mouse at its first column,
        // using the SAME shared `tab_cell_width` the renderer uses.
        let last = *SettingsSection::ALL.last().expect("non-empty");
        let mut start = inner.x;
        for section in SettingsSection::ALL {
            if *section == last {
                break;
            }
            start += section.tab_cell_width(state.settings_section_has_badge(*section)) + 1;
        }
        assert_eq!(
            state.settings_tab_at(start, tab_y),
            Some(last),
            "rightmost tab must be clickable"
        );

        // A column at/after the inner right edge never maps to a tab.
        assert_eq!(
            state.settings_tab_at(inner.x + inner.width, tab_y),
            None,
            "columns beyond inner width must not map to a tab"
        );
    }

    #[test]
    fn settings_tab_at_none_at_visible_edge_even_when_a_cell_straddles_it() {
        // Ultra-narrow terminal: `centered_popup_rect` caps the popup at
        // `area.width - 4`, so the tab row overflows the inner width and a tab cell
        ...
        assert!(
            inner.width > 0,
            "popup must render for this test to be meaningful"
        );
        let tab_y = inner.y + 1;
        let edge = inner.x + inner.width; // tab_row_end

        // Prove a tab cell actually straddles the edge (starts < edge, ends > edge),
        // so the early guard — not merely the per-cell `x >= tab_row_end` break — is
        // what returns `None`.
        let mut x = inner.x;
        let straddles = SettingsSection::ALL.iter().any(|s| {
            let w = s.tab_cell_width(state.settings_section_has_badge(*s));
            let hit = x < edge && x + w > edge;
            x += w + 1;
            hit
        });
        assert!(
            straddles,
            "expected a tab cell to straddle the visible edge"
        );

        assert_eq!(state.settings_tab_at(edge, tab_y), None);
        assert_eq!(state.settings_tab_at(edge + 5, tab_y), None);
        // A column inside the visible row still resolves (the first tab).
        assert_eq!(
            state.settings_tab_at(inner.x, tab_y),
    ...
    fn integration_recommendation(
        ...
        crate::integration::IntegrationRecommendation {
            ...
            command: "claude",
            available,
            path: std::path::PathBuf::from("/tmp/zynk-test-integration"),
            state,
        }
    }
}
