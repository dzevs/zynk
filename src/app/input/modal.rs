...
mod tests {
    ...
    #[test]
    fn navigator_ignores_modified_j_k_and_arrows() {
        let mut state = state_with_workspaces(&["alpha", "beta"]);
        let terminal_runtimes = crate::terminal::TerminalRuntimeRegistry::new();
        state.mode = Mode::Navigator;
        for (code, selected, target) in [
            (KeyCode::Char('k'), 1, 0),
            (KeyCode::Up, 1, 0),
            (KeyCode::Char('j'), 0, 1),
            (KeyCode::Down, 0, 1),
            ...
            for modifiers in [
                ...
                assert_eq!(state.navigator.selected, selected, "{code:?} {modifiers:?}");
            }
            handle_navigator_key(
                &mut state,
                &terminal_runtimes,
            ...
            assert_eq!(state.navigator.selected, target, "{code:?}");
        }
    }
