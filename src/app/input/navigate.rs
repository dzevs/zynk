...
impl App {
    ...
    pub(in crate::app) fn custom_command_env(
        &self,
            ...
            if let Some(workspace) = self.state.workspaces.get(ws_idx) {
                ...
                if let Some(pane_id) = workspace.focused_pane_id() {
                    ...
                    if let Some(pane_cwd) = workspace.active_tab().and_then(|tab| {
                        ...
                        env.push((
                            "ZYNK_ACTIVE_PANE_CWD".to_string(),
    ...
    fn open_focused_scrollback_in_editor(&mut self) -> std::io::Result<()> {
        ...
        let scrollback = self
            ...
            .recent_unwrapped_text(usize::MAX);
...
fn unique_scrollback_path(attempt: u32) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "zynk-scrollback-{}-{nanos}-{attempt}.txt",
        std::process::id()
    ))
}
...
mod tests {
    ...
    #[test]
    fn m844_next_agent_starts_at_first_visible_entry_when_focus_is_filtered_out() {
    ...
    fn rename_workspace_prefills_live_terminal_cwd_label() {
        ...
        state.workspaces[0].identity_cwd = "/__zynk_original__".into();
        state.terminals.insert(
            terminal_id.clone(),
            TerminalState::new(terminal_id, "/__zynk_projects__".into()),
        );
        state.keybinds.rename_workspace = crate::config::ActionKeybinds::prefix("g");
        ...
        assert_eq!(state.mode, Mode::RenameWorkspace);
        assert_eq!(state.name_input, "__zynk_projects__");
        assert_eq!(state.workspaces[0].display_name(), "__zynk_original__");
    }
...
"#,
    ...
    #[test]
    fn m93_direct_and_prefix_resize_bindings_map_to_typed_actions() {
        let config: Config = toml::from_str(
            r#"
[keys]
...
"#,
        ...
        assert_eq!(
            ...
            Some(NavigateAction::ResizePaneLeft)
        );
        assert_eq!(
            ...
            Some(NavigateAction::ResizePaneRight)
        );
    }

    #[test]
    fn m93_move_tab_actions_reorder_wrap_and_noop_for_one_tab() {
        let mut state = state_with_workspaces(&["test"]);
        ...
        assert_eq!(
            (0..3)
                .map(|idx| state.workspaces[0].tab_display_name(idx).unwrap())
                .collect::<Vec<_>>(),
            vec!["a", "c", "b"]
        );
        ...
        let mut single = state_with_workspaces(&["single"]);
        execute_navigate_action(&mut single, NavigateAction::MoveTabNext);
        assert_eq!(single.workspaces[0].tabs.len(), 1);
        assert_eq!(single.workspaces[0].active_tab, 0);
    }

    #[test]
    fn m93_direct_move_tab_binding_maps_to_typed_action() {
        let mut state = state_with_workspaces(&["test"]);
        ...
        assert_eq!(
            ...
            Some(NavigateAction::MoveTabNext)
        );
    }

    #[test]
    fn m93_single_tab_move_is_noop_and_exits_navigate_mode_via_runtime_path() {
...
"#,
    ...
    async fn custom_command_runs_from_prefix_key_in_navigate_mode() {
        ...
        let command = format!(
            "printf '%s\\n%s\\n%s\\n%s\\n' \"$$\" \"$ZYNK_ACTIVE_WORKSPACE_ID\" \"$ZYNK_ACTIVE_TAB_ID\" \"$ZYNK_ACTIVE_PANE_ID\" > '{}'; i=0; while [ ! -e '{}' ] && [ \"$i\" -lt 250 ]; do sleep 0.02; i=$((i + 1)); done",
            output_path.display(),
