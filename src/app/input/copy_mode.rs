...
mod tests {
    ...
    /// Fork-only oracle for the re-applied `8d2af4e0` hunk 3: `last_character_col` measures
    /// columns with the emulator's own width table (`crate::ghostty::unicode_codepoint_width`),
    /// never the `unicode-width` crate's `width(ch).unwrap_or(1)`. The two forms disagree on
    /// any codepoint `unicode-width` reports as `None` — a C0 control advances a column under
    /// `unwrap_or(1)` and none under the emulator table.
    #[test]
    fn last_character_col_uses_the_emulator_width_table() {
        assert_eq!(crate::ghostty::unicode_codepoint_width(0x301), 0);
        assert_eq!(crate::ghostty::unicode_codepoint_width('\u{7}' as u32), 0);
        assert_eq!(crate::ghostty::unicode_codepoint_width('界' as u32), 2);

        // A zero-width combining mark shares the column of the character it follows and never
        // becomes the reported last column.
        assert_eq!(last_character_col("ae\u{301}"), Some(1));
        assert_eq!(last_character_col("\u{301}"), None);

        // A wide character advances two columns: 'a' at 0, '界' at 1; reversed, '界' spans 0-1
        // and 'a' lands on 2.
        assert_eq!(last_character_col("a界"), Some(1));
        assert_eq!(last_character_col("界a"), Some(2));

        // The discriminating case: under the `unicode-width` form the BEL would occupy
        // column 1 and push 'b' to column 2.
        assert_eq!(last_character_col("a\u{7}b"), Some(1));

        assert_eq!(last_character_col(""), None);
    }
