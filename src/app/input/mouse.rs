...
mod tests {
    ...
    #[test]
    fn tab_click_survives_stray_drag_outside_tab_bar() {
        ...
        assert!(app.state.tab_presses.contains_key(&0));
        app.handle_mouse(mouse(
            MouseEventKind::Drag(MouseButton::Left),
            tab.x + 1,
            area.height - 1,
    ...
    #[test]
    fn tab_presses_are_owned_by_their_input_source() {
        let mut app = app_for_mouse_test();
        let mut workspace = Workspace::test_new("test");
        workspace.test_add_tab(None);
        workspace.test_add_tab(None);
        ...
        let first = app.state.view.tab_hit_areas[0];
        let second = app.state.view.tab_hit_areas[1];
    ...
    async fn mouse_dispatcher_forwards_horizontal_wheel_to_mouse_reporting_pane() {
        ...
        assert!(
            ...
            "reproduction must use the default Zynk mouse dispatcher"
        );
    ...
    #[tokio::test]
    async fn mouse_dispatcher_does_not_forward_motion_behind_zynk_modes() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        let (runtime, mut input_rx) =
    ...
    #[tokio::test]
    async fn unset_right_click_passthrough_keeps_modified_right_click_as_zynk_menu() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        let pane_infos = ws.tabs[0].layout.panes(Rect::new(26, 2, 80, 18));
    ...
    fn bottom_mode_bar_consumes_hidden_tab_mouse_actions() {
        for mode in [Mode::Prefix, Mode::Navigate, Mode::Copy, Mode::Resize] {
            let mut app = app_for_mouse_test();
            let mut ws = Workspace::test_new("one");
            ws.test_add_tab(Some("two"));
            app.state.workspaces = vec![ws];
    ...
    #[test]
    fn clicking_tab_context_menu_close_last_tab_in_group_keeps_confirmation_mode() {
        let mut app = app_for_mouse_test();
    ...
    #[tokio::test]
    async fn mobile_new_workspace_creates_immediately_by_default() {
        let mut app = app_for_mouse_test();
        app.state.workspaces = vec![Workspace::test_new("one")];
        app.state.ensure_test_terminals();
        app.state.active = Some(0);
