...
impl SettingsSection {
    ...
    /// Rendered tab-cell width (display columns) in the settings tab row — the
    /// SINGLE source of truth shared by the `Tabs` render (`.padding(" ", " ")` =
    /// 1 col each side; optional "● " badge = 2 cols) and the `settings_tab_at`
    /// mouse hit math, so the two can never diverge (#116 tab-overflow fix).
    /// Excludes the 1-col inter-tab divider (the caller adds it).
    pub(crate) fn tab_cell_width(self, has_badge: bool) -> u16 {
        use unicode_width::UnicodeWidthStr;
        self.label().width() as u16 + 2 + if has_badge { 2 } else { 0 }
    }
}

/// Feature #116: the two editable rows in the `[header]` Settings section.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HeaderSettingRow {
    Verbose,
    MaxWidth,
}

impl HeaderSettingRow {
    /// Number of rows in the Header settings section (verbose + max_width).
...
pub struct ToastNotification {
    ...
    pub title: String,
    pub context: String,
    pub position: Option<crate::config::ToastZynkPosition>,
...
pub struct AppState {
    ...
    /// Last reported focus state for the outer terminal hosting zynk.
    ...
    /// Feature #116: mirror of `[header] verbose` for the Settings UI. Applied
    /// per-send client-side by `resolve_header_options`; this copy is display-only.
    pub header_verbose: bool,
    /// Feature #116: mirror of `[header] max_width` for the Settings UI (clamped
...
impl AppState {
    ...
    /// Feature #116: the loaded `[header] verbose` flag for the Settings UI.
    pub fn header_verbose(&self) -> bool {
        self.header_verbose
    }

    /// Feature #116: the loaded `[header] max_width` for the Settings UI, clamped
    /// to `[MIN_HEADER_MAX_WIDTH, MAX_HEADER_MAX_WIDTH]` ([24, 200]) for display.
    pub fn header_max_width(&self) -> usize {
        self.header_max_width.clamp(
...
impl AppState {
    ...
    pub fn test_new() -> Self {
        Self {
            ...
            worktree_directory: std::path::PathBuf::from("/tmp/zynk-worktrees"),
            ...
            update_install_command: "zynk update".into(),
...
mod tests {
    ...
    #[test]
    fn rose_pine_uses_overlay_for_dim_surfaces() {
