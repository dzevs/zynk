...
impl App {
    ...
    pub(super) fn save_toast_delivery(&mut self, delivery: crate::config::ToastDelivery) {
        let value = match delivery {
            ...
            crate::config::ToastDelivery::Zynk => "\"zynk\"",
    ...
    /// Feature #116: persist `[header] verbose` and re-apply from disk so the next
    /// send (which reads `[header]` client-side via `resolve_header_options`) picks it
    /// up with no server reload. Only the `verbose` key is touched (upsert), so any
    /// other `[header]`/unrelated section keys are preserved.
    pub(super) fn save_header_verbose(&mut self, enabled: bool) {
        let value = if enabled { "true" } else { "false" };
        if self.update_config_file("header setting", |content| {
            crate::config::upsert_section_value(content, "header", "verbose", value)
        }) {
            self.apply_config_from_disk(false);
        }
    }

    /// Feature #116 (Q2/Q4): persist `[header] max_width`, clamped to
    /// `[MIN_HEADER_MAX_WIDTH, MAX_HEADER_MAX_WIDTH]` ([24, 200]) before writing, then
    /// re-apply from disk (per-send client-side, no server reload). Only the
    /// `max_width` key is touched, preserving the rest of the config.
    pub(super) fn save_header_max_width(&mut self, width: usize) {
        ...
        if self.update_config_file("header setting", |content| {
            crate::config::upsert_section_value(content, "header", "max_width", &value)
        }) {
            self.apply_config_from_disk(false);
        }
    }
