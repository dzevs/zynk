...
mod tests {
    ...
    fn spaces_working_animation_schedules_when_nonactive_workspace_working() {
        // §4 follow-up: with agent_panel_scope = CurrentWorkspace and the ACTIVE workspace idle,
        // a different working workspace must still keep the animation timer alive so its `spaces`
        // working dot pulses.
        let mut app = test_app();
        let now = Instant::now();
