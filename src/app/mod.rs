...
pub struct App {
    ...
    /// discarded and concurrent operations on a checkout rejected. (upstream 46a2b25)
    ...
    // zynk fork (M3a): App-owned receipt DB worker. Installed by the headless
    // server at startup; `None` in CLI / unit-test App constructors.
    pub(crate) zynk_receipt_worker: Option<crate::zynk::receipt_worker::ReceiptWorkerHandle>,
    pub(crate) zynk_embedding_worker: Option<crate::zynk::embedding_worker::EmbeddingWorkerHandle>,
}
...
impl App {
    pub fn new(
        ...
        // Manifest-sensitive tests reload explicitly; ordinary App fixtures should
        // not recompile every bundled regex in each nextest process.
        #[cfg(test)]
    ...
    fn apply_live_config(
        ...
        if !invalid_section("header") {
            // Feature #116: mirror the loaded `[header]` config onto the Settings UI
            // copy. The per-send header read (`resolve_header_options`) is unaffected —
            // these fields are display-only for the Settings panel.
            self.state.header_verbose = config.header.verbose;
            self.state.header_max_width = config.header.max_width;
        }
...
impl App {
    ...
    /// key-handling path as monolithic zynk so they are re-encoded for the
...
mod tests {
    ...
    async fn m833_popup_launch_paths_remove_identity_and_failed_launch_keeps_state() {
        ...
        for shell in [false, true] {
            ...
            loop {
                let text = app.popup_runtime().unwrap().recent_text(50);
    ...
    #[test]
    fn m828d2_row_reload_is_section_atomic_and_changes_geometry() {
        ...
        for terminal in app.state.terminals.values_mut() {
            ...
            terminal.set_manual_label("reload-pane".into());
            assert_eq!(terminal.manual_label.as_deref(), Some("reload-pane"));
        }
        app.state.active = Some(0);
        app.state.mode = Mode::Terminal;
        let area = ratatui::layout::Rect::new(0, 0, 106, 60);
        crate::ui::compute_view(&mut app.state, area);
        let before_cards = app.state.view.workspace_card_areas.clone();
        assert_eq!(before_cards.len(), 2);
        assert_eq!(before_cards[0].rect.height, 1);
        let (_, agent_area) = crate::ui::expanded_sidebar_sections(
            app.state.view.sidebar_rect,
            app.state.sidebar_section_split,
        );
        let children = |state: &AppState| {
            crate::ui::agent_visible_rows(state, agent_area)
                .into_iter()
        ...
        let before_children = children(&app.state);
        assert_eq!(before_children.len(), 3);
        assert_eq!(before_children[1].1, before_children[0].1 + 1);

        let valid = "[ui]\nmouse_capture = false\n[ui.sidebar.spaces]\nrow_gap = 2\nrows = [[\"workspace\"], [\"workspace\"], [\"workspace\"]]\n[ui.sidebar.agents]\nrow_gap = 1\nrows = [[\"agent\"]]\nrows_by_agent = { claude = [[\"agent\"], [\"pane\"], [\"state_text\"]] }\n";
        ...
        assert_eq!(cards[0].rect.height, 3);
        assert_eq!(cards[1].rect.height, 3);
        assert_eq!(cards[1].rect.y, cards[0].rect.y + 5);
        assert_ne!(cards[1].rect, before_cards[1].rect);
        let after_children = children(&app.state);
        assert_eq!(after_children.len(), 3);
        assert_eq!(after_children[1].1, after_children[0].1 + 4);
        assert_ne!(after_children, before_children);
        let agents = app.state.sidebar_agents.clone();
        let spaces = app.state.sidebar_spaces.clone();
        ...
        assert_eq!(app.state.sidebar_agents, agents);
        assert_eq!(app.state.sidebar_spaces, spaces);
        assert!(!app.state.mouse_capture);
        assert!(app
            .state
            .keybinds
        ...
        crate::ui::compute_view(&mut app.state, area);
        assert_eq!(children(&app.state), after_children);
        assert_eq!(
            app.state
                .view
                ...
                .map(|c| c.rect)
                .collect::<Vec<_>>(),
            cards.iter().map(|c| c.rect).collect::<Vec<_>>()
        );
    ...
    async fn explicit_text_commit_bypasses_bindings_leases_and_key_encoding() {
        ...
        assert_eq!(
            rx.try_recv().expect("forwarded input").as_ref(),
            "你🙂".as_bytes()
        );
        assert!(rx.try_recv().is_err());
        assert!(app.input_leases.is_empty());
    ...
    #[tokio::test]
    async fn kitty_associated_ime_text_bypasses_key_encoding() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("test");
        let focused = workspace.focused_pane_id().unwrap();
        let (runtime, mut rx) =
        ...
        assert_eq!(
            rx.try_recv().expect("forwarded IME text").as_ref(),
            "你好".as_bytes()
        );
        assert!(rx.try_recv().is_err());
        assert!(app.input_leases.is_empty());
    ...
    async fn physical_count_one_repeats_and_release_keep_the_pressed_pane() {
        ...
        let (pressed_runtime, mut pressed_rx) =
            TerminalRuntime::test_with_channel_and_scrollback_bytes(80, 24, 0, b"\x1b[>3u", 5);
        let (other_runtime, mut other_rx) =
            TerminalRuntime::test_with_channel_and_scrollback_bytes(80, 24, 0, b"\x1b[>3u", 2);
        workspace.tabs[0]
            .runtimes
        ...
        for expected in [
            b"\x1b[97;5:1u".as_slice(),
            b"\x1b[97;5:2u".as_slice(),
            b"\x1b[97;5:2u".as_slice(),
            b"\x1b[97;5:3u".as_slice(),
        ] {
    ...
    async fn grouped_physical_press_and_runtime_loss_preserve_count_then_close_the_lease() {
        ...
        assert_eq!(
            ...
            bytes::Bytes::from_static(b"\x1b[97;5:1u\x1b[97;5:2u\x1b[97;5:2u")
        );
        assert_eq!(
            rx.try_recv()
                .expect("synthetic release before runtime shutdown"),
            bytes::Bytes::from_static(b"\x1b[97;5:3u")
        );
        assert!(rx.try_recv().is_err());
        assert!(app.input_leases.is_empty());
    ...
    #[tokio::test]
    async fn key_lifecycle_ownership_is_isolated_by_client() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("test");
        ...
        let (first_runtime, mut first_rx) =
            TerminalRuntime::test_with_channel_and_scrollback_bytes(80, 24, 0, b"\x1b[>3u", 3);
        ...
        for rx in [&mut first_rx, &mut second_rx] {
            assert_eq!(
                rx.try_recv().expect("forwarded input"),
    ...
    #[tokio::test]
    async fn disconnected_input_source_releases_owned_keys() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("test");
        let focused = workspace.focused_pane_id().unwrap();
        let (runtime, mut rx) =
    ...
    #[tokio::test]
    async fn committed_text_does_not_erase_owned_semantic_key() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("test");
        let focused = workspace.focused_pane_id().unwrap();
        let (runtime, mut rx) =
            TerminalRuntime::test_with_channel_and_scrollback_bytes(80, 24, 0, b"\x1b[>3u", 4);
        workspace.tabs[0].runtimes.insert(focused, runtime);
        app.state.workspaces = vec![workspace];
        app.state.active = Some(0);
        app.state.selected = 0;
        ...
        app.route_client_input(b"\x1b[106;5u".to_vec());
        assert_eq!(app.input_leases.len(), 1);
        app.route_client_input(b"j".to_vec());
        assert_eq!(app.input_leases.len(), 1, "text must preserve the held key");
        app.route_client_input(b"\x1b[106;5:3u".to_vec());

        assert_eq!(
            rx.try_recv().expect("semantic press"),
            bytes::Bytes::from_static(b"\x1b[106;5:1u")
        );
        assert_eq!(
        ...
        assert_eq!(
            rx.try_recv().expect("semantic release"),
            bytes::Bytes::from_static(b"\x1b[106;5:3u")
        );
        assert!(app.input_leases.is_empty());
    }
    ...
    #[test]
    fn unit_app_construction_does_not_reload_bundled_manifests() {
        let app = test_app();
        assert!(app.state.agent_manifest_summaries.is_empty());
    }
    ...
    #[test]
    fn notification_show_api_creates_zynk_toast_with_position() {
        let mut app = test_app();
        app.state.toast_config.delivery = crate::config::ToastDelivery::Zynk;
    ...
    #[test]
    fn notification_show_api_zynk_toast_expires() {
        let mut app = test_app();
        app.state.toast_config.delivery = crate::config::ToastDelivery::Zynk;
    ...
    #[test]
    fn m93_reload_unknown_theme_keeps_previous_palette_and_applies_other_sections() {
        let _guard = config_env_lock().lock().unwrap();
        let path = temp_config_path("reload-config-unknown-theme");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        std::env::set_var(crate::config::CONFIG_PATH_ENV_VAR, &path);

        let mut app = test_app();
        app.state.theme_name = "dracula".into();
        app.state.palette = state::Palette::dracula();
        app.state.theme_runtime.manual_name = "dracula".into();
        ...
        assert!(report
            .diagnostics
            .iter()
            .any(|diagnostic| diagnostic.contains("theme.name = \"catppucin\"")));
        assert_eq!(app.state.theme_name, "dracula");
        assert_eq!(app.state.palette, palette);
    ...
    // Fork addition: upstream wired `hide_tab_bar_when_single_tab` into `reload_config` without a
    // test. The reload path is the only way the key reaches a running app, so it gets one here.
    #[test]
    fn reload_config_updates_hide_tab_bar_when_single_tab() {
        let _guard = config_env_lock().lock().unwrap();
        let path = temp_config_path("reload-config-hide-tab-bar-single-tab");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        std::env::set_var(crate::config::CONFIG_PATH_ENV_VAR, &path);
    ...
    #[test]
    fn b3_reload_keeps_startup_headless_geometry() {
        let _guard = config_env_lock().lock().unwrap();
        let path = temp_config_path("reload-config-headless-size");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        std::env::set_var(crate::config::CONFIG_PATH_ENV_VAR, &path);

        let mut app = test_app();
        let startup_size = app.state.headless_size;
        ...
        let _ = std::fs::remove_dir_all(path.parent().unwrap());
        assert_eq!(report.status, crate::config::ConfigReloadStatus::Applied);
        assert_eq!(app.state.headless_size, startup_size);
    }
    ...
    // Feature #116: saving `[header]` settings touches ONLY the `[header]` keys, leaves
    // the rest of the config intact, re-applies live (per-send client-side, no reload),
    // and clamps `max_width` to [24, 200] on the write path.
    #[test]
    fn settings_save_header_verbose_and_max_width_preserve_other_sections() {
        let _guard = config_env_lock().lock().unwrap();
        let path = temp_config_path("settings-save-header");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        // A config that already has other sections updated; [header] starts default.
        std::fs::write(
            &path,
            "onboarding = false\n\n[ui.toast]\ndelivery = \"terminal\"\n\n[experimental]\npane_history = true\n",
        )
        ...
        let mut app = test_app();
        assert!(!app.state.header_verbose());
        assert_eq!(
            app.state.header_max_width(),
            crate::config::Config::default().header.max_width
        );

        app.save_header_verbose(true);
        app.save_header_max_width(64);

        // Live state reflects the new values (no server reload needed).
        assert!(app.state.header_verbose());
        assert_eq!(app.state.header_max_width(), 64);

        let content = std::fs::read_to_string(&path).unwrap();
        assert!(
            content.contains("[header]"),
            "header section written: {content}"
        );
        assert!(
            content.contains("verbose = true"),
            "verbose written: {content}"
        );
        assert!(
            content.contains("max_width = 64"),
            "max_width written: {content}"
        );
        // ONLY [header] is touched — the other sections are preserved verbatim.
        assert!(
            content.contains("delivery = \"terminal\""),
            "ui.toast preserved: {content}"
        );
        assert!(
            content.contains("pane_history = true"),
            "experimental preserved: {content}"
        );
        // [header] still parses cleanly with no unknown-section warning.
        let loaded = toml::from_str::<crate::config::Config>(&content).unwrap();
        assert!(loaded.header.verbose);
        assert_eq!(loaded.header.max_width, 64);
        assert!(app.state.config_diagnostic.is_none());
    ...
    #[test]
    fn settings_save_header_max_width_clamps_low_and_high_on_write_path() {
        let _guard = config_env_lock().lock().unwrap();
        let path = temp_config_path("settings-save-header-clamp");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        std::fs::write(&path, "onboarding = false\n").unwrap();
        std::env::set_var(crate::config::CONFIG_PATH_ENV_VAR, &path);
        ...
        // Below the floor clamps up to MIN_HEADER_MAX_WIDTH (24).
        app.save_header_max_width(10);
        assert_eq!(
            app.state.header_max_width(),
            crate::config::MIN_HEADER_MAX_WIDTH
        );
        let content = std::fs::read_to_string(&path).unwrap();
        assert!(content.contains("max_width = 24"), "clamped low: {content}");

        // Above the cap clamps down to MAX_HEADER_MAX_WIDTH (200).
        app.save_header_max_width(999);
        assert_eq!(
            app.state.header_max_width(),
            crate::config::MAX_HEADER_MAX_WIDTH
        );
        let content = std::fs::read_to_string(&path).unwrap();
        assert!(
            content.contains("max_width = 200"),
            "clamped high: {content}"
        );
        assert!(toml::from_str::<crate::config::Config>(&content).is_ok());
        assert!(app.state.config_diagnostic.is_none());
    ...
    async fn assert_key_lifecycle_stays_with_pressed_pane(headless: bool, focus_lost: bool) {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("key-lifecycle");
        let pressed_pane = workspace.tabs[0].root_pane;
        let other_pane = workspace.test_split(ratatui::layout::Direction::Horizontal);
        workspace.tabs[0].layout.focus_pane(pressed_pane);
        let (runtime, mut pressed_rx) =
            TerminalRuntime::test_with_channel_and_scrollback_bytes(80, 24, 0, b"\x1b[>3u", 8);
        let key = crate::input::TerminalKey::new(KeyCode::Up, KeyModifiers::empty());
        let press = runtime.encode_terminal_key(key.clone());
        let repeat = runtime.encode_terminal_key(key.clone().with_kind(KeyEventKind::Repeat));
        let release = runtime.encode_terminal_key(key.with_kind(KeyEventKind::Release));
        assert!(!press.is_empty() && !repeat.is_empty() && !release.is_empty());
        assert_ne!(repeat, release);
        workspace.insert_test_runtime(pressed_pane, runtime);
        let (runtime, mut other_rx) = TerminalRuntime::test_with_channel(80, 24);
        workspace.insert_test_runtime(other_pane, runtime);
        app.state.workspaces = vec![workspace];
        app.state.active = Some(0);
        app.state.selected = 0;
        app.state.mode = Mode::Terminal;

        let event = raw_key(KeyCode::Up, KeyModifiers::empty(), KeyEventKind::Press);
        if headless {
            app.route_client_events(vec![event], false);
        } else {
            app.handle_raw_input_event(event).await;
        }
        assert_eq!(pressed_rx.try_recv().unwrap().as_ref(), press);
        assert!(app.state.focus_pane_in_workspace(0, other_pane));

        let events = if focus_lost {
            vec![crate::raw_input::RawInputEvent::OuterFocusLost]
        } else {
            vec![
                raw_key(KeyCode::Up, KeyModifiers::empty(), KeyEventKind::Repeat),
                raw_key(KeyCode::Up, KeyModifiers::empty(), KeyEventKind::Release),
            ]
        };
        for event in events {
            if headless {
        ...
        assert!(
            ...
            "held key reached a different pane"
        );
        if !focus_lost {
            assert_eq!(
                pressed_rx
                    .try_recv()
                    .expect("repeat at original pane")
                    .as_ref(),
                repeat
            );
        }
        assert_eq!(
            pressed_rx
                .try_recv()
                .expect("release at original pane")
                .as_ref(),
            release
        );
        assert!(pressed_rx.try_recv().is_err());
    ...
    #[tokio::test]
    async fn key_lifecycle_release_matches_owner_after_modifiers_change() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("key-release-order");
        let pane = workspace.tabs[0].root_pane;
        let (runtime, mut rx) =
            TerminalRuntime::test_with_channel_and_scrollback_bytes(80, 24, 0, b"\x1b[>3u", 4);
        let press = crate::input::TerminalKey::new(KeyCode::Up, KeyModifiers::ALT);
        let release = crate::input::TerminalKey::new(KeyCode::Up, KeyModifiers::empty())
            .with_kind(KeyEventKind::Release);
        let expected_press = runtime.encode_terminal_key(press.clone());
        let expected_release = runtime.encode_terminal_key(release.clone());
        assert!(!expected_release.is_empty());
        workspace.insert_test_runtime(pane, runtime);
        app.state.workspaces = vec![workspace];
        app.state.active = Some(0);
        app.state.selected = 0;
        app.state.mode = Mode::Terminal;
        app.route_client_events_from(1, vec![crate::raw_input::RawInputEvent::Key(press)], false);
        app.route_client_events_from(
            2,
            vec![crate::raw_input::RawInputEvent::Key(release.clone())],
            false,
        );
        assert_eq!(
            app.input_leases.len(),
            1,
            "another source cannot release this key"
        );
        assert_eq!(rx.try_recv().unwrap().as_ref(), expected_press);
        assert!(rx.try_recv().is_err());
        app.route_client_events_from(
            1,
            vec![crate::raw_input::RawInputEvent::Key(release)],
            false,
        );
        assert_eq!(rx.try_recv().unwrap().as_ref(), expected_release);
        assert!(rx.try_recv().is_err());
        assert!(app.input_leases.is_empty());
    }

    #[tokio::test]
    async fn headless_key_lifecycle_retains_pressed_pane_after_focus_change() {
        assert_key_lifecycle_stays_with_pressed_pane(true, false).await;
    }

    #[tokio::test]
    async fn local_key_lifecycle_retains_pressed_pane_after_focus_change() {
        assert_key_lifecycle_stays_with_pressed_pane(false, false).await;
    }

    #[tokio::test]
    async fn headless_key_lifecycle_releases_pressed_pane_on_focus_loss() {
        assert_key_lifecycle_stays_with_pressed_pane(true, true).await;
    }

    #[tokio::test]
    async fn local_key_lifecycle_releases_pressed_pane_on_focus_loss() {
        assert_key_lifecycle_stays_with_pressed_pane(false, true).await;
    }
    ...
    fn workspace_creation_in_navigate_mode_uses_selected_workspace_seed_cwd() {
        let mut app = test_app();
        let mut first = Workspace::test_new("zynk");
        first.identity_cwd = std::path::PathBuf::from("/tmp/zynk");
    ...
    fn new_terminal_cwd_path_uses_configured_path() {
        let cwd = creation::resolve_new_terminal_cwd(
            &crate::config::NewTerminalCwdConfig::Path("/tmp/zynk-fixed".into()),
            Some(std::path::PathBuf::from("/tmp/zynk-source")),
        );

        assert_eq!(cwd, std::path::PathBuf::from("/tmp/zynk-fixed"));
    }
    ...
    fn pane_close_request_requires_confirmation_before_closing_parent_worktree_group() {
        ...
        child.worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            ...
            label: "zynk".into(),
            repo_root: "/repo/zynk".into(),
            checkout_path: "/repo/zynk-child".into(),
    ...
    fn workspace_info_agent_status_prefers_working_over_done() {
        ...
        // and a working pane reports `working` (not `done`) in WorkspaceInfo.agent_status.
        let mut app = test_app();
        let mut ws = Workspace::test_new("mixed");
        let p_done = ws.tabs[0].root_pane;
        let p_working = ws.test_split(ratatui::layout::Direction::Horizontal);
        app.state.workspaces = vec![ws];
        app.state.ensure_test_terminals();

        // done / unseen pane
        let tid_done = app.state.workspaces[0].tabs[0].panes[&p_done]
            .attached_terminal_id
            .clone();
        app.state.terminals.get_mut(&tid_done).unwrap().state = AgentState::Idle;
        app.state.workspaces[0].tabs[0]
            .panes
            .get_mut(&p_done)
            .unwrap()
            .seen = false;
        // working pane
        let tid_working = app.state.workspaces[0].tabs[0].panes[&p_working]
            .attached_terminal_id
            .clone();
        app.state.terminals.get_mut(&tid_working).unwrap().state = AgentState::Working;
        ...
        assert!(
            ...
            "mixed done+working workspace must report working in the API"
        );
    }
...
"#,
    ...
    fn route_client_events_pastes_text_into_new_linked_worktree_modal() {
        ...
        app.state.worktree_create = Some(state::WorktreeCreateState {
            ...
            source_checkout_path: "/repo/zynk".into(),
            source_existing_membership: None,
            source_repo_root: "/repo/zynk".into(),
            repo_key: "repo-key".into(),
            repo_name: "zynk".into(),
            branch: "generated-branch".into(),
            checkout_path: "/repo/zynk-generated-branch".into(),
            error: None,
            creating: false,
        });
