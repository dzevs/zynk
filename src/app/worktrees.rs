...
impl App {
    ...
    > {
        ...
        let space = existing_membership
            ...
            .ok_or_else(|| {
                "Zynk worktree actions require a workspace inside a Git work tree.".to_string()
            })?;
    ...
    pub(crate) fn open_remove_linked_worktree_confirmation(&mut self, ws_idx: usize) {
            ...
            .is_some_and(|space| space.is_linked_worktree)
        {
            self.state.config_diagnostic =
                Some("This workspace is not a Zynk-managed worktree checkout.".into());
            return;
        }
...
mod tests {
    ...
    fn set_repo_identity(repo: &std::path::Path) {
        ...
        let config_path = if dot_git.is_dir() {
            ...
            std::path::PathBuf::from(String::from_utf8_lossy(&output.stdout).trim()).join("config")
        };
    ...
    fn open_selected_existing_worktree_focuses_already_open_workspace() {
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/repo/zynk-issue")
        );
    ...
    fn sync_worktree_branch_updates_derived_path() {
        ...
        app.state.worktree_create = Some(WorktreeCreateState {
            source_workspace_id: "source".into(),
            source_checkout_path: std::path::PathBuf::from("/repo/zynk"),
            source_existing_membership: None,
            source_repo_root: std::path::PathBuf::from("/repo/zynk"),
            repo_key: "repo-key".into(),
            repo_name: "zynk".into(),
            branch: "old".into(),
        ...
        assert_eq!(
            ...
            std::path::PathBuf::from("/w/zynk/issue-137")
        );
    ...
    fn worktree_create_enter_submits_through_api_path() {
        ...
        let checkout_path =
            crate::worktree::default_checkout_path(&app.state.worktree_directory, "zynk", branch);
    ...
    fn dirty_worktree_remove_failure_requests_force_confirmation() {
        let path = std::path::PathBuf::from("/w/zynk/dirty");
        let mut app = app_for_worktree_tests();
    ...
    fn non_dirty_worktree_remove_failure_keeps_error_message() {
        let path = std::path::PathBuf::from("/w/zynk/missing");
        let mut app = app_for_worktree_tests();
        app.state.worktree_remove = Some(WorktreeRemoveState {
            workspace_id: "ws".into(),
            repo_root: std::path::PathBuf::from("/repo/zynk"),
        ...
        assert_eq!(
            remove.error,
            Some("fatal: '/w/zynk/missing' is not a working tree".into())
        );
    }
