...
mod tests {
    ...
    fn notification_context_formats_resolved_workspace_label() {
        ...
        assert_eq!(
            notification_context(&state.workspaces[0], "__zynk_projects__", 0, root),
            "__zynk_projects__ · 1"
        );
    }
    ...
    fn double_click_word_bounds_cover_terminal_text() {
        let cases = [
            ...
            (
                "open ../zynk-worktrees/issue-1",
                "zynk",
                "../zynk-worktrees/issue-1",
            ),
            (
            ...
            ("cargo test --package=zynk", "--package", "--package=zynk"),
            (
            ...
            (
                "tmux|newhoo|fixhoo|newmoo|notification|window_bell|zynk",
                "newhoo",
                "newhoo",
            ),
            (
    ...
    async fn navigator_rows_match_live_root_runtime_cwd_workspace_label() {
        ...
        state.navigator.query = "zynk".into();
        ...
        // The live cwd matches the workspace; its pane is included as context.
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].label, "zynk (1)");
    ...
    fn navigator_search_only_matches_visible_row_text() {
        let mut state = app_with_workspaces(&["one"]);
        state.workspaces[0].identity_cwd = "/tmp/zynk-worktrees/issue-work".into();
    ...
    fn navigator_search_selects_first_self_match() {
        ...
        assert!(matches!(
            rows[state.navigator.selected].target,
    ...
    #[test]
    fn navigator_display_line_jumps_and_scroll_skip_workspace_spacers() {
        let mut state = app_with_workspaces(&["one", "two"]);
        let runtimes = crate::terminal::TerminalRuntimeRegistry::new();
        state.open_navigator_from(&runtimes);
        state.navigator.selected = 1;
        state.move_navigator_selection_by_lines_from(&runtimes, 1);
        assert_eq!(state.navigator.selected, 2);
        state.move_navigator_selection_by_lines_from(&runtimes, -1);
        assert_eq!(state.navigator.selected, 1);
        state.navigator.scroll = 2;
        state.align_navigator_selection_to_scroll_from(&runtimes);
        assert_eq!(state.navigator.selected, 2);
    }
    ...
    #[test]
    fn explicit_workspace_close_preserves_surviving_focus_across_indices() {
        for active in 0..3 {
            for closed in 0..3 {
                let mut state = app_with_workspaces(&["a", "b", "c"]);
                let active_id = state.workspaces[active].id.clone();
                state.active = Some(active);
                state.selected = closed;

                state.close_selected_workspace();

                let expected = if active == closed {
                    closed.min(1)
                } else {
                    active - usize::from(closed < active)
                };
                assert_eq!(
                    state.active,
                    Some(expected),
                ...
                if active != closed {
                    assert_eq!(state.workspaces[expected].id, active_id);
                }
                state.assert_invariants_for_test();
            }
        }
    ...
    #[test]
    fn self_closing_workspace_preserves_independent_active_and_selected_indices() {
        for active in 0..3 {
            for selected in 0..3 {
                for closed in 0..3 {
                    let mut state = app_with_workspaces(&["a", "b", "c"]);
                    let active_id = state.workspaces[active].id.clone();
                    let selected_id = state.workspaces[selected].id.clone();
                    let pane_id = state.workspaces[closed].tabs[0].root_pane;
                    state.active = Some(active);
                    ...
                    if active != closed {
                        assert_eq!(state.workspaces[expected(active)].id, active_id);
                    }
                    if selected != closed {
                        assert_eq!(state.workspaces[expected(selected)].id, selected_id);
                    }
                    state.assert_invariants_for_test();
                }
            }
    ...
    fn terminal_cwd_report_updates_terminal_cwd_and_marks_session_dirty() {
        ...
        let terminal_id = state.workspaces[0]
            ...
            .attached_terminal_id
            .clone();
        let cwd = std::env::temp_dir().join(format!("zynk-cwd-report-test-{}", std::process::id()));
    ...
    #[test]
    fn active_workspace_active_tab_keeps_zynk_toast_suppressed_when_outer_terminal_is_unfocused() {
        let mut state = app_with_workspaces(&["active"]);
        state.active = Some(0);
        state.outer_terminal_focus = Some(false);
        state.toast_config.delivery = crate::config::ToastDelivery::Zynk;
