...
impl App {
    ...
    fn start_pending_agent_resume(
        ...
        // across an agent-resume respawn so the resumed agent pane keeps its id.
        let launch_env = self
            .find_pane(pane_id)
        ...
        if let Some(terminal) = self.state.terminals.get_mut(&terminal_id) {
            ...
            // resumed with (the adapter strips the resume selector again on the next restore).
            terminal.launch_argv = Some(plan.argv.clone());
...
mod tests {
    ...
    async fn pending_agent_resume_records_effective_argv_back_into_launch_argv() {
        ...
        app.state
            ...
            dedupe_key: "zynk:codex\0codex\0Id\0record-back-session".into(),
        });
    ...
    async fn pending_agent_resume_launches_hidden_panes_with_current_terminal_area() {
        ...
        for terminal_id in [&active_terminal, &hidden_terminal] {
            app.state
                ...
                agent: "codex".into(),
                argv: long_running_test_argv(),
                dedupe_key: format!("zynk:codex\0codex\0Id\0{terminal_id}"),
            });
        }
    ...
    async fn pending_agent_resume_launches_inactive_tab_panes_with_current_terminal_area() {
        ...
        app.state
            ...
            agent: "codex".into(),
            argv: long_running_test_argv(),
            dedupe_key: "zynk:codex\0codex\0Id\0inactive-tab-session".into(),
        });
    ...
    async fn pending_agent_resume_launches_zoom_hidden_active_tab_panes() {
        ...
        app.state
            ...
            agent: "codex".into(),
            argv: long_running_test_argv(),
            dedupe_key: "zynk:codex\0codex\0Id\0zoom-hidden-session".into(),
        });
    ...
    #[test]
    fn shell_command_from_argv_quotes_preserved_flags() {
        let argv = vec![
            "claude".to_string(),
            "--model".to_string(),
        ...
        assert_eq!(
            shell_command_from_argv(&argv).as_deref(),
            Some("claude --model 'weird; rm -rf /' --resume id")
        );
    }
}
