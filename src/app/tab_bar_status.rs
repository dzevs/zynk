...
impl App {
    pub(crate) fn configure_tab_bar_status(
        &mut self,
...
mod tests {
    ...
    #[tokio::test]
    async fn b3_status_command_reports_a_bounded_sanitized_last_line() {
    ...
    #[tokio::test]
    async fn b3_status_command_drains_overlong_output_and_keeps_last_line() {
        let (event_tx, mut event_rx) = tokio::sync::mpsc::channel(1);
        let task = spawn_status_command(
            event_tx,
            7,
    ...
    async fn b3_status_timeout_starts_before_the_task_is_polled() {
        let ran = std::env::temp_dir().join(format!(
            "zynk-b3-status-after-timeout-{}",
            std::process::id()
        ));
        ...
        assert!(!ran.exists(), "status command ran after its deadline");
    }

    #[tokio::test(flavor = "current_thread")]
    async fn b3_reload_synchronously_terminates_status_command_descendants() {
        let base =
            std::env::temp_dir().join(format!("zynk-b3-status-cancel-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&base);
        std::fs::create_dir_all(&base).expect("create status test root");
        let started = base.join("started");
        let survived = base.join("survived");
        let command = format!(
        ...
        assert!(started.exists(), "status command descendant did not start");

        app.configure_tab_bar_status(
            &[crate::config::TabBarRightEntryConfig::Text {
                text: "reloaded".into(),
            }],
            " ",
        );
    ...
    #[tokio::test]
    async fn b3_consumed_status_deadline_advances_and_cannot_spin() {
        let mut app = test_app();
        app.configure_tab_bar_status(
            &[crate::config::TabBarRightEntryConfig::Command {
        ...
        let due = app
            ...
            .expect("initial status deadline");

        assert!(!app.handle_tab_bar_status_tasks(due));
        assert_eq!(app.next_tab_bar_status_deadline(), None);
        let event = tokio::time::timeout(Duration::from_secs(3), app.event_rx.recv())
            .await
            .expect("status completion timed out")
            .expect("status event channel closed");
        assert!(app.handle_internal_event_with_render_impact(event));
        assert_eq!(
            app.state.tab_bar_right,
            vec![crate::app::state::TabBarStatusSegment::Text(Some(
                "ready".into()
            ))]
        );
        assert!(app
    ...
    #[tokio::test]
    async fn b3_status_command_uses_only_bound_home_and_xdg_roots() {
        let base =
            std::env::temp_dir().join(format!("zynk-b3-status-roots-{}", std::process::id()));
        let forbidden = base.with_extension("outside");
        let _ = std::fs::remove_dir_all(&base);
        ...
        for path in [&home, &config, &data, &cache, &state] {
            std::fs::create_dir_all(path).expect("create sentinel root");
        }
        let command = format!(
            "test \"$HOME\" = {home} && test \"$XDG_CONFIG_HOME\" = {config} && test \"$XDG_DATA_HOME\" = {data} && test \"$XDG_CACHE_HOME\" = {cache} && test \"$XDG_STATE_HOME\" = {state} && test -z \"$ZYNK_SOCKET_PATH\" && printf cache > {cache}/status && printf state > {state}/status && printf READY",
            home = home.display(),
        ...
        let environment = vec![
            ...
            ("ZYNK_SOCKET_PATH".into(), String::new()),
            ("ZYNK_CLIENT_SOCKET_PATH".into(), String::new()),
            ("ZYNK_ENV".into(), String::new()),
        ];
        let (event_tx, mut event_rx) = tokio::sync::mpsc::channel(1);
        let task = spawn_status_command(
            event_tx,
        ...
        assert_eq!(
            std::fs::read(cache.join("status")).expect("cache output"),
            b"cache"
        );
        assert_eq!(
            std::fs::read(state.join("status")).expect("state output"),
            b"state"
        );
        assert!(
            !forbidden.exists(),
            "status command wrote outside bound roots"
        );
        let _ = std::fs::remove_dir_all(base);
    }

    #[test]
    fn b3_status_text_strips_controls_and_stale_results_are_ignored() {
        assert_eq!(
