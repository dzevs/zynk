//! Deferred (asynchronous) worktree create/remove for the socket API.
//!
//! Ported from upstream 46a2b25 ("defer api worktree operations"). The
//! synchronous `worktree.create`/`worktree.remove` handlers blocked the app
//! runtime on a `git worktree add/remove` subprocess. Here the request is
//! intercepted (see `app/runtime.rs` + `server/headless.rs`), the git command
//! runs on a background thread, and the caller's response channel
//! (`respond_to`) is held until the completion event lands — so plugin event
//! hooks and workspace mutations stay on the main loop while git runs off it.
//!
//! Zynk divergence note: this fork never ported the dedicated
//! `worktree.created`/`worktree.removed` plugin events (no such `EventKind`).
//! As in the synchronous path this replaces, the deferred path emits only the
//! workspace lifecycle events the fork supports (`workspace.created`,
//! `tab.created`, `pane.created`, `workspace.closed`).
...
impl App {
    ...
    /// Re-resolve the parent workspace for a completed create. The source
    /// workspace may have changed (closed, re-keyed, become a worktree itself)
    /// while git ran, so fall back to a key lookup when the recorded workspace
    /// no longer matches the expected membership.
    ...
    fn start_api_worktree_create(
        ...
        std::thread::spawn(move || {
            // Preserve upstream 89ca3ba: run_worktree_add_command checks out an
            // existing local branch instead of failing on `git worktree add -b`.
        ...
        let Some(space) = self
            ...
            Self::send_api_response(
                ...
                encode_error(
                    id,
                    "not_linked_worktree",
                    "workspace is not a Zynk-managed worktree checkout",
                ),
            );
            return;
        };
    ...
    pub(crate) fn handle_api_worktree_add_finished(
        ...
        let Some(worktree) = self.worktree_info_for_checkout(&source, ws_idx) else {
            Self::send_api_response(
                api.respond_to,
                encode_error(
                    api.id,
