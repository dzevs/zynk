...
struct RawPluginManifest {
    ...
    version: String,
    #[serde(default)]
    min_zynk_version: Option<String>,
    #[serde(default)]
...
pub(crate) fn load_plugin_manifest(
    ...
    let min_zynk_version = validate_min_zynk_version(raw.min_zynk_version.as_deref())?;
    let description = raw
        .description
...
fn validate_min_zynk_version(value: Option<&str>) -> Result<String, (&'static str, String)> {
    let Some(value) = value else {
        return Err((
            "invalid_plugin_min_zynk_version",
            "plugin min_zynk_version is required".to_string(),
        ));
    };
    let value = non_empty_trimmed(
        value,
        "invalid_plugin_min_zynk_version",
        "plugin min_zynk_version is required",
    )?;
    let required = crate::update::Version::parse(&value).ok_or_else(|| {
        (
            "invalid_plugin_min_zynk_version",
            format!(
                "plugin min_zynk_version must be a semantic version like {}",
    ...
    if required > current {
        return Err((
            ...
            format!("plugin requires zynk {required} or newer; current zynk is {current}"),
        ));
    }
...
pub(in crate::app::api) fn normalize_plugin_id(value: &str) -> Option<String> {
