...
impl App {
    ...
    fn plugin_pane_launch_env(
        ...
        env.push(("ZYNK_ENV".to_string(), "1".to_string()));
        env.push(("ZYNK_PLUGIN_ID".to_string(), plugin.plugin_id.clone()));
        env.push((
            "ZYNK_PLUGIN_ENTRYPOINT_ID".to_string(),
            entrypoint.to_string(),
        ));
        env.push(("ZYNK_PLUGIN_CONTEXT_JSON".to_string(), context_json));
...
fn plugin_pane_protected_env_key(key: &str) -> bool {
    matches!(
        ...
        crate::api::SOCKET_PATH_ENV_VAR
            ...
            | "ZYNK_PLUGIN_ID"
            | "ZYNK_PLUGIN_ROOT"
            | "ZYNK_PLUGIN_CONFIG_DIR"
            | "ZYNK_PLUGIN_STATE_DIR"
            | "ZYNK_PLUGIN_ENTRYPOINT_ID"
            | "ZYNK_PLUGIN_CONTEXT_JSON"
            | "ZYNK_BIN_PATH"
    )
}
