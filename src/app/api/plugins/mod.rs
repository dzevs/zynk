...
impl App {
    ...
    pub(super) fn handle_plugin_pane_close(
        ...
        let public_pane_id = params.pane_id;
        // Delegate to the shared pane-close handler, then re-shape the success
        // response into PluginPaneClosed. The plugin_pane record is dropped by
        // close_pane / handle_pane_died cleanup paths.
        let response = self.handle_pane_close(
            id.clone(),
...
"#,
    ...
    #[test]
    fn m842_manifest_preserves_literal_argv_and_rejects_empty_elements() {
        let whitespace_root = unique_temp_path("plugin-whitespace-argv");
        write_manifest_content(
            &whitespace_root,
            r#"
...
"#,
        ...
        let plugin = load_plugin_manifest(&whitespace_root.display().to_string(), true)
            .expect("literal whitespace argv elements should be valid");
        assert_eq!(plugin.panes[0].command, ["awk", "-F", "\t", " {print $1} "]);

        let event_root = unique_temp_path("plugin-event-whitespace-order");
        write_manifest_content(
            &event_root,
            r#"
...
"#,
        ...
        let plugin = load_plugin_manifest(&event_root.display().to_string(), true)
...
"#,
            ...
            assert!(matches!(
                ...
                Err(("invalid_plugin_command", _))
            ));
            let _ = std::fs::remove_dir_all(root);
        }

        let _ = std::fs::remove_dir_all(whitespace_root);
...
"#,
    ...
    fn plugin_link_lists_and_unlinks_manifest() {
        ...
        assert_eq!(plugin.events[0].on, "pane.created");
    ...
    #[test]
    fn link_rejects_invalid_min_zynk_versions() {
        let cases = [
            (
                "plugin-missing-min-zynk",
                r#"
id = "example.missing-min-zynk"
name = "Missing Min Zynk"
version = "0.1.0"
platforms = ["linux", "macos", "windows"]
"#,
                "invalid_plugin_min_zynk_version",
            ),
            (
                "plugin-invalid-min-zynk",
                r#"
id = "example.invalid-min-zynk"
name = "Invalid Min Zynk"
version = "0.1.0"
min_zynk_version = "soon"
platforms = ["linux", "macos", "windows"]
"#,
                "invalid_plugin_min_zynk_version",
            ),
            (
                "plugin-future-min-zynk",
                r#"
id = "example.future-min-zynk"
name = "Future Min Zynk"
version = "0.1.0"
min_zynk_version = "999.0.0"
platforms = ["linux", "macos", "windows"]
"#,
                "plugin_requires_newer_zynk",
            ),
        ];
...
"#,
    ...
    fn plugin_command_output_reader_caps_and_marks_truncation() {
        ...
        assert_eq!(output, "abc\n[zynk truncated plugin output after 3 bytes]");
    }
...
id = "board"
title = "Plugin Board"
command = ["sh", "-c", "printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \"$PWD\" \"$ZYNK_PLUGIN_ID\" \"$ZYNK_PLUGIN_ENTRYPOINT_ID\" \"$ZYNK_WORKSPACE_ID\" \"$ZYNK_PANE_ID\" \"$ZYNK_BIN_PATH\" \"$ZYNK_PLUGIN_CONTEXT_JSON\" > {}"]
"#,
                capture.display()
            ),
        );
        ...
        let open = app.handle_api_request(Request {
            ...
            method: Method::PluginPaneOpen(PluginPaneOpenParams {
                ...
                env: std::collections::HashMap::from([
                    ("ZYNK_PLUGIN_ID".to_string(), "spoofed-plugin".to_string()),
                    (
                        "ZYNK_PLUGIN_ENTRYPOINT_ID".to_string(),
                        "spoofed-entrypoint".to_string(),
                    ),
                    (
                        "ZYNK_PLUGIN_CONTEXT_JSON".to_string(),
                        "{\"spoofed\":true}".to_string(),
                    ),
                    ("ZYNK_BIN_PATH".to_string(), "/tmp/spoofed-zynk".to_string()),
                ]),
            }),
        });
        ...
        assert_ne!(bin_path, "/tmp/spoofed-zynk");
        assert_eq!(
            bin_path,
            std::env::current_exe()
...
id = "board"
title = "Plugin Board"
command = ["sh", "-c", "printf '%s\n%s\n%s\n' \"$ZYNK_PLUGIN_ROOT\" \"$ZYNK_PLUGIN_CONFIG_DIR\" \"$ZYNK_PLUGIN_STATE_DIR\" > {}"]
"#,
                capture.display()
            ),
        );
        ...
        let open = app.handle_api_request(Request {
            ...
            method: Method::PluginPaneOpen(PluginPaneOpenParams {
                ...
                env: std::collections::HashMap::from([
                    (
                        "ZYNK_PLUGIN_ROOT".to_string(),
                        "/tmp/spoofed-root".to_string(),
                    ),
                    (
                        "ZYNK_PLUGIN_CONFIG_DIR".to_string(),
                        "/tmp/spoofed-config".to_string(),
                    ),
                    (
                        "ZYNK_PLUGIN_STATE_DIR".to_string(),
...
id = "run"
title = "Run"
command = ["sh", "-c", "printf '%s' \"$ZYNK_PLUGIN_ACTION_ID\""]
"#,
        );
        link_manifest(&mut app, &root);
...
id = "run"
title = "Run"
command = ["sh", "-c", "printf '%s\n%s\n%s' \"$ZYNK_PLUGIN_ROOT\" \"$ZYNK_PLUGIN_CONFIG_DIR\" \"$ZYNK_PLUGIN_STATE_DIR\""]
"#,
        );
        link_manifest(&mut app, &root);
...
command = ["sh", "-c", "printf '%s' \"$ZYNK_PLUGIN_CONTEXT_JSON\" > {}"]
"#,
                capture.display()
            ),
        );
        ...
        app.run_plugin_event_hooks(&crate::api::schema::EventEnvelope {
            ...
            data: crate::api::schema::EventData::WorkspaceFocused {
                workspace_id: target_workspace.workspace_id.clone(),
            },
        });
...
id = "open"
title = "Open link"
command = ["sh", "-c", "printf '%s|%s' \"$ZYNK_PLUGIN_LINK_HANDLER_ID\" \"$ZYNK_PLUGIN_CLICKED_URL\""]
...
"#,
        ...
        let handled = app
            .invoke_plugin_link_handler_for_url(
                "https://github.com/example/repo/issues/398",
                pane_id,
            )
        ...
        assert_eq!(
            ...
            Some("github-issue|https://github.com/example/repo/issues/398")
        );
...
"#,
        ...
        let (_plugin, handler) = app
            .find_plugin_link_handler("https://github.com/example/repo/issues/398")
...
"#,
    ...
    fn manifest_action_invoke_builds_default_workspace_context() {
        ...
        app.state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            key: "repo-key".into(),
            label: "acme".into(),
            repo_root: "/repo/acme".into(),
            checkout_path: "/repo/acme-issue".into(),
...
"#,
        ...
        assert_eq!(worktree.repo_name, "acme");
        assert_eq!(worktree.repo_root, "/repo/acme");
        assert_eq!(worktree.checkout_path, "/repo/acme-issue");
    ...
    fn link_with_unknown_event_name_succeeds_with_warning() {
        ...
        assert_eq!(
            plugin
                .warnings
                .iter()
                .filter(|w| w.contains("pane.created"))
                .count(),
            0
        );
        assert!(
...
"#,
    ...
    fn m847_live_plugin_consumers_refresh_global_enabled_state() {
        ...
        assert!(!app
            .invoke_plugin_link_handler_for_url(
                "https://github.com/example/zynk/issues/1174",
                pane_id,
            )
            .unwrap());
        ...
        let response: serde_json::Value = serde_json::from_str(&response).unwrap();
        assert_eq!(response["error"]["code"], "plugin_disabled");
