...
/// Serializable snapshot of the entire zynk session.
...
/// Choose the `launch_argv` to persist for a pane.
///
/// For a pane with a valid official native `agent_session`, persist ONLY a sanitized resume argv
/// (Tier B from the live foreground command, or Tier A from a safe existing argv) — or `None`. We
/// NEVER fall back to the raw `existing_launch_argv` here: it feeds the native resume path and could
/// carry secrets/unknown/`--` payload/config/variadic forms that must not be persisted or replayed
/// (TB-001). When nothing safe is preserved, `None` lets restore rebuild the canonical resume from
/// the session ref.
///
/// Non-agent panes (and panes whose `agent_session` is not a valid official native session, i.e.
/// outside native agent resume) keep their existing `launch_argv` unchanged — that is the handoff
/// respawn argv and the operator's own chosen command, not a resume input.
fn persisted_agent_launch_argv(
...
fn capture_tab(
    ...
    for id in tab.panes.keys() {
        ...
        let agent_session = attached_terminal_id
            .as_ref()
        ...
        let existing_launch_argv = attached_terminal_id
            .as_ref()
            .and_then(|tid| terminals.get(tid))
...
mod tests {
    ...
    fn session_fixture(name: &str) -> &'static str {
        match name {
            "current-zynk" => {
                include_str!("../../tests/fixtures/session/current-zynk-session.json")
            }
            "current-zynk-dev" => {
                include_str!("../../tests/fixtures/session/current-zynk-dev-session.json")
            }
            "legacy-pre-tabs-v2" => {
    ...
    fn round_trip_full_workspace_snapshot() {
        ...
        let snap = SessionSnapshot {
            workspaces: vec![WorkspaceSnapshot {
                ...
                identity_cwd: PathBuf::from("/home/user/Projects/zynk"),
                worktree_space: None,
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/user/Projects/zynk")
        );
        assert_eq!(
    ...
    fn current_session_fixture_parses() {
        let snap = parse_snapshot(session_fixture("current-zynk")).unwrap();
    ...
    fn current_dev_session_fixture_parses_additive_fields() {
        let snap = parse_snapshot(session_fixture("current-zynk-dev")).unwrap();
    ...
    fn legacy_workspace_snapshot_migrates_to_single_tab() {
        ...
        assert_eq!(ws.tabs[0].panes[&1].cwd, PathBuf::from("/tmp/zynk"));
    }
    ...
    fn capture_contract_tracks_worktree_space_membership() {
        ...
        state.workspaces[0].worktree_space = Some(crate::workspace::WorktreeSpaceMembership {
            key: "repo-key".into(),
            label: "zynk".into(),
            repo_root: PathBuf::from("/repo/zynk"),
            checkout_path: PathBuf::from("/repo/zynk/worktree-a"),
    ...
    fn capture_contract_tracks_workspace_identity_and_pane_cwds() {
        ...
        let second_terminal_id = state.workspaces[0].tabs[0].panes[&second]
            .attached_terminal_id
            .clone();
        state.terminals.get_mut(&second_terminal_id).unwrap().cwd = PathBuf::from("/tmp/zynk");
        ...
        assert_eq!(tab.panes[&second.raw()].cwd, PathBuf::from("/tmp/zynk"));
    }
    ...
    fn capture_contract_tracks_hook_authority_agent_session() {
        ...
        assert_eq!(agent_session.source, "zynk:pi");
    ...
    fn capture_contract_preserves_restored_agent_session() {
        ...
        assert_eq!(agent_session.source, "zynk:opencode");
    ...
    #[test]
    fn launch_argv_round_trips_through_snapshot() {
        let mut state = state_with_workspaces(&["one"]);
        let root = state.workspaces[0].tabs[0].root_pane;
        state.ensure_test_terminals();
        let terminal_id = state.workspaces[0].tabs[0].panes[&root]
        ...
        assert_eq!(
            restored.workspaces[0].tabs[0].panes[&root.raw()].launch_argv,
            Some(argv)
        );
    }

    #[test]
    fn persisted_agent_launch_argv_gates_and_falls_back() {
        ...
        let sess = PaneAgentSessionSnapshot {
            source: "zynk:claude".into(),
            agent: "claude".into(),
            kind: crate::agent_resume::AgentSessionRefKind::Id,
            value: "X".into(),
        };
        // Agent pane, no live foreground (child_pid None): a flag-preserving Tier A launch_argv is
        // sanitized into a resume argv.
        let tier_a = Some(vec![
            "claude".to_string(),
            "--dangerously-skip-permissions".to_string(),
        ]);
        ...
        // Agent pane, no foreground, canonical-only existing argv (nothing to preserve): None — the
        // raw argv is NOT persisted; restore rebuilds canonical from the session ref.
        let canonical = Some(vec![
            "claude".to_string(),
            "--resume".to_string(),
            "X".to_string(),
        ]);
    ...
    #[test]
    fn persisted_agent_launch_argv_never_persists_raw_unsafe_argv_for_official_agent() {
        // TB-001: an official agent pane whose existing launch_argv is secret-bearing must NOT have
        // that raw argv persisted when sanitization rejects it; the helper returns None.
        let pi = PaneAgentSessionSnapshot {
            source: "zynk:pi".into(),
            agent: "pi".into(),
    ...
    fn restore_falls_back_to_home_when_cwd_missing() {
        ...
        assert_eq!(
            restored.workspaces[0].tabs[0].panes[&0].cwd,
            PathBuf::from("/tmp/this-directory-does-not-exist-for-zynk-test")
        );
    }
}
