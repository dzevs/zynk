...
fn restore_tab(
    ...
    for id in &pane_ids {
        ...
        let public_pane_id = old_pane_id
            ...
            .map(String::as_str);
        // Restore the pane identity triple (ZYNK_WORKSPACE_ID/ZYNK_TAB_ID/
        // ZYNK_PANE_ID) so a restored pane keeps the id it had before.
        ...
        if let Some(plan) = pending_native_agent_restore {
            ...
            // Carry the original launch argv onto the deferred terminal so a re-snapshot taken
            // before the resume actually launches still preserves the operator's flags. Do NOT set
            // respawn_shell_on_exit — that is the handoff lifecycle, not native agent resume.
...
mod tests {
    ...
    fn restored_worktree_space_membership_drops_missing_checkout() {
        let missing =
            std::env::temp_dir().join(format!("zynk-missing-worktree-{}", std::process::id()));
    ...
    #[test]
    fn restore_plan_threads_launch_argv_into_resume() {
        let session = super::super::snapshot::PaneAgentSessionSnapshot {
            source: "zynk:claude".into(),
            agent: "claude".into(),
        ...
        let plan = restore_plan_for_snapshot(&session, true, Some(argv.as_slice())).unwrap();
        // Selector-index semantics: `--continue` was at the front, so `--resume <id>` replaces it
        // there, ahead of the preserved policy flag. Matches the agent_resume adapter contract.
        assert_eq!(
            plan.argv,
            vec![
                "claude",
    ...
    #[test]
    fn restore_plan_without_launch_argv_falls_back_to_identity() {
        let session = super::super::snapshot::PaneAgentSessionSnapshot {
            source: "zynk:claude".into(),
            agent: "claude".into(),
            kind: crate::agent_resume::AgentSessionRefKind::Id,
        ...
        let plan = restore_plan_for_snapshot(&session, true, None).unwrap();
        assert_eq!(plan.argv, vec!["claude", "--resume", "new-id"]);
    }
    ...
    fn pane_restore_startup_suppresses_history_for_native_agent_resume() {
        ...
        let startup =
            pane_restore_startup(Some(&session), Some(&history), &mut agent_restore, None);
    ...
    fn restore_rehydrates_agent_session_metadata() {
        ...
        assert_eq!(preserved.source, "zynk:hermes");
