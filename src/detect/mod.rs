...
/// foreground job's identified agent matches `expected_agent` (the hook-reported official agent label).
///
/// Returns the raw process argv (which the resume-flag sanitizer then default-deny-rewrites); returns
/// `None` when there is no foreground job, the identified agent does not match, or argv is unavailable.
...
mod tests {
    ...
    #[test]
    fn foreground_agent_argv_does_not_misfire_for_non_agent_process() {
        // The test runner is not an agent: no foreground job matches `claude`, so we get None and
        // never capture an unrelated process's argv. Unknown agent labels also yield None.
        assert!(foreground_agent_argv(std::process::id(), "claude").is_none());
        assert!(foreground_agent_argv(std::process::id(), "not-an-agent").is_none());
    }
        ...
        let job = crate::platform::ForegroundJob {
            ...
            processes: vec![foreground_process(
                ...
                &[
                    ...
                    "-NoProfile",
                    "-File",
                    "C:\\Users\\zynk\\Documents\\PowerShell\\Scripts\\claude.ps1",
                ],
            )],
        };
