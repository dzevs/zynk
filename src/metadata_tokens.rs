...
mod tests {
    ...
    fn sequence_sources_are_bounded() {
        ...
        assert_eq!(
            accept_sequence(&mut sequences, "unsequenced", None),
            Ok(true)
        );
        assert!(sequences.is_empty());
        for index in 0..32 {
            assert_eq!(
                accept_sequence(&mut sequences, &format!("source-{index}"), Some(0)),
                Ok(true)
            );
        }
        assert_eq!(sequences.len(), 32);
        assert_eq!(
            accept_sequence(&mut sequences, "one-too-many", Some(0)),
            Err(())
        );
        assert!(!sequences.contains_key("one-too-many"));
        assert_eq!(
            accept_sequence(&mut sequences, "unsequenced", None),
            Ok(true)
        );
        assert_eq!(sequences.len(), 32);
        assert_eq!(
            accept_sequence(&mut sequences, "source-0", Some(0)),
            Ok(false)
        );
        assert_eq!(
        ...
        assert_eq!(accept_sequence(&mut sequences, "source-0", None), Ok(true));
        assert_eq!(sequences["source-0"], 2);
    }
    ...
    fn patches_and_clears_individual_keys() {
        ...
        let update = patch(&[("summary", Some("two")), ("model", None)]);
        assert_eq!(tokens.key_count_after_patch(&update), 2);
        assert!(tokens.patch(update, None, now));
        assert_eq!(
            tokens.values(),
            HashMap::from([
                ("summary".into(), "two".into()),
                ("branch".into(), "main".into())
            ])
        );
        assert!(!tokens.patch(
            patch(&[("summary", Some("two")), ("missing", None)]),
            None,
            now
        ));
    }
    ...
    fn ttl_only_changes_keys_in_the_patch() {
        ...
        assert!(!tokens.expire_at(deadline - Duration::from_millis(1)));
        assert!(tokens.expire_at(deadline));
        assert_eq!(
            tokens.values(),
            HashMap::from([("persistent".into(), "two".into())])
        );
        assert_eq!(tokens.next_expiry(), None);
    }
    ...
    fn values_remain_stable_until_expiry_mutates_state() {
        ...
        assert_eq!(tokens.next_expiry(), Some(deadline));
        assert!(tokens.expire_at(deadline));
    ...
    fn delayed_expiry_sweep_removes_every_token_due_at_now() {
        ...
        assert!(tokens.values().is_empty());
        assert!(!tokens.expire_at(now + Duration::from_secs(11)));
    }
    ...
    fn stale_expiry_does_not_clear_replacement() {
        ...
        assert_eq!(tokens.next_expiry(), Some(now + Duration::from_secs(5)));
        assert!(tokens.patch(
            patch(&[("summary", Some("new"))]),
