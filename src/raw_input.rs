...
impl RawInputByteFramer {
    ...
    /// Hold a lone trailing ESC for one idle flush so an OSC 4/10/11 reply split
    ...
    /// Hold split XTWINOPS replies for the same bounded window as color replies.
    ...
    pub(crate) fn flush_timeout(&mut self) -> Vec<Vec<u8>> {
        ...
        if self.lone_escape_recently_flushed && self.buffer.starts_with(b"[<") {
            self.discard_until = Some(ControlStringFamily::OrphanedSgrMouseTail);
            self.discarded_tail_bytes = 0;
            self.buffer.clear();
        ...
        if (self.host_appearance_reply_awaited || self.host_cell_size_replies_awaited > 0)
...
fn plausible_control_string_tail(family: ControlStringFamily, buffer: &[u8]) -> bool {
    match family {
        ...
        ControlStringFamily::HostReplyCsi | ControlStringFamily::OrphanedSgrMouseTail => false,
    }
}
...
pub fn spawn_input_reader() -> mpsc::Receiver<RawInputEvent> {
    ...
    std::thread::spawn(move || {
        ...
        raw_input_reader_loop(stdin.lock(), &tx);
    });

    rx
}

fn raw_input_reader_loop<R: Read + AsRawFd>(mut reader: R, tx: &mpsc::Sender<RawInputEvent>) {
        ...
        match reader.read(&mut scratch) {
            ...
            Ok(n) => {
                ...
                if stdin_read_ready(&reader, input_flush_timeout_ms(&framer)) == Some(false) {
                    ...
                    if held_escape
                        && stdin_read_ready(&reader, RAW_INPUT_IDLE_FLUSH_TIMEOUT_MS) == Some(false)
                    {
                        send_raw_input_events(framer.flush_timeout(), tx, &mut pending_palette);
                        flush_host_palette_events(tx, &mut pending_palette);
                    }
                }
            }
...
fn discard_complete_orphaned_sgr_mouse_tail(buffer: &mut Vec<u8>) -> bool {
    ...
    let Some(end) = buffer
        .iter()
        .take(MAX_ORPHANED_SGR_MOUSE_TAIL_BYTES)
        .position(|byte| matches!(*byte, b'M' | b'm'))
    else {
        return false;
    };
    let len = end + 1;
    let mut sequence = [0; MAX_ORPHANED_SGR_MOUSE_TAIL_BYTES + 1];
    sequence[0] = ESC;
    sequence[1..=len].copy_from_slice(&buffer[..len]);
    let Ok(sequence) = std::str::from_utf8(&sequence[..=len]) else {
        return false;
    };
...
fn discard_orphaned_sgr_mouse_tail(buffer: &mut Vec<u8>, discarded_tail_bytes: &mut usize) -> bool {
    // Drain on every push, not only timeout, so an unterminated tail cannot accumulate.
...
fn control_string_terminator_for_family(
    ...
    match family {
        ...
        ControlStringFamily::HostReplyCsi | ControlStringFamily::OrphanedSgrMouseTail => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn m831_split_motion_reassembles_at_every_boundary_before_timeout() {
        for report in [b"\x1b[<35;58;30M".as_slice(), b"\x1b[<35;58;30m".as_slice()] {
            for split in 1..report.len() {
                let mut framer = RawInputFramer::default();
                assert!(framer.push(&report[..split]).is_empty());
                let events = framer.push(&report[split..]);
                assert!(
                    matches!(events.as_slice(), [RawInputEvent::Mouse(mouse)]
                    if mouse.kind == MouseEventKind::Moved && mouse.column == 57 && mouse.row == 29
                        && mouse.modifiers.is_empty()),
                    "report={report:?}, split={split}, events={events:?}"
                );
    ...
    #[test]
    fn m831_full_prefix_timeout_discards_tail_and_keeps_following_key() {
        for report in [b"\x1b[<35;58;30M".as_slice(), b"\x1b[<35;58;30m".as_slice()] {
            for split in 3..report.len() {
                let mut framer = RawInputFramer::default();
                assert!(framer.push(&report[..split]).is_empty());
                assert!(framer.flush_timeout().is_empty());
                let mut tail = report[split..].to_vec();
                tail.push(b'x');
                let events = framer.push(&tail);
                assert_eq!(events.len(), 1, "split={split}, events={events:?}");
                assert_raw_key(
        ...
        let mut interleaved = RawInputByteFramer::default();
        interleaved.host_color_query_sent();
        let awaiting = interleaved.host_color_replies_awaited;
        assert!(awaiting > 0);
        assert!(interleaved.push(b"\x1b").is_empty());
        assert!(interleaved.flush_timeout().is_empty());
        assert!(interleaved.held_pending_host_reply_esc);
        assert!(interleaved.push(b"[<3").is_empty());
        assert!(interleaved.flush_timeout().is_empty());
        assert_eq!(interleaved.host_color_replies_awaited, awaiting);
        assert!(interleaved.push(b"5;58;30M").is_empty());
        assert!(interleaved.push(b"\x1b").is_empty());
        assert!(
            interleaved.flush_timeout().is_empty(),
            "next host reply gets its own Escape hold"
        );
        assert_eq!(interleaved.host_color_replies_awaited, awaiting);
        assert_eq!(
            interleaved.push(b"]11;#123456\x07"),
            vec![b"\x1b]11;#123456\x07".to_vec()]
        );
        assert_eq!(interleaved.host_color_replies_awaited, awaiting - 1);
    }

    #[test]
    fn m831_quiescence_releases_both_mouse_origins_but_not_host_csi() {
        for orphan_origin in [true, false] {
            ...
            assert!(framer.flush_timeout().is_empty());
            assert_eq!(
                framer.discard_until,
                Some(ControlStringFamily::OrphanedSgrMouseTail)
            );
            assert!(framer.flush_timeout().is_empty());
            assert_eq!(framer.discard_until, None, "orphan_origin={orphan_origin}");
            assert!(framer.buffer.is_empty());
            assert_eq!(framer.discarded_tail_bytes, 0);
            assert_eq!(framer.push(b"43;26Mx").concat(), b"43;26Mx");
        }

        let mut host = RawInputByteFramer::default();
        host.host_cell_size_query_sent();
        assert!(host.push(b"\x1b[6;21").is_empty());
        assert!(host.flush_timeout().is_empty());
        for _ in 0..2 {
            assert!(host.flush_timeout().is_empty());
            assert_eq!(host.discard_until, Some(ControlStringFamily::HostReplyCsi));
        }
        assert_eq!(host.push(b";10xabc").concat(), b"abc");
    }

    #[test]
    fn m831_full_prefix_budget_counts_prefix_and_preserves_surplus() {
        for prefix_len in [3usize, 4, 127, 128, 129] {
            let mut prefix = b"\x1b[<".to_vec();
            prefix.resize(prefix_len, b'0');
            let mut framer = RawInputByteFramer::default();
            ...
            if prefix_len <= 128 {
                ...
                assert_eq!(framer.discarded_tail_bytes, prefix_len);
                assert!(framer.push(&vec![b'7'; 128 - prefix_len]).is_empty());
                assert_eq!(framer.discard_until, None, "prefix_len={prefix_len}");
                assert_eq!(framer.discarded_tail_bytes, 0);
            } else {
                assert_eq!(framer.discard_until, None);
            }
            assert_eq!(framer.push(b"7x").concat(), b"7x");
        }

        for suffix in [
            b"7x".as_slice(),
            b"\x1b[A".as_slice(),
            b"\x1b[200~p\x1b[201~".as_slice(),
        ] {
            let mut framer = RawInputByteFramer::default();
            assert!(framer.push(b"\x1b[<3").is_empty());
            assert!(framer.flush_timeout().is_empty());
            assert_eq!(framer.discarded_tail_bytes, 4);
            let mut tail = vec![b'7'; 124];
            tail.extend_from_slice(suffix);
            assert_eq!(framer.push(&tail).concat(), suffix, "suffix={suffix:?}");
            assert_eq!(framer.discard_until, None);
            assert!(framer.buffer.is_empty());
        }
    }
    ...
    #[test]
    fn m831_legacy_timeout_extends_only_undiscarded_full_mouse_prefix() {
        for (bytes, expected) in [
            ...
            let mut framer = RawInputFramer::default();
            framer.push(bytes);
            assert_eq!(input_flush_timeout_ms(&framer), expected, "{bytes:?}");
        }

        for introducer in [b"\x1b]".as_slice(), b"\x1bP".as_slice()] {
            let mut discarding = RawInputFramer::default();
            ...
            assert_eq!(input_flush_timeout_ms(&discarding), 10);
        }
    }

    #[test]
    fn raw_key_source_retains_exact_vt_bytes_and_separate_text_metadata() {
        for (bytes, text) in [
            (b"\x1b".as_slice(), None),
            (b"\x1b[106;5u".as_slice(), None),
            (b"j".as_slice(), Some("j")),
            ("\u{00c9}".as_bytes(), Some("\u{00c9}")),
        ] {
            let events = parse_raw_input_bytes_sync(bytes);
            let [RawInputEvent::Key(key)] = events.as_slice() else {
                panic!("expected one key: {events:?}");
            };
            assert_eq!(key.vt_bytes(), Some(bytes));
            assert_eq!(key.generated_text.as_deref(), text);
        }
    }
    ...
    async fn input_reader_releases_escape_without_another_read_after_focus() {
        ...
        let thread = std::thread::spawn(move || raw_input_reader_loop(reader, &tx));
        ...
        assert!(matches!(focus, Some(RawInputEvent::OuterFocusGained)));
        assert_raw_key(escape.unwrap(), KeyCode::Esc, KeyModifiers::empty());
    }
    ...
    #[test]
    fn cell_size_and_appearance_replies_keep_independent_pending_windows() {
        let cell = b"\x1b[6;21;10t".as_slice();
        let appearance = GHOSTTY_COLOR_SCHEME_DARK_REPORT;
        for (first, second) in [(cell, appearance), (appearance, cell)] {
            let mut framer = RawInputByteFramer::default();
            framer.host_cell_size_query_sent();
            framer.enable_host_appearance_query_on_focus();
            assert_eq!(framer.push(b"\x1b[I"), vec![b"\x1b[I".to_vec()]);
            assert_eq!(framer.push(first), vec![first.to_vec()]);

            assert!(framer.push(b"\x1b").is_empty());
            assert!(framer.flush_timeout().is_empty());
            assert_eq!(framer.push(&second[1..]), vec![second.to_vec()]);
    ...
    #[test]
    fn timed_out_cell_size_reply_preserves_pending_appearance_reply() {
        let mut framer = RawInputByteFramer::default();
        framer.host_cell_size_query_sent();
        framer.enable_host_appearance_query_on_focus();
        assert_eq!(framer.push(b"\x1b[I"), vec![b"\x1b[I".to_vec()]);
        ...
        assert!(framer.flush_timeout().is_empty());
        assert_eq!(
            framer.push(b"[?997;1n"),
            vec![GHOSTTY_COLOR_SCHEME_DARK_REPORT.to_vec()]
        );
    }

    #[test]
    fn timed_out_appearance_reply_preserves_pending_cell_size_reply() {
        let mut framer = RawInputByteFramer::default();
        framer.host_cell_size_query_sent();
        framer.enable_host_appearance_query_on_focus();
        assert_eq!(framer.push(b"\x1b[I"), vec![b"\x1b[I".to_vec()]);
        assert!(framer.push(b"\x1b[?997;1").is_empty());
        assert!(framer.flush_timeout().is_empty());
        assert!(framer.flush_timeout().is_empty());
    ...
    #[test]
    fn m810_sgr_before_flush_preserves_mouse_coordinates_and_kind() {
        for (report, kind) in [
            (
                b"\x1b[<0;43;26M".as_slice(),
            ...
            for split in 1..report.len() {
                ...
                assert!(
                    matches!(events.as_slice(), [RawInputEvent::Mouse(mouse)]
                    if mouse.kind == kind && mouse.column == 42 && mouse.row == 25
                        && mouse.modifiers.is_empty()),
                    "split={split}: {events:?}"
                );
                assert!(framer.flush_timeout().is_empty());
    ...
    #[test]
    fn m810_orphan_mouse_complete_tail_is_discarded_after_emitted_escape() {
        for tail in [b"[<0;43;26M".as_slice(), b"[<0;43;26m".as_slice()] {
            let mut framer = RawInputFramer::default();
            assert!(framer.push(b"\x1b").is_empty());
            let mut escape = framer.flush_timeout().into_iter();
            assert_raw_key(escape.next().unwrap(), KeyCode::Esc, KeyModifiers::empty());
    ...
    #[test]
    fn m810_orphan_mouse_complete_tail_preserves_following_input() {
        let mut framer = RawInputByteFramer::default();
        assert!(framer.push(b"\x1b").is_empty());
        assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
        assert_eq!(framer.push(b"[<65;43;26Mx"), vec![b"x".to_vec()]);
        assert!(framer.flush_timeout().is_empty());
    }

    #[test]
    fn m810_orphan_mouse_invalid_tail_is_released_byte_exact() {
        for tail in [
            b"[<x".as_slice(),
            b"[<0;0;26M".as_slice(),
            b"[<0;43;26;M".as_slice(),
            b"[<0;43;26;7M".as_slice(),
        ] {
            let mut framer = RawInputByteFramer::default();
            assert!(framer.push(b"\x1b").is_empty());
            assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
            assert_eq!(framer.push(tail).concat(), tail);
            assert_eq!(framer.push(b"[<0;43;26M").concat(), b"[<0;43;26M");
        }
    }

    #[test]
    fn m810_orphan_mouse_double_split_timeout_finishes_at_either_terminator() {
        for last in [b"3;26Mx".as_slice(), b"3;26mx".as_slice()] {
            let mut framer = RawInputByteFramer::default();
            assert!(framer.push(b"\x1b").is_empty());
            assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
            assert!(framer.push(b"[<0;4").is_empty());
            assert!(framer.flush_timeout().is_empty());
            assert_eq!(framer.push(last), vec![b"x".to_vec()]);
    ...
    #[test]
    fn m810_orphan_mouse_incremental_drain_has_a_continuous_budget() {
        let mut framer = RawInputByteFramer::default();
        assert!(framer.push(b"\x1b").is_empty());
        assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
        assert!(framer.push(b"[<0;").is_empty());
        assert!(framer.flush_timeout().is_empty());
        for _ in 0..MAX_DISCARDED_CONTROL_TAIL_BYTES {
            assert!(framer.push(b"7").is_empty());
            assert!(
                framer.buffer.is_empty(),
                "drained bytes must not accumulate"
            );
            let discarded = framer.discarded_tail_bytes;
            assert!(framer.push(b"").is_empty());
            assert_eq!(framer.discarded_tail_bytes, discarded);
        }
        assert!(
            framer.discard_until.is_none(),
            "budget must end without a timeout"
        );
        assert_eq!(framer.push(b"7"), vec![b"7".to_vec()]);
    }

    #[test]
    fn m810_orphan_mouse_terminator_alone_ends_the_drain() {
        for tail in [b"43;26M".as_slice(), b"43;26m".as_slice()] {
            let mut framer = RawInputByteFramer::default();
            assert!(framer.push(b"\x1b").is_empty());
            assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
            assert!(framer.push(b"[<0;").is_empty());
    ...
    #[test]
    fn m810_orphan_mouse_invalid_continuation_preserves_new_escape() {
        let mut framer = RawInputByteFramer::default();
        assert!(framer.push(b"\x1b").is_empty());
        assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
        assert!(framer.push(b"[<0;").is_empty());
        assert!(framer.flush_timeout().is_empty());
        assert_eq!(
            framer.push(b"12x\x1b[A"),
            vec![b"x".to_vec(), b"\x1b[A".to_vec()]
        );
        assert!(framer.discard_until.is_none());
    ...
    #[test]
    fn m810_orphan_mouse_recognition_is_bounded_at_32_bytes() {
        for length in [32, 33] {
            let mut tail = b"[<".to_vec();
            tail.extend(vec![b'0'; length - b"[<;43;26M".len()]);
            tail.extend_from_slice(b";43;26M");
            assert_eq!(tail.len(), length);
            let mut framer = RawInputByteFramer::default();
            assert!(framer.push(b"\x1b").is_empty());
            assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
            let emitted = framer.push(&tail).concat();
            if length == 32 {
        ...
        assert!(framer.push(b"\x1b").is_empty());
        assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
        let mut incomplete = b"[<".to_vec();
        incomplete.extend([b'0'; 31]);
        assert_eq!(framer.push(&incomplete).concat(), incomplete);
        assert!(framer.buffer.is_empty());
    }

    #[test]
    fn m810_orphan_mouse_paste_payload_is_never_filtered() {
        let paste = b"\x1b[200~[<65;43;26M\x1b[201~";
        let mut framer = RawInputByteFramer::default();
        assert!(framer.push(b"\x1b").is_empty());
        assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
        assert!(framer.push(&paste[..15]).is_empty());
        assert!(framer.flush_timeout().is_empty());
        assert_eq!(framer.push(&paste[15..]), vec![paste.to_vec()]);
    }

    #[test]
    fn m810_orphan_mouse_marker_ends_when_ordinary_prefix_is_released() {
        let mut framer = RawInputByteFramer::default();
        assert!(framer.push(b"\x1b").is_empty());
        assert_eq!(framer.flush_timeout(), vec![b"\x1b".to_vec()]);
        let mut ordinary = framer.push(b"[");
        ordinary.extend(framer.flush_timeout());
        assert_eq!(ordinary.concat(), b"[");
        assert_eq!(framer.push(b"[<0;43;26M").concat(), b"[<0;43;26M");
    }
    ...
    #[test]
    fn legacy_reader_extends_incomplete_default_mouse_timeout() {
        let report = b"\x1b[MCN1";
        for split in 3..report.len() {
            let mut framer = RawInputFramer::default();
            assert!(framer.push(&report[..split]).is_empty());
            assert_eq!(
                input_flush_timeout_ms(&framer),
                MOUSE_ACTIVE_ESCAPE_SEQUENCE_FLUSH_TIMEOUT_MS
            );
            assert!(matches!(
                framer.push(&report[split..]).as_slice(),
