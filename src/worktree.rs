...
/// Run `git worktree remove`, recovering from a `force` removal that git refuses
/// with "is not a working tree" when only a leftover checkout directory remains
/// (the admin entry was already pruned). The directory is removed only when it
/// is no longer registered AND its `.git` gitdir still points inside this repo's
/// worktrees admin dir — an unrelated replacement directory is left untouched.
/// (upstream 46a2b25)
...
";
    ...
    fn default_checkout_path_appends_repo_and_branch_slug() {
        assert_eq!(
            default_checkout_path(
                Path::new("/home/me/.zynk/worktrees"),
                "zynk",
                "worktree/brave-river",
            ),
    ...
    fn dirty_remove_error_detection_matches_git_force_hint() {
        assert!(is_dirty_worktree_remove_error(
            "fatal: '/w/zynk' contains modified or untracked files, use --force to delete it"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/zynk' is a missing but already registered worktree"
        ));
        assert!(!is_dirty_worktree_remove_error(
            "fatal: '/w/zynk' contains a locked worktree, use --force only if you know why"
        ));
    }
