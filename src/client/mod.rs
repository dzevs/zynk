...
//!
//! The client:
//! - Connects to `zynk-client.sock`, sends Hello with terminal size and protocol version
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ClientError::ConnectionFailed(err) => {
                ...
                write!(f, "\nIs zynk server running? Start it with `zynk server`.")?;
            ...
            ClientError::ConnectionLost(err) => {
                ...
                {
                    write!(f, "lost connection to remote Zynk: {err}")?;
...
fn requested_render_encoding() -> RenderEncoding {
    match std::env::var("ZYNK_RENDER_ENCODING").ok().as_deref() {
...
fn run_client_with_mode(
    ...
    if let Some((terminal_id, takeover)) = attach_request {
        ...
        if let Err(err) = write_to_server(&mut stream, &attach) {
            eprintln!("zynk: failed to request terminal attach: {err}");
            std::process::exit(1);
        }
    }
    ...
    .map_err(|err| {
        eprintln!("zynk: failed to set up terminal: {err}");
        err
    })?;
...
fn window_title_osc(title: Option<&str>) -> Vec<u8> {
    let title = title.unwrap_or("zynk");
...
fn init_logging() {
    crate::logging::init_file_logging(crate::logging::LOG_FILE_CLIENT);
}
...
mod tests {
    ...
    #[test]
    fn kitty_graphics_image_id_parser_tracks_zynk_ids_only() {
    ...
    #[test]
    fn write_host_terminal_theme_query_includes_all_palette_entries() {
        let mut output = Vec::new();
        write_host_terminal_theme_query(&mut output).unwrap();
        let query = String::from_utf8(output).unwrap();
        assert!(query.starts_with(crate::terminal_theme::HOST_COLOR_QUERY_SEQUENCE));
    ...
    fn client_error_display_detached_default_session_reattach_hint() {
        ...
        let _env = EnvVarsRemovedGuard::new(&[
            ...
            crate::session::ZYNK_SESSION_ENV_VAR,
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("Run `zynk` to reattach"),
    ...
    fn client_error_display_detached_named_session_reattach_hint() {
        let _guard = env_lock().lock().unwrap();
        let _remote_env = EnvVarsRemovedGuard::new(&[
            crate::remote::REATTACH_COMMAND_ENV_VAR,
            crate::session::ZYNK_SESSION_ENV_VAR,
        ]);
    ...
    fn client_error_display_detached_remote_reattach_hint_takes_precedence() {
        ...
        let msg = err.to_string();
        assert!(
            msg.contains("Run `zynk --remote host --session work` to reattach"),
    ...
    fn client_error_display_connection_lost() {
        ...
        assert_eq!(msg, "lost connection to server: broken pipe");
    }
    ...
    fn client_error_display_remote_connection_lost_has_reattach_hint() {
        ...
        let err =
            ClientError::ConnectionLost(io::Error::new(io::ErrorKind::BrokenPipe, "broken pipe"));
        let msg = err.to_string();
        assert!(msg.contains("lost connection to remote Zynk"), "{msg}");
        assert!(msg.contains("panes may still be running"), "{msg}");
        assert!(
            msg.contains("Run `zynk --remote host --session work` to reattach"),
            "{msg}"
        );
    ...
    fn reload_local_client_config_refreshes_redraw_on_focus_gained() {
        ...
        let path = std::env::temp_dir().join(format!(
            "zynk-client-config-reload-{}-{}.toml",
            std::process::id(),
    ...
    #[test]
    fn window_title_osc_strips_terminators_and_defaults_to_zynk() {
        assert_eq!(
            window_title_osc(Some("zynk\x1b api\u{7}\u{9c}")),
            b"\x1b]0;zynk api\x07"
        );
        assert_eq!(window_title_osc(None), b"\x1b]0;zynk\x07");
    }
}
