...
use super::{
    input::{
        ...
        ghostty_prefers_zynk_text_encoding,
    },
...
impl PaneTerminal {
    ...
    pub fn modify_other_keys_enabled(&self) -> bool {
        self.ghostty.modify_other_keys_enabled()
    }
...
impl GhosttyPaneTerminal {
    ...
    pub fn process_pty_bytes(
        ...
        let render_delay =
            render_delay_after_pty_write(synchronized_output, has_kitty_graphics_sequence);
        if request_render {
    ...
    pub fn modify_other_keys_enabled(&self) -> bool {
        self.core
            .lock()
            .is_ok_and(|core| core.terminal.modify_other_keys_enabled().unwrap_or(false))
    }
    ...
    pub(crate) fn recent_text_snapshot(&self, lines: usize) -> TerminalReadSnapshot {
        self.core
            .lock()
            .ok()
            .and_then(|core| ghostty_recent_text_snapshot(&core.terminal, lines).ok())
            .unwrap_or_default()
    }
    ...
    pub(crate) fn recent_ansi_snapshot(&self, lines: usize) -> TerminalReadSnapshot {
        self.core
            .lock()
            .ok()
            .and_then(|core| ghostty_recent_ansi_snapshot(&core.terminal, lines, false).ok())
            .unwrap_or_default()
    }
    ...
    pub(crate) fn recent_unwrapped_text_snapshot(&self, lines: usize) -> TerminalReadSnapshot {
        self.core
            .lock()
            .ok()
            .and_then(|core| ghostty_recent_text_unwrapped_snapshot(&core.terminal, lines).ok())
            .unwrap_or_default()
    }
    ...
    pub(crate) fn recent_unwrapped_ansi_snapshot(&self, lines: usize) -> TerminalReadSnapshot {
        self.core
            .lock()
            .ok()
            .and_then(|core| ghostty_recent_ansi_snapshot(&core.terminal, lines, true).ok())
            .unwrap_or_default()
    }
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect, show_cursor: bool) {
        ...
        if show_cursor {
            if let Some(cursor) = current_cursor.filter(|cursor| cursor.visible) {
...
fn ghostty_recent_text(
    ...
    ghostty_recent_text_snapshot(terminal, lines).map(|snapshot| snapshot.text)
}
...
fn ghostty_recent_text_unwrapped(
    ...
    ghostty_recent_text_unwrapped_snapshot(terminal, lines).map(|snapshot| snapshot.text)
}
...
fn ghostty_recent_text_unwrapped_snapshot(
    ...
    Ok(finish_recent_snapshot(terminal, text, lines))
}
...
fn ghostty_recent_ansi(
    ...
    ghostty_recent_ansi_snapshot(terminal, lines, unwrap).map(|snapshot| snapshot.text)
}
...
mod tests {
    ...
    #[test]
    fn live_terminal_big_word_motions_expand_through_a_long_soft_wrap() {
        let (tx, _rx) = mpsc::channel(4);
        let mut terminal = crate::ghostty::Terminal::new(2, 3, 200).unwrap();
        let word = "a.".repeat(66);
        terminal.write(format!("{word} tail").as_bytes());
        let pane = PaneTerminal::new(GhosttyPaneTerminal::new(terminal, tx).unwrap());
        let text_match = pane.search_text_matches(&word, true)[0];
        let tail = pane.search_text_matches("tail", true)[0];
        assert!(text_match.end.row - text_match.start.row >= 64);
    ...
    fn process_pty_bytes_reports_latest_libghostty_pwd_callback() {
        ...
        let partial = pane.process_pty_bytes(pane_id, 0, b"\x1b]7;file:///tmp/zynk%20", &tx);
    ...
    #[test]
    fn cursor_state_reports_live_cursor_position_without_render_delay() {
        let (tx, _rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx.clone()).unwrap();
        let pane_id = PaneId::from_raw(1);
    ...
    #[test]
    fn render_delay_only_defers_for_kitty_graphics_outside_synchronized_output() {
        assert_eq!(render_delay_after_pty_write(false, false), None);
        assert_eq!(
            render_delay_after_pty_write(false, true),
            Some(KITTY_GRAPHICS_REDRAW_SETTLE)
        );
        assert_eq!(render_delay_after_pty_write(true, false), None);
        assert_eq!(render_delay_after_pty_write(true, true), None);
    }
    ...
    #[test]
    fn ghostty_char_keys_still_use_zynk_encoding() {
        let (tx, _rx) = mpsc::channel(4);
        let mut terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
    ...
    fn ghostty_normalize_buffer_symbol_prefers_grapheme_width_when_metadata_disagrees() {
        ...
        assert_eq!(
            ghostty_normalize_buffer_symbol("ｦ\u{ff9e}", crate::ghostty::CellWide::Wide),
            "ｦ\u{ff9e}"
        );
        assert_eq!(
            ghostty_normalize_buffer_symbol("ﾝ\u{ff9f}", crate::ghostty::CellWide::Wide),
            "ﾝ\u{ff9f}"
        );
    }

    #[test]
    fn halfwidth_katakana_voiced_grapheme_exception_has_exact_boundaries() {
        ...
        for symbol in ["", "ﾝ", "･\u{ff9e}", "ﾞ\u{ff9e}", "ﾝx", "ﾝ\u{ff9e}x"] {
            assert!(
                ...
                "{symbol:?} must not use the Ghostty width exception"
            );
        }
    }
    ...
    fn empty_or_short_resize_keeps_following_bottom_when_output_creates_scrollback() {
        for initial in [b"".as_slice(), b"seed\r\n".as_slice()] {
            for (rows, cols) in [(3, 10), (2, 9), (4, 12)] {
                let (tx, _rx) = mpsc::channel(4);
                ...
                assert_eq!(metrics.offset_from_bottom, 0, "resize to {rows}x{cols}");
    ...
    #[test]
    fn scrolled_resize_keeps_user_viewport_when_more_output_arrives() {
        let (tx, _rx) = mpsc::channel(4);
        let mut terminal = crate::ghostty::Terminal::new(10, 3, 10_000).unwrap();
        write_numbered_lines(&mut terminal, 20);
        let pane = GhosttyPaneTerminal::new(terminal, tx.clone()).unwrap();
        pane.set_scroll_offset_from_bottom(10);
        pane.resize(4, 10, 0, 0);
        let before = pane.visible_text();
        assert!(pane.scroll_metrics().unwrap().offset_from_bottom > 0);

        pane.process_pty_bytes(PaneId::from_raw(1), 0, b"\r\nLATEST", &tx);

        assert_eq!(pane.visible_text(), before);
        assert!(pane.scroll_metrics().unwrap().offset_from_bottom > 0);
        assert!(pane.detection_text().contains("LATEST"));
        assert!(!pane.visible_text().contains("LATEST"));
    }
