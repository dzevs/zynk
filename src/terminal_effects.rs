...
mod tests {
    ...
    #[test]
    fn window_title_strips_terminators_and_defaults_to_zynk() {
        let mut output = Vec::new();
        write_window_title(&mut output, Some("zynk\x1b api\u{7}\u{9c}")).unwrap();
        assert_eq!(output, b"\x1b]0;zynk api\x07");

        output.clear();
        write_window_title(&mut output, None).unwrap();
        assert_eq!(output, b"\x1b]0;zynk\x07");
    }
}
