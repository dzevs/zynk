...
/// Zynk-branded session-name override (ADR 0007 §5): the primary, documented name.
pub const ZYNK_SESSION_ENV_VAR: &str = "ZYNK_SESSION";
/// Transitional `ZYNK_*` compat alias for the session-name override. Kept working;
/// `ZYNK_SESSION` wins when both are set.
pub const SESSION_ENV_VAR: &str = "ZYNK_SESSION";
...
pub fn configure_from_args(args: &[String]) -> Result<Vec<String>, String> {
        ...
        && args.get(2).map(String::as_str) == Some("attach")
    {
        ...
        let Some(name) = args.get(3) else {
            return Err("usage: zynk session attach <name>".to_string());
        };
        if args.len() != 4 {
            return Err("usage: zynk session attach <name>".to_string());
        }
    ...
    } else if crate::api::socket_path_override_present() {
        EXPLICIT_SESSION_REQUESTED.store(false, Ordering::Relaxed);
    } else if let Some(session) = session_override() {
        // ADR 0007 §5: ZYNK_SESSION is primary, ZYNK_SESSION is compat. Normalize
        // an inherited invalid/default name by clearing whichever carrier holds it.
        if normalize_name(&session)?.is_none() {
            std::env::remove_var(ZYNK_SESSION_ENV_VAR);
...
/// Resolve the session-name override, preferring the Zynk-branded `ZYNK_SESSION`
/// over the retained `ZYNK_SESSION` compat alias (ADR 0007 §5).
pub fn session_override() -> Option<String> {
    crate::config::env_first(&[ZYNK_SESSION_ENV_VAR])
}
...
pub fn local_attach_command() -> String {
    match active_name() {
        Some(name) => format!("zynk session attach {name}"),
        None => "zynk".to_string(),
    }
}
...
pub fn stop_command_for(name: Option<&str>) -> String {
    match name {
        Some(name) => format!("zynk session stop {name}"),
        None => "zynk server stop".to_string(),
    }
}
...
pub fn restart_after_update_guidance(stop_command: &str, attach_command: Option<&str>) -> String {
    let restart = match attach_command {
        ...
        None => format!("Run `{stop_command}`, then restart Zynk with the same socket override."),
    };
    format!(
...
pub fn active_restart_after_update_guidance() -> String {
    if !explicit_session_requested() {
        // ADR 0007 §5: name the override var actually in effect (ZYNK primary).
        let var = if std::env::var_os(crate::api::ZYNK_SOCKET_PATH_ENV_VAR).is_some() {
            crate::api::ZYNK_SOCKET_PATH_ENV_VAR
        } else {
            crate::api::SOCKET_PATH_ENV_VAR
        };
        if let Some(socket_path) = crate::api::socket_path_override() {
            return restart_after_update_guidance(
                &format!("{var}={socket_path} zynk server stop"),
                None,
            );
        }
    }
...
pub(crate) fn clear_explicit_session_for_test() {
    EXPLICIT_SESSION_REQUESTED.store(false, Ordering::Relaxed);
    // ADR 0007 §5: `apply_explicit_name` now mirrors the session onto both the
    // Zynk-branded primary and the `ZYNK_SESSION` compat carrier. Clear both so
    // serialized env-lock tests do not leak a stale `ZYNK_SESSION` into the next.
    std::env::remove_var(ZYNK_SESSION_ENV_VAR);
}
...
pub fn api_socket_path_for(name: Option<&str>) -> PathBuf {
    // ADR 0007 §5: Zynk-branded runtime socket basename (created fresh at runtime).
    data_dir_for(name).join("zynk.sock")
}
...
pub fn active_api_socket_path() -> PathBuf {
    ...
    // ADR 0007 §5: ZYNK_SOCKET_PATH primary, ZYNK_SOCKET_PATH compat (ZYNK wins).
    if let Some(path) = crate::api::socket_path_override() {
...
pub fn client_socket_path_for(name: Option<&str>) -> PathBuf {
    // ADR 0007 §5: Zynk-branded runtime client-socket basename.
    data_dir_for(name).join("zynk-client.sock")
}
...
fn apply_explicit_name(name: &str) -> Result<(), String> {
    ...
    if let Some(session) = session {
        // ADR 0007 §5: set the Zynk-branded carrier as primary and mirror to the
        // `ZYNK_SESSION` compat alias so both in-repo (ZYNK-first) and legacy
        // (ZYNK-only) children inherit the active session.
        std::env::set_var(ZYNK_SESSION_ENV_VAR, &session);
...
mod tests {
    ...
    fn configure_from_args_removes_global_session_option() {
        ...
        assert_eq!(cleaned, vec!["zynk", "workspace", "list"]);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
    }
    ...
    fn configure_from_args_accepts_equals_form() {
        ...
        assert!(explicit_session_requested());
        assert_eq!(cleaned, vec!["zynk", "server", "stop"]);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
    }
    ...
    fn configure_from_args_maps_default_session_name_to_default_path() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("zynk-session-default-{}", std::process::id()));
        std::env::set_var("XDG_CONFIG_HOME", &config_home);
    ...
    fn env_default_session_name_uses_default_path() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("zynk-env-session-default-{}", std::process::id()));
        std::env::set_var("XDG_CONFIG_HOME", &config_home);
        std::env::remove_var(crate::api::SOCKET_PATH_ENV_VAR);
        std::env::remove_var(ZYNK_SESSION_ENV_VAR);
    ...
    fn local_attach_command_uses_default_launch_for_default_session() {
        ...
        assert_eq!(local_attach_command(), "zynk");
    }
    ...
    fn local_attach_command_uses_session_attach_for_named_session() {
        ...
        assert_eq!(local_attach_command(), "zynk session attach work");
    ...
    fn local_stop_command_uses_server_stop_for_default_session() {
        ...
        assert_eq!(local_stop_command(), "zynk server stop");
    ...
    fn local_stop_command_uses_session_stop_for_named_session() {
        ...
        assert_eq!(local_stop_command(), "zynk session stop work");
    ...
    fn restart_after_update_guidance_names_stop_and_attach_commands() {
        assert_eq!(
            restart_after_update_guidance(
                "zynk session stop work",
                Some("zynk session attach work")
            ),
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `zynk session stop work`, then run `zynk session attach work` again."
        );
    }
    ...
    fn active_restart_after_update_guidance_respects_socket_override() {
        ...
        std::env::set_var(crate::api::SOCKET_PATH_ENV_VAR, "/tmp/custom-zynk.sock");
        std::env::remove_var(ZYNK_SESSION_ENV_VAR);
        std::env::remove_var(SESSION_ENV_VAR);
        clear_explicit_session_for_test();
        ...
        assert_eq!(
            ...
            "Stop the old server to use the new version.\nStopping exits pane processes.\nRun `ZYNK_SOCKET_PATH=/tmp/custom-zynk.sock zynk server stop`, then restart Zynk with the same socket override."
        );
    ...
    fn explicit_session_socket_ignores_inherited_socket_override() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("zynk-session-precedence-{}", std::process::id()));
        std::env::set_var("XDG_CONFIG_HOME", &config_home);
        std::env::remove_var(ZYNK_SESSION_ENV_VAR);
        std::env::set_var(SESSION_ENV_VAR, "work");
        EXPLICIT_SESSION_REQUESTED.store(true, Ordering::Relaxed);
    ...
    fn env_socket_override_skips_invalid_env_session_validation_without_explicit_session() {
        let _guard = env_lock().lock().unwrap();
        // Clear any explicit session BEFORE setting the env session: post-rebrand the
        // explicit-session clear removes `ZYNK_SESSION`, which is the same var we set here.
        clear_explicit_session_for_test();
        std::env::set_var(ZYNK_SESSION_ENV_VAR, "bad/name");
        std::env::set_var(crate::api::SOCKET_PATH_ENV_VAR, "/tmp/zynk.sock");
        let args = vec![
            "zynk".to_string(),
            "workspace".to_string(),
            "list".to_string(),
        ...
        assert_eq!(cleaned, vec!["zynk", "workspace", "list"]);
        assert!(!explicit_session_requested());
        assert_eq!(active_api_socket_path(), PathBuf::from("/tmp/zynk.sock"));
        assert_eq!(
            std::env::var(ZYNK_SESSION_ENV_VAR).as_deref(),
            Ok("bad/name")
        );
    ...
    fn list_sessions_skips_reserved_default_directory() {
        let _guard = env_lock().lock().unwrap();
        let config_home =
            std::env::temp_dir().join(format!("zynk-session-list-{}", std::process::id()));
