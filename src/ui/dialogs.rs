...
// Sized to fit the branch/checkout fields plus a multi-line error row so a
// `fatal:` git stderr line stays visible (upstream 46a2b25 widened the popup +
// wrapped the error; zynk's overlay carries extra label rows, so it needs more
// height than upstream's 12 to leave room for the 3-line wrapped error).
...
pub(super) fn render_remove_worktree_overlay(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    frame.render_widget(
        Paragraph::new(" The branch is not deleted. The Zynk workspace will close.")
            .style(Style::default().fg(app.palette.overlay0)),
        rows[3],
    );
...
mod tests {
    ...
    fn worktree_overlay_caret(branch: &str) -> Position {
        ...
        app.worktree_create = Some(WorktreeCreateState {
            ...
            repo_name: "zynk".into(),
            branch: branch.into(),
            checkout_path: "/repo/.worktrees/zynk/foo".into(),
            error: None,
            creating: false,
        });
    ...
    fn rename_overlay_caret_stays_inside_the_input_when_the_name_overflows() {
        ...
        // The field is 54 columns wide. From 52 characters onward the caret
        // occupies the final blank cell, clamping whenever the name overflows.
        assert_eq!(
            rename_overlay_caret(&"a".repeat(51)),
            Position::new(input.x + 52, input.y)
        );
        assert_eq!(
            rename_overlay_caret(&"a".repeat(52)),
    ...
    #[test]
    fn name_input_caret_fits_small_fields_and_combining_text() {
        let mut app = AppState::test_new();
        app.name_input = "あx".into();
        for width in 0..=4 {
            let input = Rect::new(2, 1, width, 1);
            let mut terminal = Terminal::new(TestBackend::new(8, 4)).unwrap();
            let previous_cursor = terminal.get_cursor_position().unwrap();
            terminal
                .draw(|frame| super::render_name_input_field(&app, frame, input))
                .unwrap();
            let caret = terminal.get_cursor_position().unwrap();
            if width == 0 {
                assert_eq!(caret, previous_cursor);
            } else {
                assert_eq!(caret, Position::new(input.right() - 1, input.y));
                assert_eq!(
                    terminal.backend().buffer()[(caret.x, caret.y)].symbol(),
                    " "
                );
            }
        }
        let input = rename_input_rect(RENAME_AREA);
        assert_eq!(
            rename_overlay_caret("a\u{0301}"),
            Position::new(input.x + 2, input.y)
        );
    }

    #[test]
    fn creation_dialogs_keep_a_real_input_caret() {
        let input = rename_input_rect(RENAME_AREA);
        for mode in [Mode::RenameWorkspace, Mode::RenameTab] {
            let mut app = AppState::test_new();
            app.mode = mode;
            ...
            assert!(app.pending_workspace_create_cwd.is_some());
            assert!(app.creating_new_tab);
        }
    }
    ...
    async fn confirm_close_text_prefers_live_runtime_cwd_over_stale_terminal_cwd() {
        let root = std::env::var_os("ZYNK_TEST_ROOT")
            ...
            .join(format!(
                "zynk-confirm-close-runtime-cwd-{}",
                std::process::id()
            ));
    ...
    fn confirm_close_text_uses_selected_custom_name_instead_of_active_workspace_cwd() {
        ...
        let root = selected.tabs[0].root_pane;
        let terminal_id = selected.tabs[0].panes[&root].attached_terminal_id.clone();
    ...
    // upstream 46a2b25: the create-worktree error row wraps so a `fatal:` git
    // stderr line stays visible instead of being clipped to a single line.
    #[test]
