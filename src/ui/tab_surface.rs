...
mod tests {
    use super::{
        compute_tab_surface, render_tab_surface, tab_surface_cursor, tab_surface_hyperlinks,
        TabSurfaceView,
    };
    use crate::app::{AppState, Mode};
    use crate::terminal::TerminalRuntimeRegistry;
    ...
    #[tokio::test]
    async fn repeated_render_consumes_stored_surface_without_recomputing_geometry() {
        let mut app = full_app_characterization_state("https://example.com/stored");
        let area = Rect::new(0, 0, 106, 20);
        crate::ui::compute_view(&mut app, area);
        let pane_snapshot = |app: &AppState| {
            app.view
                .pane_infos
                .iter()
                .map(|info| {
        ...
        let split_snapshot = |app: &AppState| {
            app.view
                .split_borders
                .iter()
                .map(|split| {
        ...
        let panes = pane_snapshot(&app);
        let splits = split_snapshot(&app);
        assert_eq!(panes.len(), 2);
        // Change a compute input after capture: draw must still consume both stored panes.
        app.workspaces[0].zoomed = true;
        let surface = app.view.tab_surface();
        assert!(std::ptr::eq(
            surface.pane_infos,
            app.view.pane_infos.as_slice()
        ));
        assert!(std::ptr::eq(
            surface.split_borders,
            app.view.split_borders.as_slice()
        ));
        ...
        let text = first
            ...
            .collect::<String>();
        assert!(
            text.contains("LINK") && text.contains("RIGHT"),
            "cached panes: {text:?}"
        );
        assert_eq!(panes, pane_snapshot(&app));
        assert_eq!(splits, split_snapshot(&app));
    }
    ...
    async fn mobile_full_app_semantic_frame_is_characterized() {
        ...
        assert_eq!(
            frame_digest(&frame),
            "b4f80be976f5895fcc2920b0b55c5a05020a86b11d3e0cabfbb091a166f0f667"
        );
    }
}
