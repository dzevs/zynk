...
pub(super) fn resize_popup_pane(
    ...
    let Some((_, inner)) = popup_pane_rects(app, area) else {
        return;
    };
...
pub(super) fn render_popup_pane(
    ...
    rt.render(
        frame,
        inner,
        !app.view.popup_cursor_suppressed
            && !pane_is_scrolled_back(rt)
            && !rt.synchronized_output_active(),
    );
}
...
pub(super) fn render_panes(
    ...
    for info in pane_infos {
        if let Some(rt) = app.runtime_for_pane_in_workspace(terminal_runtimes, ws_idx, info.id) {
            let show_cursor = app.popup_pane.is_none()
                && info.is_focused
                && terminal_active
                && !pane_is_scrolled_back(rt)
...
mod tests {
    ...
    #[test]
    fn disabled_outer_borders_keep_vertical_dividers_at_nonzero_origin() {
        let mut workspace = Workspace::test_new("test");
        let root = workspace.tabs[0].root_pane;
        let bottom = workspace.test_split(ratatui::layout::Direction::Vertical);
        let infos = apply_pane_chrome(
            workspace.tabs[0].layout.panes(Rect::new(7, 3, 40, 16)),
            true,
            false,
            false,
        );
        assert_eq!(
            infos.iter().find(|info| info.id == root).unwrap().borders,
            Borders::NONE
        );
        assert_eq!(
            infos.iter().find(|info| info.id == bottom).unwrap().borders,
            Borders::TOP
        );
        assert!(apply_pane_chrome(Vec::new(), true, false, false).is_empty());
    }

    #[tokio::test]
    async fn pane_options_resize_background_and_zoomed_panes_consistently() {
        let mut app = AppState::test_new();
        let mut workspace = Workspace::test_new("test");
        let root = workspace.tabs[0].root_pane;
        let right = workspace.test_split(ratatui::layout::Direction::Horizontal);
        ...
        for pane in [root, right] {
            workspace.tabs[0].runtimes.insert(
                pane,
                TerminalRuntime::test_with_screen_bytes(40, 8, b"ready"),
            );
        }
        ...
        for outer in [true, false] {
            for scrollbars in [true, false] {
                for zoomed in [true, false] {
                    app.pane_outer_borders = outer;
                    app.pane_scrollbars = scrollbars;
                    app.workspaces[0].tabs[0].zoomed = zoomed;
                    let expected_height = area.height - 2 * u16::from(outer);
                    let expected_width = if zoomed {
                        area.width - 2 * u16::from(outer)
                    } else {
                        area.width / 2 - u16::from(outer)
                    } - u16::from(scrollbars);
                    resize_tab_panes(&app, &runtimes, &app.workspaces[0].tabs[0], area, cell);
                    assert_eq!(
                        app.workspaces[0].tabs[0].runtimes[&root].current_size(),
                        (expected_height, expected_width)
                    );
                    let infos = compute_pane_infos(&app, &runtimes, area, false, cell);
                    let info = infos.iter().find(|info| info.id == root).unwrap();
                    assert_eq!(
    ...
    #[tokio::test]
    async fn background_alternate_screen_resize_reclaims_the_gutter() {
        let mut app = AppState::test_new();
        let mut workspace = Workspace::test_new("background");
        let root = workspace.tabs[0].root_pane;
        workspace.tabs[0].runtimes.insert(
            root,
            TerminalRuntime::test_with_screen_bytes(40, 8, b"ready"),
        ...
        for (sequence, width) in [(b"\x1b[?1049h", 40), (b"\x1b[?1049l", 39)] {
            tab.runtimes[&root].test_process_pty_bytes(sequence);
            resize_tab_panes(
                &app,
                &runtimes,
                tab,
            ...
            assert_eq!(tab.runtimes[&root].current_size(), (8, width));
        }
    }
