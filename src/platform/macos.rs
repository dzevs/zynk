...
mod tests {
    ...
    fn procargs2_argv_excludes_environment_entries() {
        ...
        assert_eq!(argv, vec!["node", "/Users/user/.local/bin/pi"]);
        assert_eq!(argv.join(" "), "node /Users/user/.local/bin/pi");
    ...
    fn desktop_notification_falls_back_to_osascript_when_terminal_notifier_fails() {
        let path = std::env::temp_dir().join(format!("zynk-osascript-args-{}", std::process::id()));
        let script = r#"
