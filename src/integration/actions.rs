    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Opencode => {
            ...
            if result.updated_tui_config {
                messages.push(format!(
                    "removed zynk opencode plugin entry from {}",
        ...
        crate::api::schema::IntegrationTarget::Qwen => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed zynk qwen hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no zynk qwen hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::Mastracode => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed zynk mastracode hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no zynk mastracode hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages
        ...
        crate::api::schema::IntegrationTarget::AntigravityCli => {
            ...
            if result.updated_hooks {
                messages.push(format!(
                    "removed zynk antigravity-cli hook entries from {}",
                    result.hooks_path.display()
                ));
            } else {
                messages.push(format!(
                    "no zynk antigravity-cli hook entries found in {}",
                    result.hooks_path.display()
                ));
            }
            messages
