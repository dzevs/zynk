...
fn install_qwen_writes_session_hook_and_preserves_settings() {
    ...
    // Qwen Code counts this timeout in MILLISECONDS, unlike the qodercli hooks.
    ...
    assert_eq!(hook_asset, QWEN_HOOK_ASSET);
    assert!(hook_asset.contains("ZYNK_INTEGRATION_ID=qwen"));
    assert!(hook_asset.contains("ZYNK_INTEGRATION_VERSION=1"));
    assert!(hook_asset.contains("zynk:qwen"));
    ...
    assert_eq!(
        settings["hooks"]["SessionStart"].as_array().unwrap().len(),
        1,
        "reinstalling must not duplicate the SessionStart entry"
    );
...
#[test]
fn uninstall_qwen_removes_zynk_hooks_and_preserves_others() {
    let _lock = integration_env_lock();
    let base = unique_base();
    let qwen_dir = base.join(".qwen");
    fs::create_dir_all(&qwen_dir).unwrap();
...
fn install_qwen_errors_when_config_dir_missing() {
    ...
    assert!(
        err.contains("qwen code config directory not found"),
        "unexpected error: {err}"
    );
