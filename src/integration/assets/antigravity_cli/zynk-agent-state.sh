...
[ "${ZYNK_ENV:-}" = "1" ] || emit_and_exit
[ -n "${ZYNK_SOCKET_PATH:-}" ] || emit_and_exit
[ -n "${ZYNK_PANE_ID:-}" ] || emit_and_exit
...
seq = time.time_ns()
params = {
    "pane_id": os.environ["ZYNK_PANE_ID"],
    "source": "zynk:antigravity_cli",
    "agent": "agy",
    "seq": seq,
...
try:
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
        ...
        client.connect(os.environ["ZYNK_SOCKET_PATH"])
