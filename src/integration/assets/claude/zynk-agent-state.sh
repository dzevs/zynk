...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/zynk-claude-hook.XXXXXX")" || exit 0
...
source = "zynk:claude"
action = os.environ.get("ZYNK_ACTION", "")
...
if hook_event_name == "SubagentStop":
    # SubagentStop is a completion event. Older integrations mapped it
