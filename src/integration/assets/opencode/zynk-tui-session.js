...
function requestOnce(sessionID) {
  const paneId = process.env.ZYNK_PANE_ID;
  const socketPath = process.env.ZYNK_SOCKET_PATH;
...
export default {
  id: "zynk.opencode.session-selection",
  tui: async (api) => {
    if (
      process.env.ZYNK_ENV !== "1" ||
      !process.env.ZYNK_SOCKET_PATH ||
      !process.env.ZYNK_PANE_ID
    ) {
      return;
    }
