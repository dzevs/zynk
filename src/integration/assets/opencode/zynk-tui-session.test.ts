...
beforeEach(() => {
  ...
  process.env.ZYNK_SOCKET_PATH = "test.sock";
...
async function loadPlugin() {
  importCounter += 1;
  const module = await import(`./zynk-tui-session.js?test=${importCounter}`);
...
test("retries an initial selection while zynk detects the process", async () => {
  const plugin = await loadPlugin();
  const tui = fakeApi();
  tui.addSession({ id: "session-a" });
  tui.select("session-a");
