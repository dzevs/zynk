...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/zynk-devin-hook.XXXXXX")" || exit 0
...
def load_session_list(project_dir: str | None):
    injected = os.environ.get("ZYNK_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("ZYNK_HOOK_INPUT_FILE"))
