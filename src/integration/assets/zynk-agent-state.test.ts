...
// Shared bun harness for the pi and omp extension assets: it drives each module
// against a real Unix socket and asserts the reports that reach it. The pi-specific
// state-only guards live beside the asset in `pi/zynk-agent-state.test.ts`.
...
const originalEnvironment = {
  ZYNK_ENV: process.env.ZYNK_ENV,
  ZYNK_OMP_IDLE_DEBOUNCE_MS: process.env.ZYNK_OMP_IDLE_DEBOUNCE_MS,
  ZYNK_PANE_ID: process.env.ZYNK_PANE_ID,
  ZYNK_SOCKET_PATH: process.env.ZYNK_SOCKET_PATH,
};
...
const integrations = [
  { name: "Pi", modulePath: "./pi/zynk-agent-state.ts" },
  { name: "Oh My Pi", modulePath: "./omp/zynk-agent-state.ts" },
] as const;
...
function configureIntegrationEnvironment(recordingSocketPath: string) {
  ...
  process.env.ZYNK_SOCKET_PATH = recordingSocketPath;
  process.env.ZYNK_PANE_ID = "test:p1";
}

async function startRecordingServer(name: string): Promise<unknown[]> {
  const recordingSocketPath = join(tmpdir(), `zynk-${name}-${process.pid}.sock`);
...
test("Pi settlement preserves explicit blocked-state precedence", async () => {
  ...
  handlers.get("agent_start")?.({}, context);
  await waitFor(() => requestStates(requests).length === 2);
  eventHandlers.get("zynk:blocked")?.({ active: true, label: "approval" }, context);
  await waitFor(() => requestStates(requests).length === 3);
  ...
  eventHandlers.get("zynk:blocked")?.({ active: false }, context);
...
test("Pi waits for a replacement session report before publishing state", async () => {
  const recordingSocketPath = join(tmpdir(), `zynk-pi-session-order-${process.pid}.sock`);
  socketPath = recordingSocketPath;
  await rm(recordingSocketPath, { force: true });
...
test("Oh My Pi retries working before a queued idle state", async () => {
  ...
  process.env.ZYNK_OMP_IDLE_DEBOUNCE_MS = "0";
  const { handlers, pi } = createExtensionHarness();

  const { default: install } = await importFresh("./omp/zynk-agent-state.ts");
  install(pi);
