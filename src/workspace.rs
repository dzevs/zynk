...
impl Workspace {
    ...
    /// Like [`create_tab`](Self::create_tab) but injects caller-supplied
    /// `extra_env` into the new tab's root pane (used by `layout.apply`).
    #[allow(clippy::too_many_arguments)]
    pub fn create_tab_with_env(
        &mut self,
        rows: u16,
        cols: u16,
        cwd: PathBuf,
    ...
    /// Like [`create_tab`](Self::create_tab) but launches `argv` as the root
    /// pane command and injects caller-supplied `extra_env` (used by
    /// `layout.apply`).
    #[allow(clippy::too_many_arguments)]
    ...
    /// Build the [`PaneLaunchEnv`] for a newly-spawned pane: caller `extra_env`
    /// plus this workspace's identity triple (public workspace/tab/pane ids) so
    /// the spawned pane receives `ZYNK_WORKSPACE_ID`/`ZYNK_TAB_ID`/`ZYNK_PANE_ID`.
    ...
    pub fn split_focused_command(
        ...
        let tab_number = self
            ...
            .map(|tab| tab.number)
            .expect("workspace must always have at least one tab");
        let launch_env = self.launch_env_for_new_pane(tab_number, pane_number, extra_env.to_vec());
        let new_pane = self
            .active_tab_mut()
            .expect("workspace must always have at least one tab")
    ...
    /// Like [`split_pane_with_ratio`](Self::split_pane_with_ratio) but injects
    /// caller-supplied `extra_env` into the new pane (used by `layout.apply`).
    #[allow(clippy::too_many_arguments)]
    pub fn split_pane_with_ratio_env(
        &mut self,
        pane_id: PaneId,
        direction: Direction,
        ratio: f32,
    ...
    /// Like [`split_pane_argv_command`](Self::split_pane_argv_command) but with
    /// an explicit `ratio` and caller-supplied `extra_env` (used by
    /// `layout.apply`).
    #[allow(clippy::too_many_arguments)]
...
mod tests {
    ...
    #[tokio::test]
    async fn failed_runtime_split_preserves_layout_and_close_focus_history() {
        for focus_new in [false, true] {
            let mut ws = Workspace::test_new("failed-split");
            let first = ws.tabs[0].root_pane;
            ws.test_split(Direction::Horizontal);
            let third = ws.test_split(Direction::Vertical);
            ws.tabs[0].layout.focus_pane(first);
            ws.tabs[0].layout.focus_pane(third);
            ...
            assert!(
                result.is_err(),
                "non-executable device must refuse the runtime spawn"
            );
            assert_eq!(ws.tabs[0].layout.pane_ids(), ids);
            assert_eq!(ws.tabs[0].layout.focused(), third);
            ws.assert_invariants_for_test();
            assert!(!ws.close_pane(third));
            assert_eq!(ws.tabs[0].layout.focused(), first);
        }
    }

    #[tokio::test]
    async fn targeted_runtime_split_preserves_focus_intent_and_history() {
        for focus_new in [false, true] {
            let mut ws = Workspace::test_new("targeted-split");
            let first = ws.tabs[0].root_pane;
            ws.test_split(Direction::Horizontal);
            let third = ws.test_split(Direction::Vertical);
            ws.tabs[0].layout.focus_pane(first);
            ...
            let (_, created) = ws
                ...
                .expect("target exists")
                .expect("runtime spawn succeeds");
            ws.assert_invariants_for_test();
            let focus = if focus_new { created.pane_id } else { third };
            assert_eq!(ws.tabs[0].layout.focused(), focus);
            assert!(!ws.close_pane(focus));
            assert_eq!(
                ws.tabs[0].layout.focused(),
                if focus_new { third } else { first }
            );
        }
    }

    #[test]
    fn background_detach_and_move_preserve_close_focus_origin() {
        for moving in [false, true] {
            let mut ws = Workspace::test_new("background");
            let first = ws.tabs[0].root_pane;
            let second = ws.test_split(Direction::Horizontal);
            ws.test_split(Direction::Vertical);
            let fourth = ws.test_split(Direction::Horizontal);
            ws.tabs[0].layout.focus_pane(second);
            ws.tabs[0].layout.focus_pane(fourth);
            if moving {
                assert!(ws.take_pane_for_move(first).is_some());
            } else {
                assert!(!ws.close_pane(first));
            }
            assert_eq!(ws.tabs[0].layout.focused(), fourth);
            assert!(!ws.close_pane(fourth));
            assert_eq!(ws.tabs[0].layout.focused(), second);
        }
    }
    ...
    async fn new_workspace_retains_discovered_git_metadata() {
        ...
        let root = std::env::temp_dir().join(format!(
            "zynk-workspace-git-metadata-{}-{stamp}",
            std::process::id()
        ));
    ...
    fn display_name_reads_cached_identity_without_rechecking_filesystem() {
        let stamp = std::time::SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "zynk-workspace-label-cache-{}-{stamp}",
            std::process::id()
        ));
    ...
    #[test]
    fn discovered_workspace_label_matches_linked_checkout_identity() {
        let stamp = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let base =
            std::env::temp_dir().join(format!("zynk-linked-label-{}-{stamp}", std::process::id()));
        let common = base.join("main/.git");
        let linked = base.join("feature");
        let cwd = linked.join("nested");
        let admin = common.join("worktrees/feature");
        ...
        let (space, cached_label, _) = discover_workspace_git_identity(&cwd);
        let previous_label = derive_label_from_cwd(&cwd);
        std::fs::remove_dir_all(base).unwrap();

        assert!(space.unwrap().is_linked_worktree);
        assert_eq!(previous_label, "feature");
        assert_eq!(cached_label, previous_label);
    }
    ...
    fn workspace_identity_follows_first_tab_root_pane_cwd() {
        ...
        terminals.insert(
            terminal_id.clone(),
            TerminalState::new(terminal_id, PathBuf::from("/zynk-test/pion")),
        );
        ...
        assert_eq!(
            ...
            Some(PathBuf::from("/zynk-test/pion"))
        );
    }
