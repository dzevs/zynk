...
pub(super) fn wait_for_agent(
    ...
    let initial_sequence = event_hub.current_sequence();
    let initial = match agent_get(&request_id, &params.target, api_tx) {
        Ok(agent) => agent,
        Err(response) => return encode_agent_response(response).map(Some),
    };
...
pub(super) fn prompt_agent(
    ...
    let Ok((prompted, prompt_state_change_seq)) =
        prompted_agent_from_response(&request_id, &prompt_response)
    else {
    ...
    if initial.agent_status != crate::api::schema::AgentStatus::Working {
        ...
        after_state_change_seq = None;
        if agent_wait_matches(&initial, &until, None) {
            return agent_prompt_success(request_id, initial, prompt_state_change_seq).map(Some);
        }
    }
    ...
    match outcome {
        AgentWaitOutcome::Matched(agent) => {
            agent_prompt_success(request_id, *agent, prompt_state_change_seq).map(Some)
        }
        AgentWaitOutcome::Response(response) => Ok(Some(response)),
    }
}
...
fn wait_for_resolved_agent(
    ...
    let mut last_event_sequence = wait.initial_sequence;
        ...
        for (sequence, event) in event_hub.events_after(last_event_sequence) {
            ...
            match event.data {
                EventData::PaneAgentDetected {
                    ...
                    if released {
                        if let Some(status) =
                            matching_release_status(&wait.until, final_status, matched_event_status)
                        {
...
fn matching_release_status(
    until: &[crate::api::schema::AgentStatus],
    final_status: Option<crate::api::schema::AgentStatus>,
    matched_event_status: Option<crate::api::schema::AgentStatus>,
...
fn agent_from_response(
    ...
    let value = decode_agent_response(request_id, response)?;
...
fn prompted_agent_from_response(
    request_id: &str,
    response: &str,
) -> Result<(crate::api::schema::AgentInfo, u64), ErrorResponse> {
    let value = decode_agent_response(request_id, response)?;
    let agent =
        serde_json::from_value(value["result"]["agent"].clone()).map_err(|_| ErrorResponse {
            id: request_id.into(),
            error: ErrorBody {
                code: "internal_error".into(),
                message: "failed to decode prompted agent".into(),
            },
        })?;
    let baseline = value["result"]["baseline_state_change_seq"]
        .as_u64()
        .ok_or_else(|| ErrorResponse {
            id: request_id.into(),
            error: ErrorBody {
                code: "internal_error".into(),
                message: "failed to decode prompt baseline".into(),
            },
        })?;
    Ok((agent, baseline))
}
...
fn agent_wait_timeout(
    ...
    encode_agent_response(ErrorResponse {
        id: request_id,
        error: ErrorBody {
            code: code.into(),
            message,
...
fn encode_agent_response(response: ErrorResponse) -> std::io::Result<String> {
...
mod tests {
    ...
    #[test]
    fn m840_agent_wait_defaults_and_sequence_gate_are_explicit() {
        ...
        assert!(agent_wait_matches(
            &agent(crate::api::schema::AgentStatus::Blocked, 7),
            &defaults,
            None,
        ));
        assert!(!agent_wait_matches(
            &agent(crate::api::schema::AgentStatus::Idle, 7),
            &defaults,
            Some(7),
        ));
        assert!(agent_wait_matches(
            &agent(crate::api::schema::AgentStatus::Idle, 8),
            &defaults,
            Some(7),
        ));
        assert!(!agent_wait_matches(
            &agent(crate::api::schema::AgentStatus::Unknown, 8),
            &defaults,
            None,
        ));
    }
    ...
    #[test]
    fn m840_agent_wait_pins_terminal_name_and_known_agent() {
        let current = agent(crate::api::schema::AgentStatus::Idle, 8);
    ...
    #[test]
    fn m840_agent_wait_probe_only_translates_agent_disappearance() {
    ...
    #[test]
    fn m840_agent_release_only_matches_an_observed_requested_status() {
        use crate::api::schema::AgentStatus::{Blocked, Done, Idle, Working};

        assert_eq!(
            matching_release_status(&[Idle, Done], Some(Done), None),
            Some(Done)
        );
        assert_eq!(
            matching_release_status(&[Blocked], Some(Done), Some(Blocked)),
            Some(Blocked)
        );
        assert_eq!(matching_release_status(&[Working], Some(Done), None), None);
    }
