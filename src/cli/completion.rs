...
pub(super) fn run_completion_command(args: &[String]) -> std::io::Result<i32> {
    ...
    // Generate into memory so stdout errors use the CLI's normal I/O error path.
    let mut output = Vec::new();
    generate(shell, &mut command, "zynk", &mut output);
...
fn print_completion_help() {
    eprintln!("usage: zynk completion <{}>", supported_shells_usage());
}
