...
pub(super) fn run_plugin_command(args: &[String]) -> std::io::Result<i32> {
    ...
    // `zynk plugin <leaf> --help` -> group help, EXCEPT the `action`/`pane` sub-groups,
    // which own their own `--help` (so `plugin action --help` keeps its sub-group help).
    if crate::cli::leaf_help_requested(args) && !matches!(subcommand, "action" | "pane") {
        print_plugin_help();
        return Ok(0);
    }
...
fn plugin_link(args: &[String]) -> std::io::Result<i32> {
    let Some(path) = args.first() else {
        eprintln!("usage: zynk plugin link <path> [--disabled]");
        return Ok(2);
    };
...
fn plugin_config_dir_command(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: zynk plugin config-dir <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk plugin config-dir <plugin_id>");
...
fn plugin_unlink(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: zynk plugin unlink <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk plugin unlink <plugin_id>");
...
fn plugin_install(args: &[String]) -> std::io::Result<i32> {
    let Some(source_arg) = args.first() else {
        eprintln!("usage: zynk plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
        return Ok(2);
    };
    ...
    let install_result = (|| {
        ...
        let install_attempt = (|| {
            if let Some(parent) = final_checkout.parent() {
                crate::plugin_paths::ensure_private_dir(
                    &crate::plugin_paths::managed_plugins_dir(),
                )
...
fn plugin_uninstall(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: zynk plugin uninstall <plugin_id|owner/repo[/subdir...]>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk plugin uninstall <plugin_id|owner/repo[/subdir...]>");
...
fn plugin_action_invoke(args: &[String]) -> std::io::Result<i32> {
    let Some(action_id) = args.first() else {
        eprintln!("usage: zynk plugin action invoke <action_id> [--plugin ID]");
        return Ok(2);
    };
    let mut plugin_id = None;
    let mut index = 1;
...
fn plugin_pane_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: zynk plugin pane focus <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk plugin pane focus <pane_id>");
...
fn plugin_pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: zynk plugin pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk plugin pane close <pane_id>");
...
impl GithubPluginSource {
    fn parse(value: &str) -> Result<Self, String> {
        ...
        if parts.len() < 2 {
            return Err("usage: zynk plugin install <owner>/<repo>[/subdir...]".into());
        }
        let owner = parts[0];
        let repo = parts[1];
...
fn verify_plugin_link_source_response(
        ...
        || plugin.source.managed_path != expected.managed_path
    {
        return Err(std::io::Error::other(
            "running zynk server did not persist GitHub plugin source metadata",
        ));
    }
    Ok(())
}
...
fn ensure_manifest_unchanged_after_build(
    ...
    Err(io::Error::other(
        "plugin build changed zynk-plugin.toml after install preview; aborting install",
    ))
}
...
fn run_plugin_build_command(
    ...
    child
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    scrub_zynk_runtime_env(&mut child);
...
fn scrub_zynk_runtime_env(command: &mut Command) {
    for key in [
    ...
    for (key, _) in std::env::vars_os() {
        if key.to_string_lossy().starts_with("ZYNK_PLUGIN_") {
            command.env_remove(key);
        }
    }
}
...
fn create_plugin_temp_dir(label: &str) -> std::io::Result<PathBuf> {
    let managed_dir = crate::plugin_paths::managed_plugins_dir();
    crate::plugin_paths::ensure_private_dir(&managed_dir)?;
    let path = managed_dir.join(format!(
        ".tmp-{label}-{}-{}",
        std::process::id(),
        current_unix_ms()
    ));
    crate::plugin_paths::ensure_private_dir(&path)?;
    Ok(path)
}
...
fn is_connection_error(err: &std::io::Error) -> bool {
    super::server_not_running_response(err).is_some()
        || matches!(
            err.kind(),
...
fn print_plugin_help() {
    eprintln!("zynk plugin commands:");
    eprintln!("  zynk plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
    eprintln!("  zynk plugin uninstall <plugin_id|owner/repo[/subdir...]>");
    eprintln!("  zynk plugin link <path> [--disabled]");
    eprintln!("  zynk plugin list [--plugin ID] [--json]");
    eprintln!("  zynk plugin config-dir <plugin_id>");
    eprintln!("  zynk plugin unlink <plugin_id>");
    eprintln!("  zynk plugin enable <plugin_id>");
    eprintln!("  zynk plugin disable <plugin_id>");
    eprintln!("  zynk plugin action <list|invoke>");
    eprintln!("  zynk plugin log list [--plugin ID] [--limit N]");
    eprintln!("  zynk plugin pane <open|focus|close>");
}

fn print_plugin_action_help() {
    eprintln!("zynk plugin action commands:");
    eprintln!("  zynk plugin action list [--plugin ID]");
    eprintln!("  zynk plugin action invoke <action_id> [--plugin ID]");
}
...
mod tests {
    ...
    fn github_plugin(
        ...
        InstalledPluginInfo {
            ...
            source: PluginSourceInfo {
                ...
                managed_path: Some(format!("/tmp/zynk/plugins/{id}")),
    ...
    fn github_plugin_source_parses_root_repo() {
        let source = GithubPluginSource::parse("example/plugin-examples").unwrap();
        assert_eq!(source.owner, "example");
        assert_eq!(source.repo, "plugin-examples");
        assert_eq!(source.subdir, None);
        assert_eq!(
            source.remote_url(),
            "https://github.com/example/plugin-examples.git"
        );
    }
    ...
    fn github_plugin_source_parses_subdir() {
        let source =
            GithubPluginSource::parse("example/plugin-examples/worktree-bootstrap").unwrap();
        assert_eq!(source.owner, "example");
        assert_eq!(source.repo, "plugin-examples");
    ...
    fn github_plugin_source_rejects_non_shorthand_sources() {
        for source in [
            "https://github.com/example/plugin-examples",
            "git@github.com:example/plugin-examples.git",
            "example",
            "example/plugin-examples/../bad",
            "example/plugin-examples//bad",
        ] {
            assert!(
    ...
    fn github_source_lookup_matches_installed_plugin_source() {
        let source =
            GithubPluginSource::parse("example/plugin-examples/agent-telegram-notify").unwrap();
        let plugins = vec![
            github_plugin(
    ...
    fn github_source_lookup_ignores_local_plugins() {
        let source = GithubPluginSource::parse("example/plugin-examples").unwrap();
        let mut plugin = github_plugin("examples.local", "example", "plugin-examples", None);
