...
pub(super) fn run_status_command(args: &[String]) -> std::io::Result<i32> {
    // `zynk status <server|client> --help` (exact position) -> status help.
...
fn parse_status_args(args: &[String]) -> Option<(StatusScope, bool)> {
    match args.first().map(|arg| arg.as_str()) {
        ...
        Some("server") => {
            parse_status_scope_args(args, StatusScope::Server, "zynk status server [--json]")
        }
        Some("client") => {
            parse_status_scope_args(args, StatusScope::Client, "zynk status client [--json]")
        }
        Some("help" | "--help" | "-h") => {
            if args.len() > 1 {
                print_status_help();
...
fn print_status_help() {
    eprintln!("zynk status commands:");
    eprintln!("  zynk status [--json]         show local client and running server status");
    eprintln!("  zynk status server [--json]  show running server status");
    eprintln!("  zynk status client [--json]  show local client binary status");
}
