...
use crate::config::ToastZynkPosition;
...
fn notification_show(args: &[String]) -> std::io::Result<i32> {
    let params = match parse_notification_show_args(args) {
        ...
        Err(NotificationShowArgError::Usage) => {
            eprintln!(
                "usage: zynk notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
            );
            return Ok(2);
        }
...
fn parse_toast_position(value: &str) -> Result<ToastZynkPosition, NotificationShowArgError> {
    match value {
        "top-left" => Ok(ToastZynkPosition::TopLeft),
        "top-right" => Ok(ToastZynkPosition::TopRight),
        "bottom-left" => Ok(ToastZynkPosition::BottomLeft),
        "bottom-right" => Ok(ToastZynkPosition::BottomRight),
...
fn print_notification_help() {
    eprintln!("zynk notification commands:");
    eprintln!(
        "  zynk notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
    );
}
    ...
    fn notification_show_args_parse_title_body_and_position() {
        ...
        assert_eq!(
            ...
            NotificationShowParams {
                ...
                position: Some(ToastZynkPosition::TopRight),
