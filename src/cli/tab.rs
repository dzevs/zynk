...
fn tab_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: zynk tab get <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk tab get <tab_id>");
...
fn tab_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: zynk tab focus <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk tab focus <tab_id>");
...
fn tab_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: zynk tab rename <tab_id> <label>");
        return Ok(2);
    }
...
fn tab_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_tab_id) = args.first() else {
        eprintln!("usage: zynk tab close <tab_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: zynk tab close <tab_id>");
...
fn print_tab_help() {
    eprintln!("zynk tab commands:");
    eprintln!("  zynk tab list [--workspace <workspace_id>]");
    eprintln!(
        "  zynk tab create [--workspace <workspace_id>] [--cwd PATH] [--label TEXT] [--focus] [--no-focus]"
    );
    eprintln!("  zynk tab get <tab_id>");
    eprintln!("  zynk tab focus <tab_id>");
    eprintln!("  zynk tab rename <tab_id> <label>");
    eprintln!("  zynk tab close <tab_id>");
}
