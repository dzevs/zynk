...
pub(crate) use self::sidebar::agent_panel_scroll_for_target;
...
/// Desktop background tabs cannot share one terminal area any more: with
/// `hide_tab_bar_when_single_tab` a workspace's terminal area depends on ITS OWN tab count,
/// so each workspace resolves its own geometry before its background tabs are resized.
...
/// Split a desktop workspace's main area into its tab row and terminal area. The row is
/// dropped when `hide_tab_bar_when_single_tab` is set and the workspace holds one tab, so the
/// pane reclaims the line; a second tab brings it back.
    ...
    let toast_hit_area = app
        ...
        .map(|toast| {
            toast_notification_rect(
                ...
                toast.position.unwrap_or(app.toast_config.zynk.position),
            )
        })
        .unwrap_or_default();
...
mod tests {
    ...
    fn copy_feedback_offset_only_increases_when_toast_rect_overlaps() {
        ...
        let bottom_right_toast = toast_notification_rect(
            ...
            &toast,
            false,
            crate::config::ToastZynkPosition::BottomRight,
        );
        assert_eq!(
    ...
    fn desktop_toast_hit_area_uses_full_frame_not_terminal_area() {
        ...
        app.selected = 0;
        app.mode = Mode::Terminal;
        app.toast_config.zynk.position = crate::config::ToastZynkPosition::TopLeft;
    ...
    #[test]
    fn bottom_tab_position_does_not_change_mobile_or_one_row_geometry() {
        ...
        assert_eq!(app.view.tab_bar_rect, Rect::default());
        assert_eq!(app.view.terminal_area, Rect::new(0, 2, 44, 18));
        compute_view(&mut app, Rect::new(0, 0, 80, 1));
        assert_eq!(app.view.tab_bar_rect, Rect::default());
    ...
    fn expanded_sidebar_workspace_rows_show_state_before_name_without_numbers() {
        ...
        // No-agent workspace now uses the ◌ glyph in `spaces` (minimal-polish glyph tweak).
        assert!(line1.starts_with(" ◌ one"));
        assert!(!line1.contains("1 one"));
        // Branch line now carries the ⎇ icon before the branch (minimal-polish item 1), folded into
        // the existing indent so the branch text stays column-aligned under the name.
    ...
    fn temp_git_repo(branch: &str) -> std::path::PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("unix time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("zynk-ui-test-{unique}"));
