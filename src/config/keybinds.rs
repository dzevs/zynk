...
mod tests {
    #[test]
    fn m833_popup_config_size_is_typed_and_non_popup_size_is_diagnostic() {
        let config: crate::config::Config = toml::from_str(
            r#"
    [[keys.command]]
    key = "prefix+g"
    command = "printf popup"
    type = "popup"
    width = 90
    height = "80%"
    "#,
        ...
        assert_eq!(keys.custom_commands.len(), 1);
        assert_eq!(
            keys.custom_commands[0].action,
            crate::config::CustomCommandAction::Popup
        );
        assert_eq!(
            keys.custom_commands[0].width,
            Some(crate::popup_size::PopupSize::Cells(90))
        );
        assert_eq!(
            keys.custom_commands[0].height,
            Some(crate::popup_size::PopupSize::Percent(80))
        );
        for action in ["pane", "shell"] {
    ...
    "#
            ...
            let keys = config.keybinds();
            assert_eq!(keys.custom_commands.len(), 1);
            assert_eq!(keys.custom_commands[0].width, None);
            assert_eq!(keys.custom_commands[0].height, None);
            assert!(config
                .collect_diagnostics()
                .iter()
                .any(|s| s.contains("popup size on non-popup custom command")));
        }
        for invalid in ["0%", "101%", "01%", "+1%", "1.0%"] {
            let text = format!("[[keys.command]]\nkey = \"prefix+g\"\ncommand = \"printf popup\"\ntype = \"popup\"\nwidth = \"{invalid}\"\n");
            assert!(
                toml::from_str::<crate::config::Config>(&text).is_err(),
                "{invalid}"
            );
        }
    }
