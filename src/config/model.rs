...
/// Feature #107 (IM3, Q1/Q4): default content-width cap for the agent-visible message
/// header box (columns). The header truncates fields to fit within this cap.
pub const DEFAULT_HEADER_MAX_WIDTH: usize = 100;

/// Feature #107 (IM3, Q4): sane floor for `[header].max_width`. A box narrower than this
/// cannot hold the field labels legibly, so configured/clamped values are raised to it.
pub const MIN_HEADER_MAX_WIDTH: usize = 24;

/// Feature #116 (Settings UI, Q4): upper bound for `[header].max_width` written by the
/// Settings stepper. A box wider than this wastes horizontal space; the stepper clamps
/// the persisted value to `[MIN_HEADER_MAX_WIDTH, MAX_HEADER_MAX_WIDTH]` ([24, 200]).
/// This bounds only the Settings write path; `HeaderConfig::resolve` (the per-send
/// reader) keeps its single floor-only clamp from #107.
pub const MAX_HEADER_MAX_WIDTH: usize = 200;

/// Feature #107 (IM3, Q1): env override that forces the verbose header (the `reply:`/`note:`
/// lines that are HIDDEN by default). Truthy = `1` or `true` (case-insensitive).
pub const HEADER_VERBOSE_ENV_VAR: &str = "ZYNK_HEADER_VERBOSE";
...
/// header box. `verbose` re-adds the `reply:`/`note:` lines hidden by default (Q1);
/// `max_width` caps the content-width box and is clamped to `MIN_HEADER_MAX_WIDTH` (Q4).
#[derive(Debug, Clone, Copy, Deserialize)]
#[serde(default)]
...
impl Default for HeaderConfig {
    fn default() -> Self {
        Self {
            verbose: false,
            max_width: DEFAULT_HEADER_MAX_WIDTH,
        }
    }
}

/// The resolved options the header renderer consumes (see `render_header`). Built by
/// `HeaderConfig::resolve` so env + config + the width floor are applied in one place.
...
impl HeaderConfig {
    /// Resolve the effective header options: `verbose` is the `ZYNK_HEADER_VERBOSE` env
    /// truthy OR the config flag; `max_width` is the config value clamped up to
    /// `MIN_HEADER_MAX_WIDTH`. `env_verbose` is the raw env value (`None` when unset).
    pub fn resolve(&self, env_verbose: Option<&str>) -> HeaderOptions {
        let env_truthy = env_verbose
            .map(|raw| {
                let v = raw.trim();
            ...
            .unwrap_or(false);
        HeaderOptions {
            verbose: env_truthy || self.verbose,
            max_width: self.max_width.max(MIN_HEADER_MAX_WIDTH),
        }
    }
}
...
pub enum ToastDelivery {
    #[default]
    Off,
    // zynk fork (ADR 0007 §3): user-facing config reads `delivery = "zynk"`; the
    // legacy `"zynk"` spelling still parses (bounded transitional compat alias).
    #[serde(rename = "zynk")]
    Zynk,
    Terminal,
    System,
...
pub struct ToastConfig {
    ...
    pub zynk: ZynkToastConfig,
    pub clipboard: ClipboardToastConfig,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
pub struct ZynkToastConfig {
    pub position: ToastZynkPosition,
}
...
pub struct SessionConfig {
    ...
    /// when restoring a Zynk session. Default: true.
...
pub struct KeysConfig {
    ...
    /// Local-client shortcut that sends a clipboard image to a remote zynk session. Default: "ctrl+v".
...
pub struct WorktreesConfig {
    /// Root directory under which Zynk creates <repo>/<branch-slug> checkouts.
...
pub struct UiConfig {
    ...
    /// Capture mouse input for Zynk's mouse UI. Default: true.
    ...
    pub tab_bar_right: Vec<super::TabBarRightEntryConfig>,
...
pub struct RemoteConfig {
    /// Reuse one managed SSH connection for remote discovery, install, and the
    /// bridge, while adding keepalive fallbacks after the user's config. Set
    /// false to run independent plain ssh commands unchanged. Default: true.
...
pub struct ExperimentalConfig {
    /// Allow launching zynk inside an existing zynk pane. Default: false.
...
impl Default for ToastConfig {
    fn default() -> Self {
        Self {
            ...
            delay_seconds: 1,
            zynk: ZynkToastConfig::default(),
            clipboard: ClipboardToastConfig::default(),
        }
    }
}

impl Default for ZynkToastConfig {
    fn default() -> Self {
        Self {
            position: ToastZynkPosition::BottomRight,
        }
    }
}
...
impl<'de> Deserialize<'de> for ToastConfig {
        ...
        D: Deserializer<'de>,
    {
        ...
        struct RawToastConfig {
            ...
            // zynk fork (ADR 0007 §3): canonical TOML key is `[ui.toast.zynk]`;
            // the legacy `[ui.toast.zynk]` table still parses (compat alias).
            #[serde(rename = "zynk")]
            zynk: ZynkToastConfig,
            clipboard: ClipboardToastConfig,
        }

        let raw = RawToastConfig::deserialize(deserializer)?;
        let legacy_delivery = match raw.enabled {
            Some(true) => ToastDelivery::Zynk,
...
mod tests {
    ...
    #[test]
    fn b3_server_headless_size_defaults_parses_and_rejects_zero() {
        let defaults = Config::default();
        assert_eq!(
            defaults.headless_size(),
            (
...
"#,
        ...
        .expect("parse server size");
        assert_eq!(config.headless_size(), (132, 41));
...
"#,
        ...
        .expect("parse invalid server size for diagnostics");
...
tab_bar_right = [
  ...
  { type = "text", text = "prod" },
  { type = "command", command = "printf ready", interval_seconds = 10, timeout_seconds = 3 },
]
...
"#;
    ...
    fn worktrees_directory_defaults_and_parses() {
        let default_config = Config::default();
        assert_eq!(default_config.worktrees.directory, "~/.zynk/worktrees");

        let toml = r#"
[worktrees]
directory = "~/Projects/zynk-worktrees"
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.worktrees.directory, "~/Projects/zynk-worktrees");
    }
...
"#;
        ...
        assert_eq!(config.ui.toast.zynk.position, ToastZynkPosition::TopLeft);
    ...
    // zynk fork (ADR 0007 §3): canonical user-facing spelling is `zynk`
    // (`delivery = "zynk"` + `[ui.toast.zynk]`); the legacy `zynk` spelling
    // still parses as a bounded transitional compat alias.
    #[test]
    fn toast_config_parses_canonical_zynk_spelling() {
        let toml = r#"
[ui.toast]
delivery = "zynk"
...
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert_eq!(config.ui.toast.delivery, ToastDelivery::Zynk);
        assert_eq!(config.ui.toast.zynk.position, ToastZynkPosition::TopRight);
    }
    ...
    fn toast_config_defaults_preserve_existing_behavior_with_delay() {
        ...
        assert_eq!(
            config.ui.toast.zynk.position,
            ToastZynkPosition::BottomRight
        );
        assert!(config.ui.toast.clipboard.enabled);
        assert_eq!(
            config.ui.toast.clipboard.position,
...
"#;
    ...
    #[test]
    fn toast_config_legacy_enabled_true_maps_to_zynk() {
        let toml = r#"
[ui.toast]
enabled = true
"#;
...
"#;
    ...
    #[test]
    fn header_config_defaults_hide_verbose_and_cap_at_100() {
        // Feature #107 (IM3, Q1/Q4): reply/note HIDDEN by default; default cap is 100.
        let config = Config::default();
        assert!(!config.header.verbose);
        assert_eq!(config.header.max_width, DEFAULT_HEADER_MAX_WIDTH);
        assert_eq!(config.header.max_width, 100);
    }

    #[test]
    fn header_config_parses() {
        let toml = r#"
[header]
verbose = true
max_width = 80
"#;
        let config: Config = toml::from_str(toml).unwrap();
        assert!(config.header.verbose);
        assert_eq!(config.header.max_width, 80);
    }

    #[test]
    fn header_resolve_env_truthy_forces_verbose() {
        let cfg = HeaderConfig::default();
        // Config verbose=false, but env truthy → resolved verbose.
        for raw in ["1", "true", "TRUE", " true "] {
            assert!(
                cfg.resolve(Some(raw)).verbose,
                "env {raw:?} should force verbose"
            );
        }
        // Falsey / unset env keeps the config value (false here).
        for raw in [Some("0"), Some("false"), Some(""), None] {
            assert!(
                !cfg.resolve(raw).verbose,
                "env {raw:?} should not force verbose"
            );
        }
        // Config verbose=true wins even when env is unset/falsey.
        let cfg_on = HeaderConfig {
            verbose: true,
            max_width: DEFAULT_HEADER_MAX_WIDTH,
        };
        assert!(cfg_on.resolve(None).verbose);
        assert!(cfg_on.resolve(Some("0")).verbose);
    }

    #[test]
    fn header_resolve_clamps_max_width_to_floor() {
        let cfg = HeaderConfig {
            verbose: false,
            max_width: 4,
        };
        assert_eq!(cfg.resolve(None).max_width, MIN_HEADER_MAX_WIDTH);
        assert_eq!(MIN_HEADER_MAX_WIDTH, 24);

        let cfg_wide = HeaderConfig {
            verbose: false,
            max_width: 120,
        };
        assert_eq!(cfg_wide.resolve(None).max_width, 120);
    }
}
