...
Specialist personas that play a single role with a single perspective for the zynk codebase (Rust + ratatui + tokio TUI; Unix-socket IPC; a binary client frame protocol; a SQLite conversation layer). Each persona is a Markdown file consumed as a system prompt by your harness (Claude Code, Cursor, Copilot, etc.).

| Persona | Role | Best for |
|---------|------|----------|
| [code-reviewer](code-reviewer.md) | Senior Staff Engineer | Five-axis review before merge (correctness, readability, architecture invariants, security, performance) |
| [security-auditor](security-auditor.md) | Security Engineer | Local-socket threat model — untrusted PTY/frame input, socket permissions, hook-authoritative identity, fail-closed DB |
| [test-engineer](test-engineer.md) | QA Engineer | Hermetic test strategy, coverage analysis, characterization/parity gaps, Prove-It pattern |

To invoke a persona directly outside a slash command, read `.agents/agents/<persona>.md` and adopt it as the system prompt for the review.
...
|-------|-----------|---------|------------------|
| **Skill** | A workflow with steps and exit criteria | `code-review-and-quality` (`.agents/skills/`) | The *how* — invoked from inside a persona or command |
...
The user (or a slash command) is the orchestrator. **Personas do not call other personas.** Skills are mandatory hops inside a persona's workflow — the zynk workflow skills live under `.agents/skills/<skill-name>/SKILL.md` with supporting checklists under `.agents/references/`.
...
- "Review this diff" → invoke `code-reviewer` directly
- "Are there security issues in the new frame decoder in `src/protocol/wire.rs`?" → invoke `security-auditor` directly
- "What tests are missing for the delivery-status transition matrix?" → invoke `test-engineer` directly

### Slash command (single persona behind it)
Pick this when there's a repeatable workflow you'd otherwise re-explain every time.

- `/review` → wraps `code-reviewer` with the project's `code-review-and-quality` skill
- `/test` → wraps `test-engineer` with the `test-driven-development` skill
...
3. A persona may invoke skills (the *how*) — the zynk workflow skills under `.agents/skills/`.
4. Every persona file ends with a "Composition" block stating where it fits.
5. Personas review against zynk's load-bearing invariants (state ≠ runtime, render-is-pure, detection-reads-snapshot-only, hook-authoritative identity, body purity, submit ≠ receipt) — see `CLAUDE.md` and the ADRs in `docs/zynk/decisions/`.
...
1. Create `.agents/agents/<role>.md` with the same frontmatter format used by existing personas (`name:` = the file basename without `.md`; a `description:`).
2. Define the role, scope, output format, and rules — grounded in zynk's invariants and conventions (`CLAUDE.md`, `docs/zynk/`).
3. Add a **Composition** block at the bottom (Invoke directly when / Invoke via / Invoke skills / Do not invoke from another persona).
4. Add the persona to the table at the top of this file.
5. If the persona links to a checklist, point it at `.agents/references/<name>.md`.
