...
test:
    ...
    python3 -m unittest scripts.test_agent_detection_manifest_check scripts.test_vendor_libghostty_vt scripts.test_vendor_portable_pty scripts.test_conventional_commits scripts.test_check_public_tree scripts.test_gitleaks_config scripts.test_scrub_check scripts.test_skills_catalog scripts.test_release_audit_refs scripts.test_gitleaks_tracked scripts.test_release_evidence scripts.test_release_manifest scripts.test_release_artifact_ids scripts.test_release_workflow
...
# failures before the hosted Windows job. NOTE: zynk's bundled C deps (sqlite-vec /
...
# check is the candidate-evidence workflow's `test-windows-x86_64` job (release-dryrun.yml, on
# demand per ADR 0012). (mirrors upstream b7a504b)
windows-lint:
